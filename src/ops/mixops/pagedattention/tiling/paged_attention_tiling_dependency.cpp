...
static const std::unordered_map<PlatformType, IndexArr> nzTilingOffsetMap{
...
{
    return nzTilingOffsetMap.at(PlatformInfo::Instance().GetPlatformType());
}
...
void SetTilingKey(const PagedAttentionInfo &mmInfo, uint32_t *tilingParam, bool isMLA,
                  const OpParam::PagedAttention &param, const bool &mlaOptimization)
{
    // 目前10bit数据 00 00 00 00 00 (isMLA)
    // optfeature (MLA优化):1bit (compressHead isMLA):2bit
    // (dataFormat isParallePa splitkv):4bit quantmode:2bit
    // (dequantCube, dequantVec, dequantMsd, quant) qdatatype:2bit(FP16/BF16/INT8)
    ...
    if (param.type == OpParam::PagedAttention::PAGED_MULTI_LATENT_ATTENTION_COMBINE_CACHE_MASK_ND) {
        ...
        tilingKey = (mmInfo.compressHead << NUM9) +
...
Status GetNdPagedAttentionTiling(const PagedAttentionInfo &mmInfo, uint32_t &blockDim, uint32_t *tilingParam,
                                 const OpParam::PagedAttention &param)
{
    uint32_t prefillBatchSize = 0;
    uint32_t decoderBatchSize = 0;
    uint32_t paOnly = (mmInfo.qSeqLen == nullptr) ? 1 : 0;
    AddrOffsets addrOffsets;
    ...
    if (param.scaleType == OpParam::PagedAttention::SCALE_LOGN_FP32) {
        MKI_CHECK(!isMLA, "MLA does not support logN", return Status::FailStatus(ERROR_INVALID_VALUE));
    }
    ...
    SetTilingKey(mmInfo, tilingParam, isMLA, param, mlaOptimization);
