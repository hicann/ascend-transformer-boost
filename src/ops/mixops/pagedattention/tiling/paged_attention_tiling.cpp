...
Status GetPagedAttentionMaskInfo(const LaunchParam &launchParam, PagedAttentionInfo &mmInfo,
                                 OpParam::PagedAttention &param, bool isMLA)
{
    Tensor maskTensor = param.type == OpParam::PagedAttention::PAGED_MULTI_LATENT_ATTENTION_COMBINE_CACHE_MASK_ND ?
                        launchParam.GetInTensor(DIM_3) : launchParam.GetInTensor(DIM_4);
    ...
    if ((qTensor.desc.dtype == TENSOR_DTYPE_FLOAT16) && kTensor.desc.dtype == TENSOR_DTYPE_INT8) {
    ...
    } else if ((qTensor.desc.dtype == TENSOR_DTYPE_BF16) && kTensor.desc.dtype == TENSOR_DTYPE_INT8 &&
               deqScale1.desc.dtype == TENSOR_DTYPE_FLOAT) {
        mmInfo.type = TilingKeyType::TILING_INT8_VEC_QUANTBF16;
        MKI_CHECK(!isMLA, "MLA does not support antiquant for now",
                return Status::FailStatus(ERROR_INVALID_VALUE));
    } else if ((qTensor.desc.dtype == TENSOR_DTYPE_INT8) && (outTensor.desc.dtype == TENSOR_DTYPE_FLOAT16)) {
        mmInfo.type = TilingKeyType::TILING_QUANT_FP16OUT;
        mmInfo.tBlockAlign = BLOCK_SIZE_32;
    } else if ((qTensor.desc.dtype == TENSOR_DTYPE_INT8) && (outTensor.desc.dtype == TENSOR_DTYPE_BF16)) {
        mmInfo.type = TilingKeyType::TILING_QUANT_BF16OUT;
        mmInfo.tBlockAlign = BLOCK_SIZE_32;
    } else if (qTensor.desc.dtype == TENSOR_DTYPE_BF16) {
                               ...
                               OpParam::PagedAttention &param)
{
    ...
    auto tableShape = param.type == OpParam::PagedAttention::PAGED_MULTI_LATENT_ATTENTION_COMBINE_CACHE_MASK_ND ?
                      launchParam.GetInTensor(DIM_2).desc.dims : launchParam.GetInTensor(3).desc.dims;
...
Status PagedAttentionTiling(const LaunchParam &launchParam, KernelInfo &kernelInfo)
{
    ...
    if (param.type == OpParam::PagedAttention::PAGED_ATTENTION_MASK_ND ||
        ...
        // (num_tokens, num_heads, kvsplit, head_size)
        uint32_t oCoreTempSize = blockDim * 0.8 * static_cast<uint32_t>(mmInfo.numHeads) *
            blockDim *  static_cast<uint32_t>(mmInfo.embeddingSize * sizeof(float));
        // (num_tokens, num_heads, kvsplit)
        uint32_t lCoreTempSize = blockDim * 0.8 * static_cast<uint32_t>(mmInfo.numHeads) *
            blockDim * sizeof(float);
        uint32_t k16TempSize = 2 * blockDim * 256 * static_cast<uint32_t>(mmInfo.numHeads) *
                               static_cast<uint32_t>(mmInfo.embeddingSize) * sizeof(uint16_t);
        uint32_t v16TempSize = 2 * blockDim * 256 * static_cast<uint32_t>(mmInfo.numHeads) *
                               static_cast<uint32_t>(mmInfo.embeddingSize) * sizeof(uint16_t);

        kernelInfo.GetScratchSizes() = {blockDim * 65536 * dataLenHalf,      // gmOutputS
                                        blockDim * 65536 * dataLenHalf,      // gmOutputP
                                        blockDim * 65536 * 2 * dataLenHalf * 2, // gmOutPutTempO
                                        blockDim * 65536 * 2 * dataLenHalf,
                                        oCoreTempSize,
                                        lCoreTempSize,
                                        k16TempSize,
                                        v16TempSize,
        ...
        MKI_LOG(INFO) << "TilingId: " << tilingParam[TILING_KEY_ID];
    }
