...
inline bool CalculateMaxUbSizePerRow(SwiGluQuantTilingData *tilingData)
{
    ...
    if (ubAvail == 0) {
        ...
        tilingData->optBaseColLen =  AlignDown(tilingData->dataNumSingleUb, tilingData->blockNum);
        if (tilingData->optBaseColLen > tilingData->cacheLineLen) {
            tilingData->optBaseColLen =  AlignDown<uint32_t>(tilingData->optBaseColLen, tilingData->cacheLineLen);
        }
        ubAvail = ONE;
    } else {
...
void CalTilingData(SwiGluQuantTilingData *tilingData)
{
    ...
    CalculateMaxUbSizePerRow(tilingData);
}
