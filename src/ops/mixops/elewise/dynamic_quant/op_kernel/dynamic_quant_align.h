...
#include "mixops/elewise/dynamic_quant/dynamic_quant_tiling/dynamic_quant_util.h"
...
public:
    ...
    __aicore__ inline void InitParams(AtbOps::DynamicQuantTilingData *tilingdata)
    {
    ...
    {
        pipe_.InitBuffer(inQueue_, AtbOps::DYNAMIC_QUANT_BUFFER_NUM_TWO,
                         lenCopyRow_ * AtbOps::DYNAMIC_QUANT_SIZE_OF_HALF);
        pipe_.InitBuffer(outQueue_, AtbOps::DYNAMIC_QUANT_BUFFER_NUM_ONE,
                         lenCopyRow_ * AtbOps::DYNAMIC_QUANT_SIZE_OF_INT8);
        pipe_.InitBuffer(scaleQueue_, AtbOps::DYNAMIC_QUANT_BUFFER_NUM_ONE,
                         numCopyRow_ * AtbOps::DYNAMIC_QUANT_SIZE_OF_FLOAT);
        pipe_.InitBuffer(fp16Buf_, sizeH_ * AtbOps::DYNAMIC_QUANT_SIZE_OF_HALF);
        if (asymmetric_) {
            pipe_.InitBuffer(offsetQueue_, AtbOps::DYNAMIC_QUANT_BUFFER_NUM_ONE,
                             numCopyRow_ * AtbOps::DYNAMIC_QUANT_SIZE_OF_FLOAT);
        }
    }
    ...
    __aicore__ inline void Compute(uint32_t calRow, uint32_t sizeZ)
    {
        ...
        for (uint32_t i = 0; i < calRow; i++) {
            ...
            if (maxValue <= AtbOps::DYNAMIC_QUANT_EPSINON) {
                scale = 0.0f;
            } else {
                scale = AtbOps::DYNAMIC_QUANT_MAX_VALUE / maxValue;
            }
        ...
        Muls(scaleLocal, scaleLocal, static_cast<float>(1.0f / AtbOps::DYNAMIC_QUANT_MAX_VALUE),
             calRow);
    ...
    __aicore__ inline void ComputeAsymmetric(uint32_t calRow, uint32_t sizeZ)
    {
        ...
        for (uint32_t i = 0; i < calRow; i++) {
            ...
            if (subValue <= AtbOps::DYNAMIC_QUANT_EPSINON) {
                scale = 0.0f;
            } else {
                scale = AtbOps::DYNAMIC_QUANT_ASYMMETRIC_VALUE / subValue;
            }
            scaleLocal.SetValue(i, subValue / AtbOps::DYNAMIC_QUANT_ASYMMETRIC_VALUE);
...
public:
    ...
    AscendC::TQue<AscendC::QuePosition::VECIN, AtbOps::DYNAMIC_QUANT_BUFFER_NUM_TWO> inQueue_;
    AscendC::TQue<AscendC::QuePosition::VECOUT, AtbOps::DYNAMIC_QUANT_BUFFER_NUM_ONE> outQueue_;
    AscendC::TQue<AscendC::QuePosition::VECOUT, AtbOps::DYNAMIC_QUANT_BUFFER_NUM_ONE> scaleQueue_;
    AscendC::TQue<AscendC::QuePosition::VECOUT, AtbOps::DYNAMIC_QUANT_BUFFER_NUM_ONE> offsetQueue_;
