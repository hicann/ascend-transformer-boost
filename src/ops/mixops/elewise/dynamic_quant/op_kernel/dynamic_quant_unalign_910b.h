...
public:
    ...
    __aicore__ inline void InitParams(AtbOps::DynamicQuantTilingData *tilingdata)
    {
        ...
        lenXSizeH_ = this->sizeH_ * AtbOps::DYNAMIC_QUANT_SIZE_OF_HALF;
        lenZSizeH_ = this->sizeH_ * AtbOps::DYNAMIC_QUANT_SIZE_OF_INT8;
        numRightPad_ = this->sizeX_ - this->sizeH_;

        isSizeXPad_ = this->sizeH_ % AtbOps::DYNAMIC_QUANT_ALIGN_NUM_X != 0;
        isSizeZPad_ = this->sizeH_ % AtbOps::DYNAMIC_QUANT_ALIGN_SIZE != 0;
    }
    ...
    {
        this->pipe_.InitBuffer(this->inQueue_, AtbOps::DYNAMIC_QUANT_BUFFER_NUM_TWO,
            this->sizeX_ * this->numCopyRow_ * AtbOps::DYNAMIC_QUANT_SIZE_OF_HALF);
        this->pipe_.InitBuffer(this->outQueue_, AtbOps::DYNAMIC_QUANT_BUFFER_NUM_ONE,
            sizeZOut_ * this->numCopyRow_ * AtbOps::DYNAMIC_QUANT_SIZE_OF_INT8);
        this->pipe_.InitBuffer(this->scaleQueue_, AtbOps::DYNAMIC_QUANT_BUFFER_NUM_ONE,
            sizeCopyRow_ * AtbOps::DYNAMIC_QUANT_SIZE_OF_FLOAT);
        this->pipe_.InitBuffer(this->fp16Buf_, this->sizeX_ * AtbOps::DYNAMIC_QUANT_SIZE_OF_HALF);
        this->pipe_.InitBuffer(this->fp32Buf_, this->sizeH_ * AtbOps::DYNAMIC_QUANT_SIZE_OF_FLOAT);
        if (this->asymmetric_) {
            this->pipe_.InitBuffer(this->offsetQueue_, AtbOps::DYNAMIC_QUANT_BUFFER_NUM_ONE,
                sizeCopyRow_ * AtbOps::DYNAMIC_QUANT_SIZE_OF_FLOAT);
        }
    }
    ...
    __aicore__ inline void Process()
    {
        ...
        bool isCopyRowPad = this->numCopyRow_ % AtbOps::DYNAMIC_QUANT_ALIGN_NUM_X != 0;
...
private:
    ...
    {
        if (this->isTailLoop_) {
            bool isLastRowPad = this->numLastTailRow_ % AtbOps::DYNAMIC_QUANT_ALIGN_NUM_X != 0;
        ...
        bool isScalePad)
    {
        ...
        uint16_t lenScale = calRow * AtbOps::DYNAMIC_QUANT_SIZE_OF_FLOAT;
        if (isScalePad) {
