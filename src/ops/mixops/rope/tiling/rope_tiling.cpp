...
static constexpr uint32_t LARGE_BATCH_THRESHOLD = 64;
static constexpr uint32_t SLICE_SIZE_FP16_LARGE_BATCH = 4096;
...
static constexpr uint32_t TILING_HIGH_PERF_LARGE_BATCH = 23;
...
void RopeNdProcess(const LaunchParam &launchParam, KernelInfo &kernelInfo, RopeTilingData *tilingDataPtr)
{
    ...
    bool condition = tilingDataPtr->cosFormat == 0 && cosSize == NUM_COSIN &&
                     ...
                     batch >= LARGE_BATCH_THRESHOLD && headDim / tilingDataPtr->rotaryCoeff % ELE_NUM_FP16 == 0;
    if (condition) { // 不对齐场景, multiple为1
        uint32_t hiddenSize = hiddenSizeK > hiddenSizeQ ? hiddenSizeK : hiddenSizeQ;
        multiple = SLICE_SIZE_FP16_LARGE_BATCH / hiddenSize;
        multiple = multiple > 0 ? multiple : 1;
        while (batch % multiple != 0 && multiple > 1) {
            --multiple;
        }
        if (batch / multiple < maxCore || UINT32_MAX / multiple < hiddenSize) {
            multiple = 1;
        } else {
            ntokens /= multiple;
            batch /= multiple;
...
Status TilingKeyChose(const LaunchParam &launchParam, KernelInfo &kernelInfo, const RopeTilingData *tilingDataPtr)
{
    ...
    if (cosSize == NUM_COSIN) {
        if (launchParam.GetInTensor(0).desc.dtype == TENSOR_DTYPE_BF16) {
            ...
            bool condition = (alignRotary == 0) && (tilingDataPtr->batch >= LARGE_BATCH_THRESHOLD);
            if (condition) { // batch >= 64时，走TILING_BF16_ALIGN
            ...
            bool condition = tilingDataPtr->batch * tilingDataPtr->multiple >= LARGE_BATCH_THRESHOLD &&
                             tilingDataPtr->cosFormat == 0;
            if (condition) { // batch >= 64时，走TILING_HIGH_PERF_LARGE_BATCH
                kernelInfo.SetTilingId(TILING_HIGH_PERF_LARGE_BATCH);
            } else {
        ...
        if (launchParam.GetInTensor(0).desc.dtype == TENSOR_DTYPE_BF16) {
            ...
            uint32_t alignRotary = (tilingDataPtr->headDim / tilingDataPtr->rotaryCoeff) % ELE_NUM_FP16;
            bool condition = (alignRotary == 0) && (tilingDataPtr->batch >= LARGE_BATCH_THRESHOLD);
            if (condition) { // batch >= 64时，走TILING_BF16_ALIGN_BROARD
