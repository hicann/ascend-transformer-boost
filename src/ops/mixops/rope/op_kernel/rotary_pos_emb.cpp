...
#include "rotary_pos_emb_fp16_large_batch.h"
                                                     ...
                                                     GM_ADDR workspace, GM_ADDR sync, GM_ADDR tiling)
{
    ...
    if (TILING_KEY_IS(30)) {
        ...
        RopeFp16LargeBatch<half, half, false> ropeFp16LargeBatch(&tiling_data, &pipe);
        ropeFp16LargeBatch.RopeInitGm(q, k, cos, sin, seqLen, outQ, outK);
        ropeFp16LargeBatch.Process(workspace, sync);
    }
}
