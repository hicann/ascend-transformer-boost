...
struct QkCommLargeBatchParams {
    __aicore__ QkCommLargeBatchParams()
    {
        hiddenSizeTmp = 0;
        headNumTemp = 0;
        offsetQK = 0;
    ...
    __aicore__ QkCommLargeBatchParams(const uint32_t hiddenSizeTmp, const uint32_t headNumTemp,
...
public:
    ...
    // qk 大batch场景的公用函数
    template <typename BUF_TYPE>
    __aicore__ inline void QkCommLargeBatch(const AscendC::GlobalTensor<BUF_TYPE> &src,
                                            const AscendC::GlobalTensor<uint8_t> &extraGm1,
                                            const AscendC::LocalTensor<BUF_TYPE> &tempBuf,
                                            QkCommLargeBatchParams qkParams)
    {
...
public:
    ...
    uint32_t blockIdx_;
    uint32_t multiple_{1}; // batch减小，qk增大的倍率
