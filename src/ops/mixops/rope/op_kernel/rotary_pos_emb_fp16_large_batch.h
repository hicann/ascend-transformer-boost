...
*/
#ifndef ROTARY_POS_EMB_FP16_LARGE_BATCH
#define ROTARY_POS_EMB_FP16_LARGE_BATCH
#include "rotary_pos_emb_base.h"

using AscendC::HardEvent;

template <typename QK_DTYPE, typename COS_DTYPE, bool IF_COS_BROADCAST>
class RopeFp16LargeBatch : public RopeBase<QK_DTYPE, COS_DTYPE, IF_COS_BROADCAST> {
public:
    __aicore__ inline RopeFp16LargeBatch(AtbOps::RopeTilingData *tilingData, AscendC::TPipe *pipe)
        :RopeBase<QK_DTYPE, COS_DTYPE, IF_COS_BROADCAST>(tilingData, pipe)
    {
        ...
        this->sliceSizeTmp_ = (SLICE_SIZE_FP16_LARGE_BATCH / this->tilingData_->headDim) *
    ...
    __aicore__ inline void CalcOutQK(CalcOutQKParams &calcOutQKParams)
    {
        for (uint32_t perSlice = 0; perSlice < calcOutQKParams.sliceTime; ++perSlice) {
            ...
            QkCommLargeBatchParams qkParams = {dynamicSliceTemp, headNumTemp, offsetSubPingPong};
            this->QkCommLargeBatch(calcOutQKParams.Gm[outOffset],
