...
public:
    __aicore__ inline PostRmsNormShort(__gm__ uint8_t *x, __gm__ uint8_t *bias, __gm__ uint8_t *res_in,
                                       __gm__ uint8_t *g, __gm__ uint8_t *y, AtbOps::PostRmsNormTilingData &tiling_data)
    {
...
public:
    __aicore__ inline PostRmsNormLong(__gm__ uint8_t *x, __gm__ uint8_t *bias, __gm__ uint8_t *res_in,
                                      __gm__ uint8_t *g, __gm__ uint8_t *y, AtbOps::PostRmsNormTilingData &tiling_data)
    {
        uint32_t numRow = tiling_data.numRow;
        numCore_ = tiling_data.numCore;
        ...
        if constexpr (HAS_BIAS) {
            ...
            pipe.InitBuffer(calc_buf_, sliceSize_ * sizeof(float));
            pipe.InitBuffer(fp16_x_buf_, NUM_THREE * sliceSize_ * sizeof(T));
        }
...
extern "C" __global__ __aicore__ void post_rms_norm(GM_ADDR x, GM_ADDR bias, GM_ADDR res_in, GM_ADDR g, GM_ADDR y,
                                                    GM_ADDR tiling)
{
    GET_TILING_DATA(tiling_data, tiling);
    if (TILING_KEY_IS(0)) { // 000
        PostRmsNormShort<half, true> kernel(x, bias, res_in, g, y, tiling_data);
        kernel.Launch();
    } else if (TILING_KEY_IS(2)) { // 010
        PostRmsNormShort<half, false> kernel(x, bias, res_in, g, y, tiling_data);
        kernel.Launch();
    } else if (TILING_KEY_IS(4)) { // 100
        PostRmsNormLong<half, true> kernel(x, bias, res_in, g, y, tiling_data);
        kernel.Launch();
    } else if (TILING_KEY_IS(6)) { // 110
        PostRmsNormLong<half, false> kernel(x, bias, res_in, g, y, tiling_data);
        kernel.Launch();
    } else if (TILING_KEY_IS(1)) { // 001
#if defined(__CCE_KT_TEST__) || (__CCE_AICORE__ == 220)
        PostRmsNormShort<bfloat16_t, true> kernel(x, bias, res_in, g, y, tiling_data);
        kernel.Launch();
#endif
    } else if (TILING_KEY_IS(3)) { // 011
#if defined(__CCE_KT_TEST__) || (__CCE_AICORE__ == 220)
        PostRmsNormShort<bfloat16_t, false> kernel(x, bias, res_in, g, y, tiling_data);
        kernel.Launch();
#endif
    } else if (TILING_KEY_IS(5)) { // 101
#if defined(__CCE_KT_TEST__) || (__CCE_AICORE__ == 220)
        PostRmsNormLong<bfloat16_t, true> kernel(x, bias, res_in, g, y, tiling_data);
        kernel.Launch();
#endif
    } else if (TILING_KEY_IS(7)) { // 111
#if defined(__CCE_KT_TEST__) || (__CCE_AICORE__ == 220)
        PostRmsNormLong<bfloat16_t, false> kernel(x, bias, res_in, g, y, tiling_data);
        kernel.Launch();
#endif
    }
}