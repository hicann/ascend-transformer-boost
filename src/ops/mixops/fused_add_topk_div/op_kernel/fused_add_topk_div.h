...
public:
    ...
    __aicore__ inline void InitTilingData(AtbOps::FusedAddTopkDivTilingData &__restrict tilingData, GM_ADDR x,
        GM_ADDR addNum, GM_ADDR y, GM_ADDR indices, GM_ADDR workspace);
...
__aicore__ inline void FusedAddTopkDiv<inputT, calT>::InitTilingData(
    AtbOps::FusedAddTopkDivTilingData &__restrict tilingData, GM_ADDR x, GM_ADDR addNum, GM_ADDR y, GM_ADDR indices,
    GM_ADDR workspace)
{
...
__aicore__ inline void FusedAddTopkDiv<inputT, calT>::InitBuffer(TPipe *inputPipe)
{
    ...
    pipe_->InitBuffer(tempBuf_, sizeof(int64_t) * baseCountGroupNum);
    ...
    pipe_->InitBuffer(dataBuf_, sizeof(float) * baseCountGroupNum);
}
...
__aicore__ inline void FusedAddTopkDiv<inputT, calT>::CopyFromWorkspace()
{
    ...
    DataCopyPad(assistLocal, mGmAssist_,
                {(uint16_t)1, (uint32_t)(groupElesAlignSortCount_ * groupNum_ * sizeof(uint32_t)), 0, 0, 0},
                {false, 0, 0, 0});
    event_t eventIDMTE2ToV = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::MTE2_V));
    SetFlag<HardEvent::MTE2_V>(eventIDMTE2ToV);
    WaitFlag<HardEvent::MTE2_V>(eventIDMTE2ToV);
...
__aicore__ inline void FusedAddTopkDiv<inputT, calT>::GroupReduceSumInternelImpl()
{
    ...
    LocalTensor<float> data = dataBuf_.Get<float>();
    ...
    GatherMask(data, sortedTensor, 1, false, 0, gatherMaskParams, rsvdCnt);
    AscendC::PipeBarrier<PIPE_V>();

    for (size_t i = 0; i < groupNum_; i++)
    {
        ReduceSum<calT>(xLocal[i * SORT_UNIT], data[i * SORT_UNIT], tempTensor, n_);
    }
    AscendC::PipeBarrier<PIPE_V>();

    auto gatherOffset = tempTensor.template ReinterpretCast<int32_t>();
