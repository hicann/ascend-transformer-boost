...
namespace AtbOps {
    ...
    constexpr int DIVIDE_UB_NUM = 12;
    ...
    template <typename TilingData>
    void FusedAddTopkDivTiling<TilingData>::GetTilingKey()
    {
        tilingKey = dtype;
    }
    ...
    Status CheckMkiParam(const LaunchParam &launchParam)
    {
        ...
        auto isNorm = static_cast<uint32_t>(param.isNorm);
        ...
        MKI_CHECK(k > 0, "k should be greater than 0.",
                  return Status::FailStatus(ERROR_INVALID_VALUE));
        MKI_CHECK(b % NUM_SIXTEEN == 0, "b should be a multiple of 16.",
                  return Status::FailStatus(ERROR_INVALID_VALUE));
        ...
        MKI_CHECK(activateType == 0, "activateType currently only supports equal to 0.",
                  return Status::FailStatus(ERROR_INVALID_VALUE));
        MKI_CHECK(isNorm == 1, "isNorm currently only supports equal to true.",
                  return Status::FailStatus(ERROR_INVALID_VALUE));
        if (b >= SORT_UNIT) {
