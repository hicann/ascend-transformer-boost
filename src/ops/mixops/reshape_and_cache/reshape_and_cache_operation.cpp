...
public:
    ...
    bool CheckCommonReshapeAndCache(const LaunchParam &launchParam) const
    {
        ...
        MKI_CHECK(value.desc.dtype == indtype && keyCache.desc.dtype == indtype && valueCache.desc.dtype == indtype,
            "K&V&KCache&VCache's dtype must be same", return false);
        ...
        MKI_CHECK(valueCache.desc.dims[DIM_0] == numBlocks &&
            valueCache.desc.dims[DIM_1] == blockSize &&
            valueCache.desc.dims[DIM_2] == keyCache.desc.dims[DIM_2],
            "dim_0, dim_1, dim_2 of KCache/VCache should be same", return false);
        return true;
    }
    ...
    Status CheckReshapeAndCacheNdNz(const LaunchParam &launchParam) const
    {
        ...
        if (param.type == OpParam::ReshapeAndCache::RESHAPE_AND_CACHE_NZ) {
            MKI_CHECK(keyCache.desc.dims[DIM_3] == 16 && valueCache.desc.dims[DIM_3] == 16
                && numHeads * headSizeK == keyCache.desc.dims[DIM_1] * 16
                && numHeads * headSizeV == valueCache.desc.dims[DIM_1] * 16,
                "NZ format tensor dim should be aligned to 16", return Status::FailStatus(ERROR_INFERSHAPE_ERROR));
        }
