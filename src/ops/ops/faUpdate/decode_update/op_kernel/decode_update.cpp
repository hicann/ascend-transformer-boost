...
#include "ops/faUpdate/decode_update/tiling/tiling_data.h"
...
private:
    __aicore__ inline void CopyIn(int32_t progress)
    {
        ...
        if (curLength % ALIGNED_TO_8==0) {
            for (int32_t i = 0; i < sp; i++) {
                DataCopy(LesLocal[curLength*i],
                    lesGm[progress * tileLength + totalLength * i + GmStartOffset],
                    curLength);
            ...
            for (int32_t i = 0; i < sp; i++) {
                DataCopyPad(LesLocal[curLengthAlig*i],
                    lesGm[progress * tileLength + totalLength * i + GmStartOffset],
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        const uint32_t dstShape[2] = {sp*curLengthPad, hDim};
        ...
        for (int32_t i = 0; i < sp; i++) {
            Mul(inLocal[i*curLength*hDim], inLocal[i*curLength*hDim], lesexpLocal[i*curLengthPad*hDim], curLength*hDim);
        }
        PipeBarrier<PIPE_V>();
...
private:
    ...
    uint16_t curLength;          //  Tiling 循环数据长度为 curLength，最后一轮为 lastLength，内存申请使用 tileLength，数据量用 curLength
