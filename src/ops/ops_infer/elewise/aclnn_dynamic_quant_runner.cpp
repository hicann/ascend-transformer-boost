...
aclnnStatus AclnnDynamicQuantRunner::SetAclNNWorkspaceExecutor()
{
    ...
    Status load_status =  AclnnDynamicQuantRunner::LoadMethod();
    if (load_status != NO_ERROR) {
...
Status AclnnDynamicQuantRunner::LoadMethod()
{
    ...
    Status ret = dlManager.getSymbol("aclnnDynamicQuantGetWorkspaceSize",
                                     (void *&)AclnnDynamicQuantRunner::aclnnGetWorkspaceSizeFunc_);
    if (ret != NO_ERROR) {
        ATB_LOG(ERROR) << "load aclnnDynamicQuantGetWorkspaceSize failed! Consider upgrade the CANN first!";
        return ret;
    }
    ret = dlManager.getSymbol("aclnnDynamicQuant", (void *&)AclnnDynamicQuantRunner::aclnnExecuteFunc_);
    if (ret != NO_ERROR) {
        ATB_LOG(ERROR) << "load aclnnDynamicQuant failed! Consider upgrade the CANN first!";
        return ret;
    }
    ATB_LOG(INFO) << "load aclnnDynamicQuant two-staged method success!";
    return NO_ERROR;
}
