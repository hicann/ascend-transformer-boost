...
inline aclTensor *In(const AclNNVariantPack &variantPack)
{
    return variantPack.aclInTensors.at(IN_TENSOR_IDX)->tensor;
}
inline aclTensor *Out(const AclNNVariantPack &variantPack)
{
    return variantPack.aclOutTensors.at(OUT_TENSOR_IDX)->tensor;
}

ActivationAclnnRunner::KernelAdapters ActivationAclnnRunner::MakeAdaptersByType(const infer::ActivationParam &param)
{
    switch (param.activationType) {
        ...
        case infer::ActivationType::ACTIVATION_SWISH:
            return {"swish",
                    [scale = param.scale](const AclNNVariantPack &vp, uint64_t *wsSize, aclOpExecutor **executor) {
                        ...
                        aclnnStatus aclnnstatus = ActivationAclnnRunner::swishGetWorkspaceSizeFunc_(
                            In(vp), scalar, Out(vp), wsSize, executor);
        ...
        default:
            return {"unsupported",
                    ...
                    [](void *, uint64_t, aclOpExecutor *, aclrtStream) { return ACLNN_ERR_INNER_FIND_KERNEL_ERROR; }};
    }
}
...
Status ActivationAclnnRunner::LaunchAclnnKernel()
{
    ...
    aclnnStatus ret = this->execFn_(this->atbVariantPack_.workspaceBuffer, this->atbVariantPack_.workspaceBufferSize,
