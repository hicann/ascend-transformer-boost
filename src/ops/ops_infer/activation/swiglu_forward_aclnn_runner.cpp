...
namespace atb {
// 初始化类函数指针
aclnnStatus (*SwigluForwardAclnnRunner::aclnnGetWorkspaceSizeFunc_)(const aclTensor *x, int64_t dim,
                                                                    const aclTensor *out, uint64_t *workspaceSize,
                                                                    aclOpExecutor **executor) = nullptr;

aclnnStatus (*SwigluForwardAclnnRunner::aclnnExecuteFunc_)(void *, uint64_t, aclOpExecutor *, aclrtStream) = nullptr;
...
aclnnStatus SwigluForwardAclnnRunner::SetAclNNWorkspaceExecutor()
{
    ...
    aclnnStatus ret = SwigluForwardAclnnRunner::aclnnGetWorkspaceSizeFunc_(
        input, -1, output, &(this->atbVariantPack_.workspaceBufferSize), &raw_executor_ptr);
...
Status SwigluForwardAclnnRunner::LaunchAclnnKernel()
{
    ...
    aclnnStatus ret = SwigluForwardAclnnRunner::aclnnExecuteFunc_(this->atbVariantPack_.workspaceBuffer,
...
Status SwigluForwardAclnnRunner::LoadMethod()
{
    ...
    if (SwigluForwardAclnnRunner::aclnnGetWorkspaceSizeFunc_ &&
        SwigluForwardAclnnRunner::aclnnExecuteFunc_) {
        return NO_ERROR;
    }
    static DlManager dlManager = DlManager(std::string(std::getenv("ASCEND_HOME_PATH")) + "/lib64/libopapi.so");
    Status ret = dlManager.getSymbol("aclnnSwiGluGetWorkspaceSize",
                                     (void *&)SwigluForwardAclnnRunner::aclnnGetWorkspaceSizeFunc_);
    if (ret != NO_ERROR) {
        ATB_LOG(ERROR) << "load aclnnSwiGluGetWorkspaceSize failed! Consider upgrade the CANN first!";
        return ret;
    }
    ret = dlManager.getSymbol("aclnnSwiGlu", (void *&)SwigluForwardAclnnRunner::aclnnExecuteFunc_);
    if (ret != NO_ERROR) {
        ATB_LOG(ERROR) << "load aclnnSwiGlu failed! Consider upgrade the CANN first!";
        return ret;
    }
    ATB_LOG(INFO) << "load aclnnSwiGlu two-staged method success!";
    return NO_ERROR;
}
