...
Status RopeAclnnRunner::LoadMethod()
{
    ...
    Status ret = dlManager.getSymbol("aclnnApplyRotaryPosEmbV2GetWorkspaceSize",
                                     (void *&)RopeAclnnRunner::aclnnGetWorkspaceSizeFunc_);
    if (ret != NO_ERROR) {
        ATB_LOG(ERROR) << "load aclnnApplyRotaryPosEmbV2GetWorkspaceSize failed! Consider upgrade the CANN first!";
        return ERROR_CANN_ERROR;
    }
    ret = dlManager.getSymbol("aclnnApplyRotaryPosEmbV2", (void *&)RopeAclnnRunner::aclnnExecuteFunc_);
    if (ret != NO_ERROR) {
        ATB_LOG(ERROR) << "load aclnnApplyRotaryPosEmbV2 failed! Consider upgrade the CANN first!";
        return ERROR_CANN_ERROR;
    }
    ATB_LOG(INFO) << "load RopeAclnnRunner two-staged method success!";
    return NO_ERROR;
}
...
Status RopeAclnnRunner::LaunchAclnnKernel()
{
    ...
    Status status = RopeAclnnRunner::LoadMethod();
    if (status != NO_ERROR) {
