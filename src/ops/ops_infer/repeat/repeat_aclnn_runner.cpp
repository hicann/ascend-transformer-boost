...
aclnnStatus (*RepeatAclnnRunner::aclnnGetWorkspaceSizeFunc_)(const aclTensor *, const aclIntArray *, const aclTensor *,
                                                             uint64_t *, aclOpExecutor **) = nullptr;
aclnnStatus (*RepeatAclnnRunner::aclnnExecuteFunc_)(void *, uint64_t, aclOpExecutor *, aclrtStream) = nullptr;
...
aclnnStatus RepeatAclnnRunner::SetAclNNWorkspaceExecutor()
{
    ...
    aclnnStatus ret = RepeatAclnnRunner::aclnnGetWorkspaceSizeFunc_(
        self, size_, out, &(this->atbVariantPack_.workspaceBufferSize), &raw_executor_ptr);
    if (ret != ACL_SUCCESS) {
        ATB_LOG(ERROR) << GetLogPrefix() << "aclnnGetWorkspaceSizeFunc_ failed with return value: " << ret;
        return ret;
    }
    ret = aclDestroyIntArray(size_);
    if (ret != ACL_SUCCESS) {
        ATB_LOG(ERROR) << GetLogPrefix() << "aclDestroyIntArray failed with return value: " << ret;
        return ret;
    }
