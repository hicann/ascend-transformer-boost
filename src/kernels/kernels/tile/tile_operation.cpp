...
namespace AsdOps {
using namespace Mki;
class TileOperation : public OperationBase {
public:
    explicit TileOperation(const std::string &opName) noexcept : OperationBase(opName) {}
    Kernel* GetBestKernel(const LaunchParam &launchParam) const override
    {
        auto dtype = launchParam.GetOutTensor(0).desc.dtype;
        if (dtype == TENSOR_DTYPE_FLOAT16) {
            return GetKernelByName("TileF16Kernel");
        }
        MKI_LOG(ERROR) << "No kernel for Tile dtype " << GetStrWithDType(dtype);
        return nullptr;
    }
protected:
    Status InferShapeImpl(const LaunchParam &launchParam, SVector<Tensor> &outTensors) const override
    {
        ...
        if (dtype0 != TENSOR_DTYPE_FLOAT16) {
        ...
        while (idx < largerSize) {
            ...
            if ((dim0 <= 0) || !((dim0 == dim1) || ((dim0 < dim1) && (dim0 == 1)))) {
                MKI_LOG(ERROR) << "cannot support this dim of Tile";
