...
#include "kernels/tile/tiling/tile_tiling.h"
namespace AsdOps {
using namespace Mki;
constexpr uint64_t TENSOR_TILE_SHAPE_IDX = 1;

class TileKernel : public KernelBase {
public:
    explicit TileKernel(const std::string &kernelName, const BinHandle *handle) noexcept
    ...
    {
        auto status = TileTiling(GetName(), launchParam, kernelInfo_, *GetBinHandle());
        MKI_CHECK_NO_LOG(status.Ok(), return status);
        kernelInfo_.SetConstTensorOffset(launchBufferSize_);
        auto param = AnyCast<OpParam::Expand>(launchParam.GetParam());
        kernelInfo_.AddConstTensorData<int64_t>(TENSOR_TILE_SHAPE_IDX, param.shape);
        return Status::OkStatus();
    }
};

// TileF16Kernel
class TileF16Kernel : public TileKernel {
public:
    explicit TileF16Kernel(const std::string &kernelName, const BinHandle *handle) noexcept
        : TileKernel(kernelName, handle)
    {
    }

    bool CanSupport(const LaunchParam &launchParam) const override
    {
        MKI_CHECK_NO_LOG(TileKernel::CanSupport(launchParam), return false);
