...
#include <algorithm>
#include "ring_attention_tiling.h"
...
namespace AtbOps {
using namespace Mki;
Status GetRingAttentionNoCacheMaskInfo(RingAttentionInfo &mmInfo, OpParam::RingAttention &param,
                                       uint32_t batch);

Status InitInfo(RingAttentionInfo &mmInfo, OpParam::RingAttention &param);

void InitMaxKVSeqlenBNSD(Mki::SVector<int64_t> &kcacheShape, AtbOps::RingAttentionInfo &mmInfo, size_t &batch);

void InitEmbed(int32_t &embed, const RingAttentionInfo &mmInfo,
               const AtbOps::OpParam::RingAttention &param);

Status InitMaxKVSeqlen(size_t &batch, Mki::SVector<int64_t> &kcacheShape,
                       RingAttentionInfo &mmInfo, AtbOps::OpParam::RingAttention &param);

inline void InitGetInputTensors(RingAttentionInfo &mmInfo, const LaunchParam &launchParam,
                                const AtbOps::OpParam::RingAttention &param)
{
    uint32_t index = 0;
    switch (param.type) {
...
void GetSwaTilingKey(RingAttentionInfo &mmInfo)
{
    if (mmInfo.windowSize > 0 &&
        mmInfo.type == OpParam::RingAttention::RING_ATTENTION) {
        mmInfo.tilingKey = static_cast<uint32_t>(TilingKeyType::TILING_SWA);
        if (mmInfo.maskType == OpParam::RingAttention::MASK_TYPE_SWA_COMPRESS) {
            mmInfo.tilingKey = static_cast<uint32_t>(TilingKeyType::TILING_SWA_CMP);
        }
    }
}

void GetKernelTilingId(const RingAttentionInfo &mmInfo, KernelInfo &kernelInfo, uint32_t *tilingParam)
{
    if (!mmInfo.isMLA &&
        mmInfo.type == OpParam::RingAttention::RING_ATTENTION) {
...
Status RingAttentionTiling(const LaunchParam &launchParam, KernelInfo &kernelInfo)
{
    RingAttentionInfo mmInfo;
    MKI_CHECK(launchParam.GetParam().Type() == typeid(OpParam::RingAttention), "OpParam is invalid",
              return Status::FailStatus(ERROR_INFERSHAPE_ERROR));
    OpParam::RingAttention param = AnyCast<OpParam::RingAttention>(launchParam.GetParam());
    ...
    ret = GetRingAttentionTilingParam(mmInfo, blockDim, tilingParam, tilingSize);
    OP_TILING_CHECK_STATUS_RETURN(ret);
...
Status GetAlibiMaskInfo(RingAttentionInfo &mmInfo, OpParam::RingAttention &param, const Tensor &tensorMask,
                        int32_t maxSeq)
{
...
Status GetSwaMaskInfo(RingAttentionInfo &mmInfo, OpParam::RingAttention &param, const Tensor &tensorMask)
{
    ...
    if (*maxQSeq != 1) {
        ...
        MKI_CHECK(maskShape.at(0) <= maskShape.at(1),
                    ...
                    return Status::FailStatus(ERROR_INVALID_VALUE));
        if (maskType == OpParam::RingAttention::MASK_TYPE_SWA_COMPRESS) {
                                       ...
                                       const Tensor &tensorMask)
{
    ...
    if (maskType == OpParam::RingAttention::MASK_TYPE_SWA_COMPRESS ||
        maskType == OpParam::RingAttention::MASK_TYPE_SWA_NORM) {
        auto ret = GetSwaMaskInfo(mmInfo, param, tensorMask);
        OP_TILING_CHECK_STATUS_RETURN(ret);
    } else if (maskType != OpParam::RingAttention::MASK_TYPE_NONE) {
        auto maskDim = maskShape.size();
        int32_t maxSeq = maskShape.at(maskDim - 1);
        mmInfo.maxSeqLen = maxSeq;
        ...
        if (maskType == OpParam::RingAttention::MASK_TYPE_ALIBI
            || maskType == OpParam::RingAttention::MASK_TYPE_ALIBI_COMPRESS
            || maskType == OpParam::RingAttention::MASK_TYPE_ALIBI_COMPRESS_SQRT
            || maskType == OpParam::RingAttention::MASK_TYPE_ALIBI_COMPRESS_LEFT_ALIGN
            || maskType == OpParam::RingAttention::MASK_TYPE_ALIBI_COMPRESS_128) {
...
inline void RingAttentionLog(RingAttentionInfo &mmInfo, OpParam::RingAttention &param)
{
...
inline Status FlashAttentionPreCheck(OpParam::RingAttention &param)
{
...
inline Status FlashAttentionPostCheck(RingAttentionInfo &mmInfo, OpParam::RingAttention &param)
{
    ...
    MKI_CHECK(mmInfo.isAlibiMaskSqrt == 0 || mmInfo.isAlibiMaskSqrt == 1, "param isAlibiMaskSqrt is invalid",
                 return Status::FailStatus(ERROR_INVALID_VALUE));
    MKI_CHECK(mmInfo.cacheType <= OpParam::RingAttention::CACHE_TYPE_CYCLE, "param cacheType is invalid",
                 return Status::FailStatus(ERROR_INVALID_VALUE));
    if (mmInfo.isMLA) {
    ...
    MKI_CHECK((mmInfo.maskType == OpParam::RingAttention::MASK_TYPE_SWA_NORM ||
            mmInfo.maskType == OpParam::RingAttention::MASK_TYPE_SWA_COMPRESS) == (mmInfo.windowSize > 0),
...
inline Status FlashAttentionSwaCheck(const RingAttentionInfo mmInfo)
{
    ...
    MKI_CHECK(mmInfo.isClamp == 0, "swa not support clamp",
                return Status::FailStatus(ERROR_INVALID_VALUE));
    MKI_CHECK(mmInfo.scaleType == OpParam::RingAttention::SCALE_TOR,
    ...
    MKI_CHECK(mmInfo.tensors.query.desc.dtype != TENSOR_DTYPE_INT8,
                ...
                return Status::FailStatus(ERROR_INVALID_VALUE));
    MKI_CHECK(mmInfo.type == OpParam::RingAttention::RING_ATTENTION,
                "swa only support encoder nd and deocder nd type",
                return Status::FailStatus(ERROR_INVALID_VALUE));
    return Status::OkStatus();
}

inline void GetMLATilingKey(RingAttentionInfo &mmInfo, OpParam::RingAttention &param)
{
...
void GetKvCacheTilingKey(RingAttentionInfo &mmInfo)
{
    uint32_t int8Key = (mmInfo.tensors.query.desc.dtype == TENSOR_DTYPE_INT8) ? 1 : 0;
    uint32_t typeKey = (mmInfo.tensors.query.desc.dtype == TENSOR_DTYPE_BF16) ? 1 : 0;
...
void GetTilingKey(RingAttentionInfo &mmInfo, OpParam::RingAttention &param)
{
    if (param.type == OpParam::RingAttention::RING_ATTENTION) {
...
void GetSpecTilingKey(RingAttentionInfo &mmInfo, const OpParam::RingAttention &param)
{
    if ((param.type == OpParam::RingAttention::RING_ATTENTION) &&
...
void FlashAttentionEncoderBNSD(RingAttentionInfo &mmInfo, OpParam::RingAttention &param, int32_t embed)
{
    bool optEnable = mmInfo.maskType == 0 && embed == 128 &&
                     mmInfo.scaleType == OpParam::RingAttention::SCALE_TOR && param.dataShapeType == 1;
    if (param.type == OpParam::RingAttention::RING_ATTENTION && optEnable) {
        mmInfo.splitm = true;
...
void FlashAttentionFillRazorInfo(RingAttentionInfo &mmInfo, OpParam::RingAttention &param)
{
...
void FlashAttentionFillInfo(RingAttentionInfo &mmInfo, OpParam::RingAttention &param,
                            size_t batch, int32_t embed)
{
    ...
    if (mmInfo.type == OpParam::RingAttention::RING_ATTENTION) {
        mmInfo.isRing =  param.isRing;
        }
    MKI_LOG(INFO) << "window size is " << mmInfo.windowSize << " cache type is " << mmInfo.cacheType;
}

Status FlashAttentionVCacheCheck(const RingAttentionInfo &mmInfo, const OpParam::RingAttention &param)
{
...
Status InitInfo(RingAttentionInfo &mmInfo, OpParam::RingAttention &param)
{
    ...
    bool optEnable = mmInfo.maskType == 0 && mmInfo.embeddingSize <= 128 &&
                     mmInfo.scaleType == OpParam::RingAttention::SCALE_TOR;
    if (optEnable) {
               ...
               const AtbOps::OpParam::RingAttention &param)
{
    ...
    if (param.dataShapeType == 1 && (param.type == OpParam::RingAttention::RING_ATTENTION)) {
...
void InitMaxKVSeqlenMLACombineCache(Mki::SVector<int64_t> &kcacheShape, RingAttentionInfo &mmInfo)
{
...
Status InitMaxKVSeqlen(size_t &batch, Mki::SVector<int64_t> &kcacheShape, RingAttentionInfo &mmInfo,
    AtbOps::OpParam::RingAttention &param)
{
    if (param.type == OpParam::RingAttention::RING_ATTENTION) {
        OP_TILING_CHECK_STATUS_RETURN(GetRingAttentionNoCacheMaskInfo(mmInfo, param, mmInfo.tensors.mask));
