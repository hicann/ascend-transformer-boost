...
*/
#include "ring_attention_tiling_dependency.h"
...
const int32_t INDEX37 = 37;
const int32_t INDEX38 = 38;
...
Status CheckSeqlen(const RingAttentionInfo &mmInfo, const AddrOffsets &addrOffsets, int32_t qSeqlen,
...
uint32_t SetTilingKey(const RingAttentionInfo &mmInfo)
{
...
void FillTilingHead(const RingAttentionInfo &mmInfo, const uint32_t &torUptr, AddrOffsets &addrOffsets,
                    uint32_t *tilingParam, int32_t kvRealHeads, int32_t ntokens)
{
    ...
    tilingParam[INDEX37] = static_cast<uint32_t>(mmInfo.isRing);
    tilingParam[INDEX38] = static_cast<uint32_t>(ntokens);
}

void RingFillTilingOffsetParam(int32_t seqIdx, AddrOffsets &addrOffsets, uint32_t *tilingParam)
{
...
void InitTilingKWithN(const RingAttentionInfo &mmInfo, int32_t &embeddingSizeAligned, int32_t &nIbd,
    int32_t seqIdx, uint32_t *tilingParam)
{
    ...
    if (mmInfo.quantType == static_cast<uint32_t>(OpParam::RingAttention::QuantType::TYPE_QUANT_QKV_OFFLINE)
        || mmInfo.quantType == static_cast<uint32_t>(OpParam::RingAttention::QuantType::TYPE_QUANT_QKV_ONLINE)) {
...
Status EncoderFillTilingParam(const RingAttentionInfo &mmInfo, const uint32_t &torUptr, AddrOffsets &addrOffsets,
                              int32_t kvRealHeads, uint32_t *tilingParam)
{
    ...
    for (int32_t seqIdx = 0; seqIdx < mmInfo.batchSize; seqIdx++) {
        ...
        int32_t mIbd = ConvertValueToIndexMM(mUbd, PP_MM_NUM - 1); // PP_MM_NUM=8
        ...
        RingFillTilingOffsetParam(seqIdx, addrOffsets, tilingParam);
    ...
    FillTilingHead(mmInfo, torUptr, addrOffsets, tilingParam, kvRealHeads, ntokens);
    addrOffsets.block = static_cast<int64_t>(mmInfo.innerBatchSize) * addrOffsets.totalQBlkNum;
    return Status::OkStatus();
}

void FillAddrOffsets(const AtbOps::RingAttentionInfo &mmInfo, AtbOps::AddrOffsets &addrOffsets,
...
void FillSplitBatchPtr(const RingAttentionInfo &mmInfo, uint32_t *tilingParam, int32_t seqIdx)
{
...
void FillSplitBatchPtr(const RingAttentionInfo &mmInfo, uint32_t *tilingParam, int32_t nowBatchTiling,
...
void SplitTaskRelay(const RingAttentionInfo &mmInfo, uint32_t *tilingParam, int32_t groupNum,
...
void SplitCoreRelay(const RingAttentionInfo &mmInfo, uint32_t *tilingParam, uint32_t shareBlockTiling)
{
...
void DecoderSplitHeadNum(const RingAttentionInfo &mmInfo, int32_t kvRealHeads, uint32_t *tilingParam)
{
...
void DecoderSplitCore(const RingAttentionInfo &mmInfo, AddrOffsets &addrOffsets, int32_t kvRealHeads,
...
void FillTilingHeadDecoder(const RingAttentionInfo &mmInfo, const uint32_t &torUptr, AddrOffsets &addrOffsets,
                           int32_t kvRealHeads, uint32_t *tilingParam)
{
    tilingParam[0] = static_cast<uint32_t>(mmInfo.batchSize);
    tilingParam[1] = static_cast<uint32_t>(mmInfo.maxSeqLen);
...
Status DecoderFillTilingParamRelay(const RingAttentionInfo &mmInfo, const uint32_t &torUptr,
...
Status DecoderFillTilingParam(const RingAttentionInfo &mmInfo, const uint32_t &torUptr, AddrOffsets &addrOffsets,
                              int32_t kvRealHeads, uint32_t *tilingParam)
{
    FillTilingHeadDecoder(mmInfo, torUptr, addrOffsets, kvRealHeads, tilingParam);
    for (int32_t seqIdx = 0; seqIdx < mmInfo.batchSize; seqIdx++) {
...
void FillAddr(AtbOps::AddrOffsets &addrOffsets, int32_t qSeqlen, const AtbOps::RingAttentionInfo &mmInfo,
              int32_t kvRealHeads)
{
    addrOffsets.addrQSeqOffset += static_cast<uint64_t>(qSeqlen * mmInfo.innerBatchSize * mmInfo.embeddingSize);
    addrOffsets.addrKSeqOffset += static_cast<uint64_t>(mmInfo.maxKvSeqLen * kvRealHeads * mmInfo.embeddingSize);
...
Status GetFillTilingParam(const RingAttentionInfo &mmInfo,
...
Status GetRingAttentionTilingParam(const RingAttentionInfo &mmInfo, uint32_t &blockDim,
                                   uint32_t *tilingParam, uint32_t tilingParamSize)
{
    ...
    uint32_t tilingHeadSize = mmInfo.type == OpParam::RingAttention::RING_ATTENTION ?
                              TILING_HEAD_SIZE : TILING_RELAY_HEAD_SIZE;
    uint64_t initSize =
