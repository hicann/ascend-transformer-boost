...
void OperationWrapper::Setup(std::vector<torch::Tensor> &inTensors)
{
    if (!operation_) {
    ...
    outTensors_.clear();
    outTensors_.resize(outTensorDescs.size());
    for (size_t i = 0; i < outTensorDescs.size(); ++i) {
        outTensors_.at(i) = Utils::CreateTorchTensorFromTensorDesc(outTensorDescs.at(i));
    }
    BuildOutTensorVariantPack();
...
void OperationWrapper::BuildOutTensorVariantPack()
{
    variantPack_.outTensors.resize(outTensors_.size());
    for (size_t i = 0; i < outTensors_.size(); ++i) {
        variantPack_.outTensors.at(i) = Utils::ConvertToAtbTensor(outTensors_.at(i));
    }
}
} // namespace TorchAtb
