...
{
    workspaceBufferSizes_.reserve(runnerGraph_.nodes.size());
...
Status GraphRunner::FillHostTilingBufferImpl(uint8_t *hostTilingBuffer, uint64_t tilingBufferSize)
{
    ...
    for (size_t nodeId = 0; nodeId < runnerGraph_.nodes.size(); ++nodeId) {
        ...
        ATB_LOG(INFO) << GetLogPrefix() << "node[" << nodeId << "] workspace buffer size:" << runnerWorkspaceBufferSize
                      << ", max:" << maxWorkspaceBufferSize_;
    }
    ATB_LOG(INFO) << GetLogPrefix() << " max workspace buffer size:" << maxWorkspaceBufferSize_;
    return NO_ERROR;
}

uint64_t GraphRunner::GetWorkspaceBufferSizeImpl()
{
    return maxWorkspaceBufferSize_;
}
...
Status GraphRunner::ExecuteImpl(RunnerVariantPack &runnerVariantPack)
{
    ...
    if (runnerVariantPack.context == nullptr || runnerVariantPack.context->GetExecuteStream() == nullptr) {
        ATB_LOG(ERROR) << GetLogPrefix() << " Execute fail, stream is null";
        return ERROR_INVALID_PARAM;
    }
    if (runnerGraph_.inTensors.size() != runnerVariantPack.inTensors.size() ||
        runnerGraph_.outTensors.size() != runnerVariantPack.outTensors.size()) {
