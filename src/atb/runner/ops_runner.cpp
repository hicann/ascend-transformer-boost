...
Status OpsRunner::SetupImpl(RunnerVariantPack &runnerVariantPack)
{
    ...
    if (!PlanKernelGraph(runnerVariantPack.hostTilingBuffer, runnerVariantPack.tilingBufferSize)) {
...
Status OpsRunner::ExecuteImpl(RunnerVariantPack &runnerVariantPack)
{
    ...
    st = RunAllKernel(*runnerVariantPack.context);
    if (st != NO_ERROR) {
    ...
    if (Probe::IsOverflowCheck()) {
        ...
        if (isOverflow) {
            ...
            if (Probe::IsOverflowStop()) {
                ...
                return ERROR_INTERNAL_ERROR; // stop process
            }
        }
    }
...
bool OpsRunner::PlanKernelGraph(uint8_t *kernelHostTilingBuffer, uint64_t maxTilingSize)
{
    ...
    for (size_t nodeId = 0; nodeId < kernelGraph_.nodes.size(); ++nodeId) {
        ...
        if (!PlanKernel(nodeId, kernelHostTilingBuffer, restTilingBufferSize)) {
...
bool OpsRunner::PlanKernel(size_t nodeId, uint8_t *kernelHostTilingBuffer, uint64_t maxTilingSize)
{
    KernelGraphNode &node = kernelGraph_.nodes.at(nodeId);
