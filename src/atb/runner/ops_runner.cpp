...
{
    if (nodesSaveTensorFlag_.at(nodeId) && Probe::IsSaveTensorInSpecificDir(GetSaveTensorDir() + "/" + std::to_string(nodeId) + "_" + node.GetName()) && Probe::IsSaveTensorBefore()) {
...
void OpsRunner::RunKernelPostProcess(KernelGraphNode &node, size_t nodeId, aclrtStream stream)
{
    ...
    if (nodesSaveTensorFlag_.at(nodeId) && Probe::IsSaveTensorInSpecificDir(GetSaveTensorDir() + "/" + std::to_string(nodeId) + "_" + node.GetName()) && Probe::IsSaveTensorAfter()) {
