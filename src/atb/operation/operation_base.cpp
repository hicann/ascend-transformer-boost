...
Status OperationBase::SetupThrow(const VariantPack &variantPack, uint64_t &workspaceSize)
{
    ...
    runnerVariantPack_.workspaceBufferSize =
        static_cast<uint64_t>(TensorUtil::AlignInt(runner_->GetWorkspaceBufferSize(), WORKSPACE_ALIGN));
...
Status OperationBase::SetupPrepare()
{
    ...
    if (GetSingleton<Mki::ProfilingFuncs>().GetProfilingLevel0Status() && !isProfArrayInited_) {
        ...
        uint64_t setupHashId =
            GetSingleton<Mki::ProfilingFuncs>().ProfGetHashId(setupName.c_str(), setupName.size());
        uint64_t executeHashId =
            GetSingleton<Mki::ProfilingFuncs>().ProfGetHashId(executeName.c_str(), executeName.size());
        hashIdArray_.at(OPERATION_SETUP) = setupHashId;
        hashIdArray_.at(OPERATION_EXECUTE) = executeHashId;
        typeIdArray_.at(OPERATION_SETUP) = GetTypeIdFromHash(setupHashId);
        typeIdArray_.at(OPERATION_EXECUTE) = GetTypeIdFromHash(executeHashId);
        GetSingleton<Mki::ProfilingFuncs>().ProfReportTypeInfo(MSPROF_REPORT_ACL_LEVEL,
                                                               typeIdArray_.at(OPERATION_SETUP), setupName);
        GetSingleton<Mki::ProfilingFuncs>().ProfReportTypeInfo(MSPROF_REPORT_ACL_LEVEL,
                                                               typeIdArray_.at(OPERATION_EXECUTE), executeName);
    }
...
Status OperationBase::CopyTilingToDevice()
{
    ...
    aclrtStream executeStream = contextBase->GetExecuteStream();
...
Status OperationBase::ExecuteVariantPackCheck(const VariantPack &variantPack)
{
    ...
    for (size_t i = 0; i < variantPack.inTensors.size(); i++) {
        ...
        if (variantPackInTensor.dataSize != Utils::GetTensorSize(runnerVariantPack_.inTensors.at(i).desc)) {
...
{
    if (context == nullptr || context->GetExecuteStream() == nullptr) {
...
Status OperationBase::ExecuteThrow(const VariantPack &variantPack, uint8_t *workspace, uint64_t workspaceSize)
{
    const uint64_t beginTime = GetSingleton<Mki::ProfilingFuncs>().GetProfilingLevel0Status() ?
                                   GetSingleton<Mki::ProfilingFuncs>().ProfSysCycleTime() :
                                   0;

    void *executeStream = runnerVariantPack_.context->GetExecuteStream();
    ATB_LOG(INFO) << GetLogPrefix() << "execute start, workspaceSize:" << workspaceSize
                  << ", variantPack:" << VariantPackToString(variantPack);
#ifdef _DEBUG
    ATB_LOG(INFO) << "workspace:" << static_cast<void *>(workspace) << ", stream" << executeStream;
#endif
...
#ifdef _DEBUG
    ATB_LOG(DEBUG) << GetLogPrefix() << "execute " << runner_->GetName() << "_" << runner_.get() << " success";
#else
    ATB_LOG(DEBUG) << GetLogPrefix() << "execute " << runner_->GetName() << " success";
#endif
    ...
    if (GetSingleton<Mki::ProfilingFuncs>().GetProfilingLevel0Status()) {
        ReportApiInfo(beginTime, OPERATION_EXECUTE);
    }
    return st;
}
                              ...
                              Context *context)
{
    ...
    try {
        ...
        st = ExecuteThrow(variantPack, workspace, workspaceSize);
        if (st != NO_ERROR) {
    ...
    GetOpExecuteStatistic().totalTime += totalTime.ElapsedMicroSecond();
    ATB_LOG(INFO) << GetLogPrefix() << "execute statistic:" << GetOpExecuteStatistic().ToString();
    if (Probe::ReportOperationStatisticEnable()) {
        Probe::ReportOperationExecuteStatistic(executeCount_, GenerateOperationName(name_, operationBaseIds_),
                                               GetOpExecuteStatistic().ToString());
    }
