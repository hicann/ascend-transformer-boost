...
ContextBase::~ContextBase() noexcept
{
    try {
        ...
        ATB_LOG(ERROR) << "An exception occurred when running DestoryCopyStreamAndEvents()." << "Traceback: \n"
                       << e.what();
    }
}
...
{
    executeStream_ = stream;
    if (Probe::IsOverflowCheck()) {
        Status st = aclrtSetStreamOverflowSwitch(executeStream_, 1);
        if (st != NO_ERROR) {
