...
AutoFusion::AutoFusion(atb::GraphParam &graph) : graph_(graph)
{
    homePath_ = std::string(std::getenv("HOME"));
    homePath_ += AUTOFUSIONSTRPATH("/.atb_auto_fusion/bishengir_bin/");
}

void AutoFusion::SetFusionClass(const std::set<std::string> &fusionClassArray)
{
    fusionClassMap_ = fusionClassArray;
    return;
}

void AutoFusion::DoAutoFusion(const std::set<std::string> &fusionClassArray)
{
...
std::vector<uint32_t> AutoFusion::findNodesWhichInputsIs(const uint32_t id)
{
...
std::vector<std::pair<std::string, std::vector<uint32_t>>> AutoFusion::parseCollectNodes()
{
    ...
    for (auto i  : linearNodes_) {
        ...
        std::vector<uint32_t> re = findNodesWhichInputsIs(outTensorIds[0]);
        if (re.size() != 1) {
    ...
    return fusionclassAndIndex;
}

void AutoFusion::CollectNodes()
{
...
std::vector<std::pair<std::string, std::vector<uint32_t>>> AutoFusion::ParseFusion()
{
    CollectNodes();
    return parseCollectNodes();
}

void AutoFusion::GetFusionBinAndUpdateFusedGraph()
{
    std::vector<std::pair<std::string, std::vector<uint32_t>>> fusionclassAndIndex = ParseFusion();
    std::vector<std::pair<std::string, std::string>> fusionclassAndBin;
    // 保留
    for (auto i : fusionclassAndIndex) {
        auto path = callBiShengIR(i);
...
std::string AutoFusion::callBiShengIR(const std::pair<std::string, std::vector<uint32_t>> &subFusion)
{
    // 以mlir为入手点
    ...
    std::string jsonPath = preFill + ".cpp";
...
void AutoFusion::UpdateFusedGraph(std::vector<std::pair<std::string, std::vector<uint32_t>>> &fusionclassAndIndex,
    const std::vector<std::pair<std::string, std::string>> &fusionclassAndBin)
{
    for (size_t i = 0; i < fusionclassAndBin.size(); i++) {
...
void AutoFusion::SortNodeByTensorIdsAndChangeTopology(std::vector<uint32_t> &fusedNodes, const std::string &fusionClass)
{
    std::sort(fusedNodes.begin(), fusedNodes.end(), [this](int index1, int index2) {
        bool flag = false;
        auto &internalTensorsEraseByFusion = graph_.internalTensorsEraseByFusion;
        for (auto &subIndex : graph_.nodes[index2].inTensorIds) {
            for (auto &subIndex1 : graph_.nodes[index1].outTensorIds) {
                if (subIndex1 == subIndex && uint32_t(-1) != subIndex1) {
                    internalTensorsEraseByFusion.insert(std::make_pair(subIndex, 1));
                    subIndex1 = -1;
                    subIndex = -1;
                    flag = true;
                }
...
void AutoFusion::SetFusionParam(atb::infer::FusionParam &param, const std::string &fusionClass)
{
...
void AutoFusion::UpdateGraphStruct(const std::vector<uint32_t> &fusedNodes, const std::string &fusionClass)
{
    graph_.nodes[fusedNodes[0]].theMain = true;
    graph_.nodes[fusedNodes[0]].fusedFlag = true;
    ...
    for (size_t i = 1; i < fusedNodes.size(); i++) {
        graph_.nodes[fusedNodes[i]].fusedFlag = true;
    ...
    for (size_t i = 1; i < fusedNodes.size(); i++) {
        ...
        graph_.nodes[fusedNodes[i]].theMain = false;
        graph_.nodes[fusedNodes[i]].fusedFlag = true;
    }
    ATB_LOG(INFO) << " UpdateGraphStruct Done!";
}

void AutoFusion::UpdateReshapeFunc(const std::vector<uint32_t> &fusedNodes)
{
    auto &intensorReshapeFuncs = graph_.nodes[fusedNodes[0]].intensorReshapeFuncs;
    for (size_t i = 1; i < fusedNodes.size(); i++) {
        for (size_t j = 1; j < graph_.nodes[fusedNodes[i]].intensorReshapeFuncs.size(); j++) {
...
std::string AutoFusion::getTilingKey(const std::string &fusionClass)
{
...
bool AutoFusion::genFusionKernelDef(const std::string &fusionClass)
{
...
bool AutoFusion::genMlirAndBin(const std::string &fusionClass)
{
    int res = -1;
    if ("matmul_add" == fusionClass) {
