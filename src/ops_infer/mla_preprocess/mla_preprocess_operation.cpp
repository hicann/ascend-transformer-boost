...
namespace {
static const uint32_t IN_TENSOR_NUM = 21;
static const uint32_t OUT_TENSOR_NUM = 2;
static const uint32_t HEAD_DIM_576 = 576;
static const uint32_t Q_HEAD_DIM_512 = 512;
static const uint32_t BIAS0_INDEX = 7;
static const uint32_t BIAS1_INDEX = 14;
static const uint32_t WDQKV_INDEX = 5;
static const uint32_t WUQ_INDEX = 12;
                                              ...
                                              SVector<TensorDesc> &outTensorDescs) const
{
    ...
    outTensorDescs.at(0).shape.dims[2] = HEAD_DIM_576; // 2 : dim 2
...
Status MlaPreprocessOperation::SetupCheckImpl(const SVector<Tensor> &inTensors, const SVector<Tensor> &outTensors) const
{
    ...
    if (outTensors.at(0).desc.shape.dims[0] != tokenNum ||     // 0 : dim 0
        ...
        outTensors.at(0).desc.shape.dims[2] != HEAD_DIM_576) { // 2 : dim 2
...
std::shared_ptr<Runner> MlaPreprocessOperation::CreateRunner(Context &context) const
{
    ...
    RunnerPool &pool = contextBase->GetRunnerPool(RUNNER_TYPE_MLAPREPROCESS);
    Runner *runner = pool.MallocRunner<MlaPreprocessOpsRunner, infer::MlaPreprocessParam>(param_);
    if (!runner) {
