...
bool RmsNormTypeCheck(const infer::RmsNormParam &opParam)
{
    ...
    if (GetSingleton<Config>().Is910A()) {
        if (opParam.layerType == infer::RmsNormParam::RMS_NORM_NORM &&
            opParam.normParam.quantType == infer::QUANT_UNDEFINED) {
            return true;
        } else {
            error.errorDesc = "Atlas 800 product only supports RMS_NORM_NORM layerType in QUANT_UNDEFINED quantType";
    ...
    error.errorData = OperationUtil::ConcatInfo(error.errorData, ", layerType = ", opParam.layerType);
    if (opParam.layerType == infer::RmsNormParam::RMS_NORM_NORM) {
        if (opParam.normParam.quantType == infer::QUANT_UNDEFINED || opParam.normParam.quantType == infer::QUANT_INT8) {
            return true;
        }
        ...
        if (opParam.preNormParam.quantType == infer::QUANT_UNDEFINED ||
        ...
        if (opParam.postNormParam.quantType == infer::QUANT_UNDEFINED) {
            return true;
        }
...
RmsNormOperation::RmsNormOperation(const infer::RmsNormParam &param) : OperationBase("RmsNormOperation"), param_(param)
{
    ...
    if (param_.layerType == infer::RmsNormParam::RMS_NORM_NORM) {
        if (param_.normParam.quantType == infer::QUANT_UNDEFINED) {
            operationIr_ =
        ...
        } else if (param_.preNormParam.quantType == infer::QUANT_UNDEFINED) {
    ...
    if (param_.layerType == infer::RmsNormParam::RMS_NORM_NORM) {
        ...
        return IN_TENSOR_COUNT_SIX;
               ...
               (param_.layerType == infer::RmsNormParam::RMS_NORM_PRENORM &&
                param_.preNormParam.quantType == infer::QUANT_UNDEFINED)) {
...
{
    if (param_.layerType == infer::RmsNormParam::RMS_NORM_NORM &&
        param_.normParam.quantType == infer::QUANT_UNDEFINED) {
        ...
        return OUT_TENSOR_COUNT_TWO;
               ...
               param_.postNormParam.quantType == infer::QUANT_UNDEFINED) {
        return 1;
    }
    ATB_LOG(ERROR) << GetLogPrefix() << "RmsNormType Undefined";
    return 0;
...
Status RmsNormOperation::InferShapeCheckImpl(const SVector<TensorDesc> &inTensorDescs) const
{
    ...
    if (param_.layerType == infer::RmsNormParam::RMS_NORM_NORM) {
        ...
        if (!PrenormInferShapeCheckImpl(inTensorDescs)) {
            return ERROR_INVALID_TENSOR_DIM;
        }
    }
...
bool RmsNormOperation::CheckHasBias() const
{
    if (param_.preNormParam.hasBias) {
        if (param_.preNormParam.quantType != infer::QUANT_UNDEFINED) {
            ATB_LOG(ERROR) << GetLogPrefix() << "hasBias option is not supported in quant";
            return false;
        }
    }
    if (param_.postNormParam.hasBias) {
        if (param_.postNormParam.quantType != infer::QUANT_UNDEFINED) {
            ATB_LOG(ERROR) << GetLogPrefix() << "hasBias option is not supported in quant";
            return false;
        }
    }
