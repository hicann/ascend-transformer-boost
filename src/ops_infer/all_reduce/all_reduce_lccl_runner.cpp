...
Status AllReduceLcclRunner::ExecuteImpl(RunnerVariantPack &runnerVariantPack)
{
    ...
    if (runnerVariantPack.inTensors.size() > 1) {
        ret = lccl_->AllReduce(
            ...
            runnerVariantPack.context->GetExecuteStream(), GetHcclDtype(runnerVariantPack.outTensors.at(0).desc.dtype),
        ...
        ret = lccl_->AllReduce(runnerVariantPack.inTensors.at(0).deviceData,
                               ...
                               GetAllReduceType(param_.allReduceType), runnerVariantPack.context->GetExecuteStream());
    }
