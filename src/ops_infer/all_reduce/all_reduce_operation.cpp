...
{
    if (opParam.backend == "lccl" && opParam.quantType != atb::infer::AllReduceParam::QUANT_TYPE_UNDEFINED &&
    ...
    if (opParam.backend == "hccl" && opParam.quantType != atb::infer::AllReduceParam::QUANT_TYPE_UNDEFINED) {
        ATB_LOG(ERROR) << "quantType: " << opParam.quantType << "  Hccl is not support quant";
        return ERROR_INVALID_PARAM;
    }
    if (opParam.outDataType != ACL_FLOAT16 && opParam.quantType != atb::infer::AllReduceParam::QUANT_TYPE_UNDEFINED) {
...
Status CheckAllReduceParamValidity(const infer::AllReduceParam &opParam)
{
    ...
    if (opParam.backend == "lccl" && opParam.commDomain != "" && opParam.commMode != infer::COMM_MULTI_THREAD) {
        ATB_LOG(ERROR) << "allReduce commMode is" << opParam.commMode
                       << "lccl multiComm just support commMode is COMM_MULTI_THREAD";
        return ERROR_INVALID_PARAM;
    }
    return NO_ERROR;
}
...
Status AllReduceOperation::DtypeCheck(const TensorDesc &inTensorDesc) const
{
    ...
    if (param_.backend == "hccl" && param_.allReduceType == "prod") {
        if (inTensorDesc.dtype == ACL_INT16 || inTensorDesc.dtype == ACL_BF16) {
            ATB_LOG(ERROR) << "hccl allreduce prod is not support int16 , bf16";
            return ERROR_INVALID_TENSOR_DTYPE;
        }
    }
    if (param_.backend == "hccl" && GetSingleton<Config>().Is310P()) {
        if (inTensorDesc.dtype == ACL_INT64) {
            ATB_LOG(ERROR) << "Atlas inference products hccl allreduce is not support int16";
            return ERROR_INVALID_TENSOR_DTYPE;
        }
        if (param_.allReduceType == "max" || param_.allReduceType == "min") {
            if (inTensorDesc.dtype == ACL_INT16) {
                ATB_LOG(ERROR) << "Atlas inference products hccl allreducetype max,min is not support int16";
                return ERROR_INVALID_TENSOR_DTYPE;
            }
        }
    }
