...
static const uint32_t DIM0 = 0;
static const uint32_t DIM1 = 1;
static const uint32_t DIM2 = 2;
static const uint32_t DIM3 = 3;
...
{
    int64_t numBlocks = inTensorDescs.at(IN_TENSOR_0_KEYCACHE).shape.dims[DIM0];    // 0: keyCache
    int64_t lenContext = inTensorDescs.at(IN_TENSOR_2_BLOCKTABLE).shape.dims[DIM0]; // 2: blocktable
    int64_t sumcontext = inTensorDescs.at(IN_TENSOR_4_KEY).shape.dims[DIM0];        // 4: key
    if (numBlocks != inTensorDescs.at(IN_TENSOR_1_VALUECACHE).shape.dims[DIM0]) {
        ATB_LOG(ERROR) << GetLogPrefix() << "numBlocks should be same";
        return ERROR_INVALID_TENSOR_DIM;
    }
    if (sumcontext != inTensorDescs.at(IN_TENSOR_5_VALUE).shape.dims[DIM0]) {
    ...
    if (param_.kvCacheCfg == infer::PagedCacheLoadParam::KvCacheCfg::K_CACHE_V_CACHE_ND) {
        ...
        if (numHeads != inTensorDescs.at(IN_TENSOR_1_VALUECACHE).shape.dims[BLOCKSIZEINDEX]) {
        ...
        if (param_.isSeqLensCumsumMode) {
            if (lenContext != inTensorDescs.at(IN_TENSOR_3_CONTEXTLENS).shape.dims[DIM0] - ONE) { // 1:-1
                ATB_LOG(ERROR) << GetLogPrefix()
                               << "the lenContext of blocktable should match the lenContext of SeqLens when "
                                  "isSeqLensCumsumMode is true.";
                return ERROR_INVALID_TENSOR_DIM;
            }
        } else if (lenContext != inTensorDescs.at(IN_TENSOR_3_CONTEXTLENS).shape.dims[DIM0]) {
            ATB_LOG(ERROR) << GetLogPrefix()
                           << "the lenContext of blocktable should match the lenContext of SeqLens when "
                              "isSeqLensCumsumMode is true.";
            return ERROR_INVALID_TENSOR_DIM;
        ...
        if (blockSize % ALIGN_FP16_BF16 != ZERO) {
            ATB_LOG(ERROR) << GetLogPrefix() << "blockSize must be aligned to 16";
            return ERROR_INVALID_TENSOR_DIM;
        }
        if (lenContext != inTensorDescs.at(IN_TENSOR_3_CONTEXTLENS).shape.dims[DIM0]) {
    ...
    return (param_.kvCacheCfg == infer::PagedCacheLoadParam::KvCacheCfg::K_CACHE_V_CACHE_NZ) ?
               KVCacheDimCheck910BNZ(inTensorDescs) :
               KVCacheDimCheck910BND(inTensorDescs);
}
...
Status PagedCacheLoadOperation::KVCacheDimCheck910BNZ(const SVector<TensorDesc> &inTensorDescs) const
{
    ...
    if (inTensorDescs.at(IN_TENSOR_0_KEYCACHE).dtype == ACL_INT8) {
        ...
        if (inTensorDescs.at(IN_TENSOR_0_KEYCACHE).shape.dims[DIM1] * THIRTYTWO > MAX_K ||
            inTensorDescs.at(IN_TENSOR_1_VALUECACHE).shape.dims[DIM1] * THIRTYTWO > MAX_V) {
        ...
        return ERROR_INVALID_TENSOR_DIM;
    } else if (inTensorDescs.at(IN_TENSOR_0_KEYCACHE).shape.dims[DIM1] * SIXTEEN > MAX_K ||
               inTensorDescs.at(IN_TENSOR_1_VALUECACHE).shape.dims[DIM1] * SIXTEEN > MAX_V) {
        ATB_LOG(ERROR) << GetLogPrefix() << "The scend dimension of blocktables must be less than 147456";
        return ERROR_INVALID_TENSOR_DIM;
    }
    return NO_ERROR;
...
Status PagedCacheLoadOperation::KVCacheDimCheck910BND(const SVector<TensorDesc> &inTensorDescs) const
{
    ...
    } else if (numHeads * headSizeK % ALIGN_FP16_BF16 != 0) { // keyCache: fp16/bf16
        ATB_LOG(ERROR) << GetLogPrefix() << "fp16/bf16 ND format numHeads*headSizeK should be aligned to 16!";
        return ERROR_INVALID_TENSOR_DIM;
    }
    if (inTensorDescs.at(IN_TENSOR_1_VALUECACHE).dtype == ACL_INT8) { // valueCache: int8
        if (numHeads * headSizeV % ALIGN_INT8 != 0) {
            ATB_LOG(ERROR) << GetLogPrefix() << "int8 ND format numHeads*headSizeV should be aligned to 32!";
            return ERROR_INVALID_TENSOR_DIM;
        }
    } else if (numHeads * headSizeV % ALIGN_FP16_BF16 != 0) { // valueCache: fp16/bf16
        ATB_LOG(ERROR) << GetLogPrefix() << "fp16/bf16 ND format numHeads*headSizeV should be aligned to 16!";
        return ERROR_INVALID_TENSOR_DIM;
    }
    int64_t lenContext =
