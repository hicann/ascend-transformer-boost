...
{
    if (opParam.type != infer::LinearParallelParam::ParallelType::ALL_GATHER_LINEAR &&
        opParam.type != infer::LinearParallelParam::ParallelType::LINEAR_REDUCE_SCATTER &&
        opParam.type != infer::LinearParallelParam::ParallelType::ALLTOALLVC_ALL_GATHER_GMM &&
        opParam.type != infer::LinearParallelParam::ParallelType::GMM_REDUCE_SCATTER_ALLTOALLVC) {
        ATB_LOG(ERROR) << "LinearParallel type:" << opParam.type << " is invalid ParallelType";
        isOK = ERROR_INVALID_PARAM;
        return true;
    }
    if (opParam.type != atb::infer::LinearParallelParam::ParallelType::ALL_GATHER_LINEAR && opParam.keepIntermediate) {
        ATB_LOG(ERROR) << "LinearParallel backend mc2 only ALL_GATHER_LINEAR support keepIntermediate is true";
        isOK = ERROR_INVALID_PARAM;
        return true;
    }
    if (opParam.type == atb::infer::LinearParallelParam::ParallelType::ALL_GATHER_LINEAR && !opParam.keepIntermediate) {
        ATB_LOG(ERROR) << "LinearParallel backend mc2 ALL_GATHER_LINEAR require keepIntermediate is true";
        isOK = ERROR_INVALID_PARAM;
        return true;
    }
    if (opParam.quantType == atb::infer::LinearParallelParam::QuantType::QUANT_TYPE_PER_TOKEN &&
        opParam.type != atb::infer::LinearParallelParam::ParallelType::ALLTOALLVC_ALL_GATHER_GMM &&
        opParam.type != atb::infer::LinearParallelParam::ParallelType::GMM_REDUCE_SCATTER_ALLTOALLVC) {
        ATB_LOG(ERROR) << "When LinearParallel backend is mc2, "
                       << "only ALLTOALLVC_ALL_GATHER_GMM, GMM_REDUCE_SCATTER_ALLTOALLVC"
                       << " support quantType[QUANT_TYPE_PER_TOKEN]";
        isOK = ERROR_INVALID_PARAM;
        return true;
    }
