...
static bool QKVQuantParamCheck(const infer::PagedAttentionParam &opParam);
...
bool DeviceParamCheck(const infer::PagedAttentionParam &opParam)
{
    if (!GetSingleton<Config>().Is910B()) {
        ...
        if (opParam.quantType != atb::infer::PagedAttentionParam::QuantType::TYPE_QUANT_UNDEFINED) {
...
bool QKVQuantParamCheck(const infer::PagedAttentionParam &opParam)
{
    if (opParam.calcType == infer::PagedAttentionParam::CalcType::CALC_TYPE_SPEC &&
        (opParam.compressType != infer::PagedAttentionParam::CompressType::COMPRESS_TYPE_UNDEFINED ||
         opParam.quantType != infer::PagedAttentionParam::QuantType::TYPE_QUANT_UNDEFINED)) {
        ATB_LOG(ERROR) << "SPEC only support when compressType is COMPRESS_TYPE_UNDEFINED and "
                       << "quantType is TYPE_QUANT_UNDEFINED";
        return false;
    }
...
bool LogNParamCheck(const infer::PagedAttentionParam &opParam)
{
    ...
    if (opParam.scaleType == infer::PagedAttentionParam::SCALE_TYPE_LOGN) {
        if (opParam.quantType != infer::PagedAttentionParam::TYPE_QUANT_UNDEFINED) {
...
bool BNSDParamCheck(const infer::PagedAttentionParam &opParam)
{
    if (opParam.inputLayout == infer::InputLayout::TYPE_BNSD) {
        ...
        if (opParam.quantType != atb::infer::PagedAttentionParam::TYPE_QUANT_UNDEFINED) {
...
bool MlaParamCheck(const infer::PagedAttentionParam &opParam)
{
    if (opParam.mlaVHeadSize > 0) {
        if (opParam.maskType == infer::PagedAttentionParam::MaskType::MASK_TYPE_ALIBI ||
            opParam.maskType == infer::PagedAttentionParam::MaskType::MASK_TYPE_SPEC) {
            ATB_LOG(ERROR) << "mla mode does not support alibi mask or parallel decoding mask";
            return false;
        }
        if (opParam.compressType != infer::PagedAttentionParam::CompressType::COMPRESS_TYPE_UNDEFINED) {
        ...
        if (opParam.calcType != infer::PagedAttentionParam::CalcType::CALC_TYPE_UNDEFINED) {
            ATB_LOG(ERROR) << "mla mode does not support parallel decoding";
            return false;
        }
    ...
    : OperationBase("PagedAttentionOperation"), param_(param)
{
    ...
    ATB_LOG(INFO) << GetLogPrefix() << "PagedAttentionParam headNum:" << param.headNum << ", qkScale:" << param.qkScale
                  << ", kvHeadNum:" << param.kvHeadNum << ", maskType:" << param.maskType << ", batchRunStatusEnable"
                  << param.batchRunStatusEnable << ", quantType" << param.quantType << ", outDataType"
                  << param.outDataType << ", hasQuantOffset" << param.hasQuantOffset << ", compressType"
                  << param.compressType << ", calcType" << param.calcType << ", scaleType" << param.scaleType
                  << ", inputLayout" << param.inputLayout;
}
...
Status PagedAttentionOperation::KVCacheDimCheck910B(const SVector<TensorDesc> &inTensorDescs) const
{
    ...
    if (param_.quantType == infer::PagedAttentionParam::TYPE_DEQUANT_FUSION ||
        param_.calcType == infer::PagedAttentionParam::CALC_TYPE_SPEC || (needQKVQuant && param_.mlaVHeadSize == 0) ||
        param_.scaleType == infer::PagedAttentionParam::SCALE_TYPE_LOGN ||
        param_.compressType != infer::PagedAttentionParam::COMPRESS_TYPE_UNDEFINED) { // 非mla情况
