...
{
    if (normParam.quantType != infer::QUANT_UNDEFINED && normParam.quantType != infer::QUANT_INT8) {
        ATB_LOG(ERROR) << "quantType is not supported, only support QUANT_UNDEFINED/QUANT_INT8";
...
{
    if (preNormParam.quantType != infer::QUANT_UNDEFINED) {
...
{
    if (postNormParam.quantType != infer::QUANT_UNDEFINED && postNormParam.quantType != infer::QUANT_INT8) {
        ATB_LOG(ERROR) << "quantType is not supported, only support QUANT_UNDEFINED/QUANT_INT8";
        return ERROR_INVALID_PARAM;
    }
