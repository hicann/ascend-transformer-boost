...
bool ParamCheck(const atb::infer::FusedAddTopkDivParam opParam)
{
    ...
    if (!opParam.isNorm) {
        ATB_LOG(ERROR) << "isNorm should be true";
        return false;
    }
    return true;
}
...
FusedAddTopkDivOperation::FusedAddTopkDivOperation(const infer::FusedAddTopkDivParam &param)
    : OperationBase("FusedAddTopkDivOperation"),
      param_(param)
{
    operationIr_ = GetSingleton<AtbOperationIrCfg>().GetOperationIr("FusedAddTopkDivOperation");
}
...
Status FusedAddTopkDivOperation::InTensorDescsCheck(const SVector<TensorDesc> &inTensorDescs) const
{
    ...
    if (inTensor0Dim1 % 16 != 0) { // 16: multiple of 16
        ATB_LOG(ERROR) << GetLogPrefix() << "inTensor0 dim1 should be integer multiple of 16"
                                                ...
                                                const SVector<TensorDesc> &inTensorDescs) const
{
    ...
    if (!TensorCheck::IsTensorDescDimNumValid(yTensorDesc, 2)) {
        return ERROR_INVALID_TENSOR_DIM_NUM;
    }
