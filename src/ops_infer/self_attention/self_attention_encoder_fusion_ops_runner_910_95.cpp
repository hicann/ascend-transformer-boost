...
static const int IDX_2 = 2;
static const int IDX_3 = 3;
static const int IDX_4 = 4;

namespace atb {

SelfAttentionEncoderFusionOpsRunner91095::SelfAttentionEncoderFusionOpsRunner91095(
    const infer::SelfAttentionParam &param)
    : OpsRunner("SelfAttentionEncoderFusionOpsRunner91095", RUNNER_TYPE_SELF_ATTENTION_PA_ENCODER), param_(param)
{
    needKernelGraphModify_ = true;
    skipSetUpKernelGraphWhenCacheHit_ = false;
    ATB_LOG(INFO) << "SelfAttentionEncoderFusionOpsRunner91095::SelfAttentionEncoderFusionOpsRunner91095 called";
    ...
    // SVector<int> inputs = {TENSORLEN, KVLISTLEN, KVLISTLEN, TENSORLEN};
    // flashAttentionEncoderNode.inputLens = inputs;
...
bool SelfAttentionEncoderFusionOpsRunner91095::NeedModifySlopes(const OpsTensorPack &opsTensorPack)
{
    bool isMaskCompress = param_.maskType == atb::infer::SelfAttentionParam::MASK_TYPE_ALIBI_COMPRESS ||
                          param_.maskType == atb::infer::SelfAttentionParam::MASK_TYPE_ALIBI_COMPRESS_SQRT ||
                          param_.maskType == atb::infer::SelfAttentionParam::MASK_TYPE_ALIBI_COMPRESS_LEFT_ALIGN;
...
Status SelfAttentionEncoderFusionOpsRunner91095::ModifyKernelGraph(const OpsTensorPack &opsTensorPack)
{
    const size_t seqLenId = param_.mlaVHeadSize > 0 ?
                                (param_.maskType == atb::infer::SelfAttentionParam::MASK_TYPE_UNDEFINED ? IDX_2 : IDX_3) :
                                (param_.maskType == atb::infer::SelfAttentionParam::MASK_TYPE_UNDEFINED ? IDX_3 : IDX_4);
...
void SelfAttentionEncoderFusionOpsRunner91095::SetFAParam(AtbOps::OpParam::UnpadFlashAttention &flashAttentionParam)
{
    if (param_.mlaVHeadSize > 0) {
