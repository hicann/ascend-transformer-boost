...
static const uint32_t DN_MAX_VALUE = 65537;
static const uint32_t DN_MAX_VALUE2 = 96256;
...
Status NormRopeReshapeOperation::InferShapeCheckImpl(const SVector<TensorDesc> &inTensorDescs) const
{
    if (inTensorDescs[0].shape.dims[inTensorDescs[0].shape.dimNum - 1] > DN_MAX_VALUE) { // 0: 0th dim 0: 0th dim
        ATB_LOG(ERROR) << GetLogPrefix() << "The last dimNum of the first input tensor"
                                         << " should be less than or equal to 65536";
        return ERROR_INVALID_TENSOR_DIM;
    }
    if (inTensorDescs[2].shape.dims[inTensorDescs[2].shape.dimNum - 1] > DN_MAX_VALUE) { // 2: 2th dim 2: 2th dim
        ATB_LOG(ERROR) << GetLogPrefix() << "The last dimNum of the third input tensor"
                                         << " should be less than or equal to 65536";
        return ERROR_INVALID_TENSOR_DIM;
    }
...
bool NormRopeReshapeOperation::NormRopeReshapeCheckImpl910B(const SVector<TensorDesc> &inTensorDescs) const
{
    ...
    if (keycacheInTensorDesc.shape.dims[DIM_THREE] > DN_MAX_VALUE2) {
        ATB_LOG(ERROR) << GetLogPrefix() << "The fourth dimension of the seventh input tensor"
                                         << " should be less than or equal to 96256";
        return false;
    }
