...
bool MatmulParamCheck(const infer::LinearParam &opParam, ExternalError &error)
{
    if (opParam.enAccum) {
        ...
        if (!GetSingleton<Config>().Is910B() && !GetSingleton<Config>().Is310B()) {
            error.errorDesc =
                "Platform is not Atlas 800I A2 inference product or Atlas 200I/500 A2 inference product, enAccum "
                "should be false,";
                                       ...
                                       SVector<TensorDesc> &outTensorDescs) const
{
    ...
    if (param_.matmulType == infer::LinearParam::MATMUL_EIN_SUM) {
        outTensorDescs.at(0).shape.dims[0] = param_.transposeA ? inTensorDescs.at(0).shape.dims[DIM_2]
