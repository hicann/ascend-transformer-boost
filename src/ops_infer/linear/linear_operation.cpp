...
template <> Status CreateOperation(const infer::LinearParam &opParam, Operation **operation)
{
    ...
    if (opParam.outDataType == ACL_DT_UNDEFINED) {
        ...
        if (opParam.outDataType == ACL_FLOAT16) {
            if (!MatmulDequantFloat16ParamCheck(opParam, error)) {
                return ERROR_INVALID_PARAM;
            }
        } else if (opParam.outDataType == ACL_BF16) {
                                   ...
                                   const TensorDesc &weightTensorDesc) const
{
    ...
    int64_t accumBatch = OperationUtil::GetTensorBatch(accumTensorDesc);
    int64_t xBatch = OperationUtil::GetTensorBatch(xTensorDesc);
    ...
    int64_t xM = OperationUtil::GetXTensorM(xTensorDesc, param_.transposeA);
    if (accumM != xM) {
                                       ...
                                       const SVector<Tensor> &outTensors) const
{
    ...
    int64_t outBatch = OperationUtil::GetTensorBatch(outTensorDesc);
    int64_t xBatch = OperationUtil::GetTensorBatch(xTensorDesc);
    if (outBatch != xBatch) {
        error.errorType = ERROR_INVALID_TENSOR_DIM;
    ...
    int64_t outM = OperationUtil::GetOutTensorM(outTensorDesc);
    int64_t xM = OperationUtil::GetXTensorM(xTensorDesc, param_.transposeA);
    if (outM != xM) {
        error.errorType = ERROR_INVALID_TENSOR_DIM;
