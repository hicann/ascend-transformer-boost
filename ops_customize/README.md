...
加速库包的安装部署可参考CANN（Compute Architecture for Neural Networks，异构计算架构）开发环境，一般需要安装如下软件：  
  (1) 驱动和固件  
  (2) CANN软件包  
...
如果编译过程顺利，则会在加速库根目录下出现output文件夹，加速库根目录下执行
...
│   │   │   │   │   ├── tiling_data.h
│   │   │   │   │   ├── blockcopy_tiling.h
│   │   │   │   │   └── blockcopy_tiling.cpp
│   │   │   │   ├── blockcopy_kernel.cpp
│   │   │   │   ├── blockcopy_operation.cpp
│   │   │   │   └── CMakeLists.txt
│   │   │   └── ... //其它算子目录
...
文件名：`ascend-transformer-boost/scripts/build.sh`
加速库编译文件，文件中可设置日志存放目录、日志文件、编译器版本等，一般无需更改。

#### 环境变量设置文件`set_env.sh`

​**文件名**​：`ascend-transformer-boost/scripts/set_env.sh`
...
* `3rdparty/ascend-op-common-lib/3rdparty/ascend-transformer-boost/src/kernels/configs/mixkernels/op_list.yaml` 增加以下内容（**这一内容非常重要**，将新增算子信息加入列表，后续构建才会将新增算子的实现和接口真正编译进去）：
...
* 在`src/include/atb/core/runner_type.h`中为``RunnerType``增加枚举值`RUNNER_TYPE_FASTSOFTMAX`
...
* 在`atb/include/atb/train_op_params.h`，新增
...
与tiling相关的文件是`tiling_data.h`、`rope_tiling.h`、`rope_tiling.cpp`文件。
...
atb算子开发完成之后，用户可编写用例对算子进行功能测试，atb仓提供了python和C++两种方式。
...
[单算子-CANN商用版8.2.RC1-昇腾社区](https://www.hiascend.com/document/detail/zh/canncommercial/82RC1/acce/ascendtb/ascendtb_0046.html)（左上角选择最新版本文档）
...
![image-20250828152057505](./.assets\精度测试1.png)
...
![image-20250828152157760](./image-20250828152157760.png)
...
![image-20250828151958356](.assets\性能测试.png)
