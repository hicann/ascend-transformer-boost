    ...
    : OpsRunner("PositionEmbeddingFusionGatherOpsRunner", RUNNER_TYPE_POSITION_EMBEDDING_2D_MIXED_FUSION), param_(param)
{
    ...
    AsdOps::Tensor &positionIds = kernelGraph_.inTensors.at(tempInTensorNum++);
    AsdOps::Tensor &cosTable = kernelGraph_.inTensors.at(tempInTensorNum++);
    AsdOps::Tensor &sinTable = kernelGraph_.inTensors.at(tempInTensorNum++);

    kernelGraph_.outTensors.resize(OUT_TENSOR_SIZE);
    int64_t tempOutTensorNum = 0;
    AsdOps::Tensor &cos_sum = kernelGraph_.outTensors.at(tempOutTensorNum++);
    AsdOps::Tensor &sin_sum = kernelGraph_.outTensors.at(tempOutTensorNum++);
