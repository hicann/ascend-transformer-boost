...
#include "self_attention_cross_ops_runner_builder.h"

namespace AclTransformer {
SelfAttentionCrossOperation::SelfAttentionCrossOperation(const SelfAttentionCrossParam &param)
    : Operation("SelfAttentionCrossParam"), param_(param)
{
#ifdef USE_TORCH_RUNNER
    runnerBuilders_ = {new SelfAttentionCrossOpsRunnerBuilder(param_)};
#else
    runnerBuilders_ = {new SelfAttentionCrossOpsRunnerBuilder(param_)};
#endif
}

SelfAttentionCrossOperation::~SelfAttentionCrossOperation() {}

uint64_t SelfAttentionCrossOperation::GetInTensorCount() const {
    if (param_.model == "llama_adapter") {
        return 6;
    } else if (param_.model == "llama_adapter_a") {
        return 9;
    } else if (param_.model == "llama_adapter_encoder") {
        return 4;
    } else if (param_.model == "llama_adapter_encoder_a") {
        return 7;
    } else {
        return 6;
    }
...
uint64_t SelfAttentionCrossOperation::GetOutTensorCount() const { return 3; }

AsdOps::Status SelfAttentionCrossOperation::InferShapeImpl(const AsdOps::SVector<AsdOps::Tensor> &inTensors,
                                                             AsdOps::SVector<AsdOps::TensorDesc> &outTensorDescs) const
{
    if (param_.model == "llama_adapter" || param_.model == "llama_adapter_a") {
    ...
    } else if (param_.model == "llama_adapter_encoder" || param_.model == "llama_adapter_encoder_a") {
        outTensorDescs.at(0) = inTensors.at(0).desc;
        outTensorDescs.at(0).dims.clear();
        outTensorDescs.at(0).dims.push_back(inTensors.at(0).desc.dims.at(0));
        outTensorDescs.at(0).dims.push_back(inTensors.at(0).desc.dims.at(1));
...
RunnerBuilder *SelfAttentionCrossOperation::FindBestRunnerBuilder() const
{
#ifdef USE_TORCH_RUNNER
    size_t index = AsdOps::GetSingleton<Config>().IsSelfAttentionCrossOpsRunnerEnable() ? 0 : 1;
#else
    size_t index = 0;
#endif
