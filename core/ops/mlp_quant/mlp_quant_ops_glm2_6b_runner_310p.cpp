...
MlpQuantOpsGlm2Runner310P::MlpQuantOpsGlm2Runner310P(const MlpQuantParam &param)
    : OpsRunner("MlpQuantOpsGlm2Runner310P", RUNNER_TYPE_MLP_QUANT), param_(param)
{
    ASD_LOG(INFO) << "MlpQuantOpsGlm2Runner310P::MlpQuantOpsGlm2Runner310P called";
}

MlpQuantOpsGlm2Runner310P::~MlpQuantOpsGlm2Runner310P() {}

AsdOps::Status MlpQuantOpsGlm2Runner310P::SetupKernelGraphNz(const RunnerVariantPack &runnerVariantPack)
{
    ASD_LOG(INFO) << "MlpQuantOpsGlm2Runner310P::MlpQuantOpsGlm2Runner310P NZ  called";
    ...
    matmulUpNode.inTensors = {&transdataA1Out, &weightUp, &biasUp, &deqScaleUp};
    ...
    transdataA2Node.opDesc = {0, "TransdataOperation",
                             AsdOps::OpParam::Transdata({AsdOps::OpParam::Transdata::ND_TO_FRACTAL_NZ, {0, 0}})};
    transdataA2Node.inTensors = {&quantOut};
    ...
    matmulDownNode.inTensors = {&transdataA2Out, &weightDown, &biasDown, &deqScaleDown};
    ...
    ASD_LOG(INFO) << "MlpQuantOpsGlm2Runner310P::MlpQuantOpsGlm2Runner310P END";
    return AsdOps::Status::OkStatus();

}


AsdOps::Status MlpQuantOpsGlm2Runner310P::SetupKernelGraphNd(const RunnerVariantPack &runnerVariantPack)
{
    ASD_LOG(INFO) << "MlpQuantOpsGlm2Runner310P::MlpQuantOpsGlm2Runner310P ND called";
    const std::size_t inTensorSize = 7;
    const std::size_t outTensorSize = 1;
    const std::size_t internalTensorSize = 12;
    const std::size_t nodeSize = 12;
    const std::size_t dimSize = 4;
    ...
    size_t internalTensorId = 0;
    AsdOps::Tensor &transdataA1Out = kernelGraph_.internalTensors.at(internalTensorId++);
    AsdOps::Tensor &transdataW1Out = kernelGraph_.internalTensors.at(internalTensorId++);
    ...
    AsdOps::Tensor &chunk1 = kernelGraph_.internalTensors.at(internalTensorId++);
    AsdOps::Tensor &transdataA2Out = kernelGraph_.internalTensors.at(internalTensorId++);
    AsdOps::Tensor &transdataW2Out = kernelGraph_.internalTensors.at(internalTensorId++);
    ...
    size_t nodeId = 0;
    auto &transdataA1Node = kernelGraph_.nodes[nodeId++];
    auto &transdataW1Node = kernelGraph_.nodes[nodeId++];
    ...
    auto &quantNode = kernelGraph_.nodes[nodeId++];
    auto &transdataA2Node = kernelGraph_.nodes[nodeId++];
    auto &transdataW2Node = kernelGraph_.nodes[nodeId++];
    ...
    transdataW1Node.opDesc = {0, "TransdataOperation",
                             AsdOps::OpParam::Transdata({AsdOps::OpParam::Transdata::ND_TO_FRACTAL_NZ, {0, 0}})};
    transdataW1Node.inTensors = {&weightUp};
    transdataW1Node.outTensors = {&transdataW1Out};
    transdataW1Node.inTensorViewFuncs.resize(transdataW1Node.inTensors.size());
    transdataW1Node.inTensorViewFuncs.at(0) = CheckDimB;
    ...
    matmulUpNode.opDesc = {0, "MatMulOperation", AsdOps::OpParam::MatMul({false, true})};
    matmulUpNode.inTensors = {&transdataA1Out, &transdataW1Out, &biasUp, &deqScaleUp};
    matmulUpNode.outTensors = {&matmulUpOut};
    matmulUpNode.inTensorViewFuncs.resize(matmulUpNode.inTensors.size());
    // matmulUpNode.inTensorViewFuncs.at(1) = CheckDimBViewFunc;
    matmulUpNode.inferShapePreFunc = [&](AsdOps::RunInfo &runInfo) {
    ...
    transdataW2Node.opDesc = {0, "TransdataOperation",
                             AsdOps::OpParam::Transdata({AsdOps::OpParam::Transdata::ND_TO_FRACTAL_NZ, {0, 0}})};
    transdataW2Node.inTensors = {&weightDown};
    transdataW2Node.outTensors = {&transdataW2Out};
    transdataW2Node.inTensorViewFuncs.resize(transdataW2Node.inTensors.size());
    transdataW2Node.inTensorViewFuncs.at(0) = CheckDimB;

    matmulDownNode.opDesc = {0, "MatMulOperation", AsdOps::OpParam::MatMul({false, true})};
    matmulDownNode.inTensors = {&transdataA2Out, &transdataW2Out, &biasDown, &deqScaleDown};
    matmulDownNode.outTensors = {&matmulDownOut};
    matmulDownNode.inTensorViewFuncs.resize(matmulDownNode.inTensors.size());
    // matmulDownNode.inTensorViewFuncs.at(1) = CheckDimBViewFunc;
    matmulDownNode.inferShapePreFunc = [&](AsdOps::RunInfo &runInfo) {
...
AsdOps::Status MlpQuantOpsGlm2Runner310P::SetupKernelGraph(const RunnerVariantPack &runnerVariantPack)
{
