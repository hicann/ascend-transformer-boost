    ...
    : OpsRunner("SelfAttentionKvCacheOpsGptNeox20bRunner", RUNNER_TYPE_SELF_ATTENTION_KV_CACHE), param_(param)
{
    ...
    const size_t inTensorSize = 6;
    const size_t outTensorSize = 3;
    const size_t interTensorSize = 12;
    const size_t nodeSize = 15;

    kernelGraph_.inTensors.resize(inTensorSize);
    int64_t inTensorId = 0;
    AsdOps::Tensor &mixedQuery = kernelGraph_.inTensors.at(inTensorId++);  //[bs, 1, hn, hs]
    AsdOps::Tensor &mixedKey = kernelGraph_.inTensors.at(inTensorId++);   //[bs, 1, hn, hs]
    AsdOps::Tensor &mixedValue = kernelGraph_.inTensors.at(inTensorId++);  //[bs, 1, hn, hs]
    AsdOps::Tensor &attention_mask = kernelGraph_.inTensors.at(inTensorId++);  // [1, 1, 1, sq]
    AsdOps::Tensor &pastKey = kernelGraph_.inTensors.at(inTensorId++);     // [bs, sq, hn, hs]
    AsdOps::Tensor &pastValue = kernelGraph_.inTensors.at(inTensorId++);    // [bs, sq, hn, hs]
