...
#include <numeric>
#include <cmath>
#include <asdops/params/broadcast.h>
...
static const uint64_t GROUPS_NUM = 8;

namespace AclTransformer {
SelfAttentionKvCacheOpsLlama70bRunner::SelfAttentionKvCacheOpsLlama70bRunner(const SelfAttentionKvCacheParam &param)
     : OpsRunner("SelfAttentionKvCacheOpsLlama70bRunner", RUNNER_TYPE_SELF_ATTENTION_KV_CACHE), param_(param)
{
     ASD_LOG(INFO) << "SelfAttentionKvCacheOpsLlama70bRunner::SelfAttentionKvCacheOpsLlama70bRunner called, transKey:"
     ...
     AsdOps::Tensor &repeatKeyOut = kernelGraph_.internalTensors.at(internalTensorNum++);
     AsdOps::Tensor &repeatValueOut = kernelGraph_.internalTensors.at(internalTensorNum++);
     ...
     auto &repeatKeyNode = kernelGraph_.nodes.at(nodeNum++);
     auto &repeatValueNode = kernelGraph_.nodes.at(nodeNum++);
     ...
     repeatKeyNode.opDesc = {0, "BroadcastOperation",
                             AsdOps::OpParam::Broadcast({AsdOps::OpParam::Broadcast::BROADCAST_EXPAND})};
     repeatKeyNode.inTensors = {&catKeyOut};
     repeatKeyNode.outTensors = {&repeatKeyOut};
     repeatKeyNode.inTensorViewFuncs.resize(repeatKeyNode.inTensors.size());
     repeatKeyNode.inTensorViewFuncs[0] = [=](const AsdOps::SVector<int64_t> &oldDims,
                                              AsdOps::SVector<int64_t> &newDims) {
         newDims = {oldDims.at(0), oldDims.at(1), 1, oldDims.at(2), oldDims.at(3)};
     };
     repeatKeyNode.inferShapePreFunc = [&](AsdOps::RunInfo &runInfo) {
         AsdOps::OpParam::Broadcast expandKParam = {AsdOps::OpParam::Broadcast::BROADCAST_EXPAND};
         AsdOps::SVector<int64_t> dims = runInfo.GetInTensor(0).desc.dims;
         expandKParam.shape = {dims[0], dims[1], GROUPS_NUM, dims[3], dims[4]};
         runInfo.SetOpDesc({0, "BroadcastOperation", AsdOps::OpParam::Broadcast(expandKParam)});
     };

     repeatValueNode.opDesc = {0, "BroadcastOperation",
                             AsdOps::OpParam::Broadcast({AsdOps::OpParam::Broadcast::BROADCAST_EXPAND})};
     repeatValueNode.inTensors = {&catValueOut};
     repeatValueNode.outTensors = {&repeatValueOut};
     repeatValueNode.inTensorViewFuncs.resize(repeatValueNode.inTensors.size());
     repeatValueNode.inTensorViewFuncs[0] = [=](const AsdOps::SVector<int64_t> &oldDims,
                                              AsdOps::SVector<int64_t> &newDims) {
         newDims = {oldDims.at(0), oldDims.at(1), 1, oldDims.at(2), oldDims.at(3)};
     };
     repeatValueNode.inferShapePreFunc = [&](AsdOps::RunInfo &runInfo) {
         AsdOps::OpParam::Broadcast expandKParam = {AsdOps::OpParam::Broadcast::BROADCAST_EXPAND};
         AsdOps::SVector<int64_t> dims = runInfo.GetInTensor(0).desc.dims;
         expandKParam.shape = {dims[0], dims[1], GROUPS_NUM, dims[3], dims[4]};
         runInfo.SetOpDesc({0, "BroadcastOperation", AsdOps::OpParam::Broadcast(expandKParam)});
     ...
     transposePresentKNode.inTensors = {&repeatKeyOut};
     ...
     transposePresentKNode.inTensorViewFuncs[0] = [=](const AsdOps::SVector<int64_t> &oldDims,
                                                      AsdOps::SVector<int64_t> &newDims) {
         newDims = {oldDims.at(0), oldDims.at(1) * oldDims.at(2), oldDims.at(3), oldDims.at(4)};
     };
     ...
     castInNode.inferShapePreFunc = [](AsdOps::RunInfo &runInfo) {
         for (size_t i = 0; i < runInfo.GetInTensorCount(); i++) {
             runInfo.GetInTensor(i).desc.format = AsdOps::TENSOR_FORMAT_ND;
         }
     };
     ...
     bmmVNode.inTensors = {&attentionProbs, &repeatValueOut};
...
SelfAttentionKvCacheOpsLlama70bRunner::~SelfAttentionKvCacheOpsLlama70bRunner() {}

void SelfAttentionKvCacheOpsLlama70bRunner::AsStrideKernelInferShapeSet(const AsdOps::SVector<int64_t> &sequence,
