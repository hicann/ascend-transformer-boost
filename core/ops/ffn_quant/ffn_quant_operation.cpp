...
#include "ffn_quant_ops_runner_builder.h"
...
FfnQuantOperation::FfnQuantOperation(const FfnQuantParam &param) : Operation("FfnQuantOperation"), param_(param)
{
    runnerBuilders_ = {new FfnQuantOpsRunnerBuilder(param_)};
}
...
bool FfnQuantOperation::IsConsistent(const AsdOps::SVector<AsdOps::TensorDesc> &inTensorDescs,
                                     AsdOps::SVector<AsdOps::TensorDesc> &outTensorDescs) const
{
...
int64_t FfnQuantOperation::GetTensorBatch(const AsdOps::TensorDesc &tensorDesc) const
{
...
int64_t FfnQuantOperation::GetTensorH(const AsdOps::TensorDesc &tensorDesc) const
{
    // make sure dims.size() == 2 or 3
    if (tensorDesc.dims.size() == DIM_2) {
        return tensorDesc.dims[DIM_0];
...
int64_t FfnQuantOperation::GetTensorW(const AsdOps::TensorDesc &tensorDesc) const
{
    // make sure dims.size() == 2 or 3
    if (tensorDesc.dims.size() == DIM_2) {
        return tensorDesc.dims[DIM_1];
...
RunnerBuilder *FfnQuantOperation::FindBestRunnerBuilder() const
{
    size_t index = 0;
