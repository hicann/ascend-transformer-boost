...
MlpOpsLlamaAdapterRunner310P::MlpOpsLlamaAdapterRunner310P(const MlpParam &param) : OpsRunner("MlpOpsLlamaAdapterRunner310P", RUNNER_TYPE_MLP), param_(param)
{
    ASD_LOG(INFO) << "MlpOpsLlamaAdapterRunner310P::MlpOpsLlamaAdapterRunner310P called";
    ...
    AsdOps::Tensor &biasGate = kernelGraph_.inTensors.at(inTensorNum++);
    AsdOps::Tensor &weightDown = kernelGraph_.inTensors.at(inTensorNum++);
    AsdOps::Tensor &biasDown = kernelGraph_.inTensors.at(inTensorNum++);
    AsdOps::Tensor &weightUp = kernelGraph_.inTensors.at(inTensorNum++);
    AsdOps::Tensor &biasUp = kernelGraph_.inTensors.at(inTensorNum++);

    int64_t outTensorNum = 0;
    AsdOps::Tensor &mlpOutput = kernelGraph_.outTensors.at(outTensorNum++);
    ...
    AsdOps::Tensor &matmulGateOutNDbias = kernelGraph_.internalTensors.at(internalTensorNum++);
    ...
    AsdOps::Tensor &matmulUpOutNDbias = kernelGraph_.internalTensors.at(internalTensorNum++);
    ...
    add1Node.inTensors = {&matmulGateOutND, &biasGate};
    add1Node.outTensors = {&matmulGateOutNDbias};
    
    float scale = 1.0;
    swishNode.opDesc = {0, "ElewiseOperation", AsdOps::OpParam::Elewise({AsdOps::OpParam::Elewise::ELEWISE_SWISH, scale})};
    swishNode.inTensors = {&matmulGateOutNDbias};
    ...
    add2Node.inTensors = {&matmulUpOutND, &biasUp};
    add2Node.outTensors = {&matmulUpOutNDbias};

    mulNode.opDesc = {0, "BroadcastOperation", AsdOps::OpParam::Broadcast({AsdOps::OpParam::Broadcast::BROADCAST_MUL})};
    mulNode.inTensors = {&swishOut, &matmulUpOutNDbias};
    ...
    add3Node.opDesc = {0, "BroadcastOperation", AsdOps::OpParam::Broadcast({AsdOps::OpParam::Broadcast::BROADCAST_ADD})};
    add3Node.inTensors = {&resultTensor, &biasDown};
    add3Node.outTensors = {&mlpOutput};

}

MlpOpsLlamaAdapterRunner310P::~MlpOpsLlamaAdapterRunner310P() {}
} // namespace AclTransformer