...
static const uint64_t INTERMIDIATE_TENSOR_COUNT = 18;
static const uint64_t NODE_COUNT = 21;
namespace AclTransformer {
SelfAttentionOpsLlama7bRunner910a::SelfAttentionOpsLlama7bRunner910a(const SelfAttentionParam &param)
    : OpsRunner("SelfAttentionOpsLlama7bRunner910a", RUNNER_TYPE_SELF_ATTENTION), param_(param)
{
    ASD_LOG(INFO) << "SelfAttentionOpsLlama7bRunner910a::SelfAttentionOpsLlama7bRunner910a called, transKey:"
    ...
    kernelGraph_.internalTensors.resize(INTERMIDIATE_TENSOR_COUNT);
    ...
    AsdOps::Tensor &permuteQTransResult = kernelGraph_.internalTensors.at(internalTensorNum++);
    ...
    AsdOps::Tensor &bmmQkTransResult = kernelGraph_.internalTensors.at(internalTensorNum++);
    ...
    AsdOps::Tensor &permutedVTransResult = kernelGraph_.internalTensors.at(internalTensorNum++);
    ...
    auto &transdataProbNode = kernelGraph_.nodes.at(nodeNum++);
    ...
    AsdOps::OpParam::Transpose permutePresentVParam = {AsdOps::OpParam::Transpose::TransposeType::TRANSPOSE, {2, 0, 1, 3}};
    permutePresentVNode.opDesc = {0, "TransposeOperation", permutePresentVParam};
    ...
    AsdOps::OpParam::Transpose transposePresentKParam= {AsdOps::OpParam::Transpose::TransposeType::TRANSPOSE, {2, 0, 1, 3}};
    ...
    transdataQNode.outTensors = {&permuteQTransResult};
    ...
    bmmQKNode.inTensors = {&permuteQTransResult, &transposedKTransResult};
    ...
    transdataQKNode.outTensors = {&bmmQkTransResult};
    ...
    castInNode.outTensors = {&attentionProbsF32};

    softMaxNode.opDesc = {0, "NormOperation", AsdOps::OpParam::Norm({AsdOps::OpParam::Norm::NORM_SOFTMAX, {-1}})};
    softMaxNode.inTensors = {&attentionProbsF32};
    softMaxNode.outTensors = {&attentionProbsF32};

    castInNode.opDesc = {0, "ElewiseOperation", AsdOps::OpParam::Elewise({AsdOps::OpParam::Elewise::ELEWISE_CAST})};
    castInNode.inTensors = {&attentionProbsF32};
    castInNode.outTensors = {&attentionProbs};

    transdataProbNode.opDesc = {0, "TransdataOperation",
                             AsdOps::OpParam::Transdata({AsdOps::OpParam::Transdata::ND_TO_FRACTAL_NZ, {0, 0}})};
    transdataProbNode.inTensors = {&attentionProbs};
    transdataProbNode.outTensors = {&attentionProbsTransResult};
    transdataProbNode.inferShapePreFunc = [&](AsdOps::RunInfo &runInfo) {
        for (size_t i = 0; i < runInfo.GetInTensorCount(); i++) {
            runInfo.GetInTensor(i).desc.format = AsdOps::TENSOR_FORMAT_ND;
        }
    ...
    transdataVNode.opDesc = {0, "TransdataOperation",
                             AsdOps::OpParam::Transdata({AsdOps::OpParam::Transdata::ND_TO_FRACTAL_NZ, {0, 0}})};
    transdataVNode.inTensors = {&permutedV};
    transdataVNode.outTensors = {&permutedVTransResult};
    ...
    bmmVNode.inTensors = {&attentionProbsTransResult, &permutedVTransResult};
...
SelfAttentionOpsLlama7bRunner910a::~SelfAttentionOpsLlama7bRunner910a() {}
