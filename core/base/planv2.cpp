...
PlanV2::PlanV2() { hostTilingBuffer_.resize(HOST_TILING_BUFFER_DEFAULT_SIZE); }

PlanV2::~PlanV2() {}

AsdOps::Status PlanV2::Setup(Handle handle, const VariantPack &variantPack)
{
...
uint64_t PlanV2::GetWorkspaceSize()
{
...
AsdOps::Status PlanV2::Execute(Handle handle, VariantPack &variantPack)
{
    runnerVariantPack_.inTensors = variantPack.inTensors;
    runnerVariantPack_.outTensors = variantPack.outTensors;
...
AsdOps::Status PlanV2::CopyHostTilingToDevice(Handle handle)
{
    if (runnerVariantPack_.tilingBufferSize > 0) {
