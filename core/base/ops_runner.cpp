...
AsdOps::Status OpsRunner::SetupImpl(const RunnerVariantPack &runnerVariantPack)
{
    ...
    if (AsdOps::GetSingleton<Config>().IsOpsRunnerSetupCacheEnable() && setupCacheEnable_) {
        if (IsRunnerVariantPackInputEqual(runnerVariantPack, lastRunnerVariantPack_)) {
            ...
            ASD_LOG(INFO) << GetName() << " runnerVariantPack input is change, setup do";
        }
    ...
    AsdOps::Status st = SetupKernelGraph(runnerVariantPack);
    if (!st.Ok()) {
        return st;
    }
    ...
    if (!PlanKernelGraph(runnerVariantPack)) {
...
void OpsRunner::CalcKernelWorkspace()
{
    ...
    for (size_t nodeId = 0; nodeId < kernelGraph_.nodes.size(); ++nodeId) {
        ...
        if (tilingSize > 0) {
            ...
            ASD_LOG(INFO) << GetName() << " node[" << nodeId << "] " << kernel->GetName()
                          << " workspaces.size:" << workspaces.size();
            uint64_t kernelWorkspaceSize = 0;
            for (size_t i = 0; i < workspaces.size(); ++i) {
                kernelWorkspaceSize += workspaces.at(i);
                ASD_LOG(INFO) << GetName() << " node[" << nodeId << "] " << kernel->GetName() << " workspaces[" << i
                              << "]:" << workspaces.at(i);
            }
...
AsdOps::Status OpsRunner::ExecuteImpl(Handle &handle, RunnerVariantPack &runnerVariantPack)
{
    ...
    for (size_t nodeId = 0; nodeId < kernelGraph_.nodes.size(); ++nodeId) {
        ...
        for (uint64_t tensorId = 0; tensorId < node.kernelRunInfo.GetInTensorCount(); tensorId++) {
            ...
            if (node.inTensorsType.at(tensorId) == TensorType::INTERMEDIATE_TENSOR) {
                ...
                ASD_LOG(INFO) << GetName() << " update tensor data, node[" << nodeId << "].inTensors[" << tensorId
                              << "] is intermeidate tensor, data:" << tensor.data;
            } else {
                tensor.data = node.inTensors.at(tensorId)->data;
                ASD_LOG(INFO) << GetName() << " update tensor data, node[" << nodeId << "].inTensors[" << tensorId
                              << "] is not intermeidate tensor, data:" << tensor.data;
            }
        }
        for (uint64_t tensorId = 0; tensorId < node.kernelRunInfo.GetOutTensorCount(); tensorId++) {
            ...
            if (node.outTensorsType.at(tensorId) == TensorType::INTERMEDIATE_TENSOR) {
                ...
                ASD_LOG(INFO) << GetName() << " update tensor data, node[" << nodeId << "].outTensors[" << tensorId
                              << "] is intermeidate tensor, data:" << tensor.data;
            } else {
...
AsdOps::Status OpsRunner::UpdateRunInfoTiling(RunnerVariantPack &runnerVariantPack)
{
    ...
    if (totalTilingSize_ > 0) {
        ...
        for (size_t i = 0; i < kernelGraph_.nodes.size(); ++i) {
            ...
            kernelRunInfo.SetDeviceLaunchBuffer(static_cast<char *>(runnerVariantPack.tilingBuffer) + offset,
                                                tilingBufferSize);
...
void OpsRunner::UpdateRunInfoWorkspace(RunnerVariantPack &runnerVariantPack)
{
    ...
    if (workspaceSize_ > 0) {
        for (auto &node : kernelGraph_.nodes) {
            ...
            uint64_t offset = 0;
            for (size_t i = 0; i < workspaces.size(); ++i) {
                deviceLaunchBufferWorkspace[i] = (char *)runnerVariantPack.workspaceBuffer + offset;
...
bool OpsRunner::PlanKernelGraph(const RunnerVariantPack &runnerVariantPack)
{
    for (size_t nodeId = 0; nodeId < kernelGraph_.nodes.size(); ++nodeId) {
...
bool OpsRunner::PlanOneKernel(size_t nodeId)
{
    ...
    if (op == nullptr) {
        ASD_LOG(ERROR) << GetName() << " node[" << nodeId
                       << "] get operation by name fail, opName:" << node.opDesc.opName;
        return false;
    }
    ...
    if (it != maxNodeIdTensorMap_.end()) {
        for (auto tensorIt : it->second) {
            ...
            ASD_LOG(INFO) << GetName() << " node[" << nodeId << "] " << opDesc.opName << " mem free:" << tensorIt->data;
        }
    }
...
{
    ASD_LOG(INFO) << GetName() << " node[" << nodeId << "] " << op->GetName() << " infer shape start, runInfo:\n"
                  << node.kernelRunInfo.ToString();
    if (node.inferShapePreFunc) {
        ASD_LOG(INFO) << GetName() << " node[" << nodeId << "] " << op->GetName()
                      << " call inferShapePreFunc, old runInfo:\n"
                      << node.kernelRunInfo.ToString();
        node.inferShapePreFunc(node.kernelRunInfo);
        ASD_LOG(INFO) << GetName() << " node[" << nodeId << "] " << op->GetName()
                      << " call inferShapePreFunc, new runInfo:\n"
                      << node.kernelRunInfo.ToString();
    }
    AsdOps::Status st = op->InferShape(node.kernelRunInfo);
    if (!st.Ok()) {
        ASD_LOG(ERROR) << op->GetName() << " node[" << nodeId << "] infer shape fail, error:" << st.Message();
        return false;
    }
    ASD_LOG(INFO) << GetName() << " node[" << nodeId << "] " << op->GetName() << " infer shape success, runInfo:\n"
                  << node.kernelRunInfo.ToString();
    ...
    ASD_LOG(INFO) << GetName() << " node[" << nodeId << "] " << op->GetName() << " after mem solve, runinfo:\n"
                  << node.kernelRunInfo.ToString();
    return true;
}
...
bool OpsRunner::PlanOneKernelSelectBestKernel(AsdOps::Operation *op, KernelGraphNode &node, size_t nodeId)
{
    if (AsdOps::GetSingleton<Config>().IsOpsRunnerKernelCacheEnable()) {
        ...
        if (node.kernel) {
            ASD_LOG(INFO) << GetName() << " node[" << nodeId << "] " << op->GetName()
                          << ", get cached best kernel:" << node.kernel->GetName();
    ...
    if (node.kernel == nullptr) {
        ASD_LOG(ERROR) << GetName() << " node[" << nodeId << "] " << tactic->GetName()
                       << " get best kernel fail, kernel count:" << tactic->GetKernelCount();
        return false;
    }
    ASD_LOG(INFO) << GetName() << " node[" << nodeId << "] " << op->GetName()
                  << " get best tactic:" << tactic->GetName() << ", best kernel:" << node.kernel->GetName();

    if (AsdOps::GetSingleton<Config>().IsOpsRunnerKernelCacheEnable()) {
        AsdOps::GetSingleton<KernelCache>().Add(runnerType_, nodeId, node.kernelRunInfo, node.kernel);
        ASD_LOG(INFO) << GetName() << " node[" << nodeId << "] " << op->GetName()
                      << ", cache best kernel:" << node.kernel->GetName();
    }

    return true;
}

void OpsRunner::CalcTilingBufferSize(const RunnerVariantPack &runnerVariantPack)
{
...
AsdOps::Status OpsRunner::SetupKernelGraph(const RunnerVariantPack &runnerVariantPack)
{
    return AsdOps::Status::OkStatus();
}
                                              ...
                                              const RunnerVariantPack &runnerVariantPack2)
{
    ...
    for (size_t i = 0; i < runnerVariantPack1.inTensors.size(); ++i) {
        if (!TensorUtil::AsdOpsTensorDescEqual(runnerVariantPack1.inTensors.at(i).desc,
