 ...
 */
#include "gptneox20bmodel_decoder_torch.h"
...
GptNeox20BModelDecoderTorch::GptNeox20BModelDecoderTorch()
{
    ASD_LOG(INFO) << "GptNeox20BModelDecoderTorch::GptNeox20BModelDecoderTorch, TASK_QUEUE_ENABLE:"
...
void GptNeox20BModelDecoderTorch::SetParam(std::string param)
{
    ...
    for (int i = 0; i < modelParam_.layerNum; ++i) {
        ...
        AclTransformer::Operation *op = new AclTransformer::GptNeox20BLayerDecoderOperation(opParam);
    ...
    ASD_LOG(INFO) << "GptNeox20BModelDecoder set param end";
}

void GptNeox20BModelDecoderTorch::SetWeight(std::vector<torch::Tensor> weightTensors)
{
    if (weightTensors.size() != modelParam_.layerNum * WEIGHT_COUNT_PER_LAYER) {
        ASD_LOG(ERROR) << "GptNeox20BModelDecoder set weight fail, weightTensors.size:" << weightTensors.size()
                       << " != " << modelParam_.layerNum * WEIGHT_COUNT_PER_LAYER;
        return;
    }

    ASD_LOG(INFO) << "GptNeox20BModelDecoder set weight success, size:" << weightTensors.size();

    weightTensors_ = weightTensors;
    Utils::ContiguousAtTensor(weightTensors_);
}

std::vector<torch::Tensor> GptNeox20BModelDecoderTorch::Execute(torch::Tensor hiddenStateTensor,
...
void GptNeox20BModelDecoderTorch::ExecuteOut(torch::Tensor hiddenStateTensor, torch::Tensor positionIdTensor,
...
void GptNeox20BModelDecoderTorch::ExecuteOutImpl(torch::Tensor &hiddenStateTensor, torch::Tensor &positionIdTensor,
                                                 torch::Tensor &cosTableTensor, torch::Tensor &sinTableTensor,
                                                 torch::Tensor &attentionMaskTensor, torch::Tensor &seqLen,
                                                 std::vector<torch::Tensor> pastKeyTensors,
                                                 std::vector<torch::Tensor> pastValueTensors,
                                                 ...
                                                 std::vector<torch::Tensor> &presentValueTensors, bool newOut)
{
    if ((int)presentKeyTensors.size() != modelParam_.layerNum ||
        ...
        ASD_LOG(ERROR) << "GptNeox20BModelDecoder presentKeyTensors.size:" << presentKeyTensors.size()
...
//        opAtInTensors.at(inTensorId++) = seqLen;              // seqLen
    ...
    ASD_LOG(FATAL) << "GptNeox20BModelDecoder executeCount:" << executeCount_ << ", statistic:["
...
void GptNeox20BModelDecoderTorch::BuildVariantPack(int layerId, std::vector<torch::Tensor> &atInTensors,
                                                  torch::Tensor &outTensor, torch::Tensor &presentKeyTensor,
                                                  torch::Tensor &presentValueTensor, bool newOut,
                                                  AclTransformer::VariantPack &variantPack)
{
    for (size_t i = 0; i < atInTensors.size(); ++i) {
        ASD_LOG(INFO) << "GptNeox20BModelDecoderLayer_" << layerId << " atInTensors[" << i
...
void GptNeox20BModelDecoderTorch::ExecuteSingleOperation(int layerId, std::vector<torch::Tensor> &opAtInTensors,
                                                        torch::Tensor &outTensor, torch::Tensor &presentKeyTensor,
                                                        torch::Tensor &presentValueTensor, bool newOut)
{
    ...
    if (!st.Ok()) {
        ASD_LOG(ERROR) << "GptNeox20BModelDecoderLayer_" << layerId << " setup plan fail, not call execute";
        return;
    }

    variantPack.workspaceSize = plan.GetWorkspaceSize();
    ASD_LOG(INFO) << "GptNeox20BModelDecoderLayer_" << layerId << " get plan workspace size:" << variantPack.workspaceSize;
    ...
    if (AsdOps::GetSingleton<AclTransformer::Config>().IsSaveTensor()) {
        ...
        std::string dirPath =
            AclTransformer::Config::GetSaveTensorDir() + "/GptNeox20BModelDecoderLayer_" + std::to_string(layerId);
        AclTransformer::TensorUtil::SaveVariantPack(handle_, variantPack, dirPath);
        ASD_LOG(FATAL) << "GptNeox20BModelDecoderLayer_" << layerId << " save variant pack, dir:" << dirPath;
    }

    AsdOps::GetSingleton<AclTransformer::Statistic>().planExecuteTime += timer2.ElapsedMicroSecond();
    ASD_LOG_IF(!st.Ok(), ERROR) << "GptNeox20BModelDecoderLayer_" << layerId << " execute plan fail, error:" << st.Message();
}

TORCH_LIBRARY(GptNeox20BModelDecoderTorch, m)
{
    m.class_<GptNeox20BModelDecoderTorch>("GptNeox20BModelDecoderTorch")
        .def(torch::init<>())
        .def("set_param", &GptNeox20BModelDecoderTorch::SetParam)
        .def("set_weight", &GptNeox20BModelDecoderTorch::SetWeight)
        .def("execute", &GptNeox20BModelDecoderTorch::Execute)
        .def("execute_out", &GptNeox20BModelDecoderTorch::ExecuteOut);
}