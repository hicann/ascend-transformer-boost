 ...
 */
#ifndef CHATGLM6BMODEL_DECODER_QUANT_FLASH_TORCH_H
...
class ChatGlm6BModelDecoderQuantFlashTorch : public torch::CustomClassHolder {
public:
    ChatGlm6BModelDecoderQuantFlashTorch();
    ~ChatGlm6BModelDecoderQuantFlashTorch();
    ...
    c10::intrusive_ptr<ChatGlm6BModelDecoderQuantFlashTorch> clone() const
    {
        return c10::make_intrusive<ChatGlm6BModelDecoderQuantFlashTorch>();
    }

private:
    void BuildVariantPack(int layerId, std::vector<torch::Tensor> &atInTensors, torch::Tensor &outTensor,
                          torch::Tensor &resIn, bool newOut, AclTransformer::VariantPack &variantPack);
