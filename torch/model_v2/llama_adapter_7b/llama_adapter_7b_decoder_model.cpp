...
*/
#include "llama_adapter_7b_decoder_model.h"
...
void LlamaAdapter7BDecoderModel::Param::FromString(const std::string &param)
{
    ...
    ASD_LOG(INFO) << "Llama_Adapter_7B_Decoder_Model param rmsNormEps:" << rmsNormEps << ", headNum:" << headNum
                    << ", dk:" << dk << ", layerNum:" << layerNum;
}

LlamaAdapter7BDecoderModel::LlamaAdapter7BDecoderModel(const std::string &param): Model("LlamaAdapter7BDecoderModel", param)
{
    param_.FromString(param);
}

LlamaAdapter7BDecoderModel::~LlamaAdapter7BDecoderModel(){}

uint64_t LlamaAdapter7BDecoderModel::GetInTensorCount() const { return graph_.inTensors.size(); }

uint64_t LlamaAdapter7BDecoderModel::GetOutTensorCount() const { return graph_.outTensors.size(); }

AsdOps::Status LlamaAdapter7BDecoderModel::InferShape(const std::vector<AsdOps::Tensor> &inTensors,
                                                 std::vector<AsdOps::TensorDesc> &outTensorDescs)
{
    ...
    const AsdOps::Tensor &keyTensor = inTensors.at(IN_TENSOR_PASTKV_START);
    const AsdOps::Tensor &valueTensor = inTensors.at(IN_TENSOR_PASTKV_START + param_.layerNum);
...
void LlamaAdapter7BDecoderModel::BuildGraph()
{
    ...
    ASD_LOG(INFO) << "Llama_Adapter_7B_Decoder_Model nodeSize is " << nodeSize;
    ...
    for (int layerId = 0; layerId < param_.layerNum; ++layerId) {
        ...
        if (layerId == 0){
            opParam.model = "llama_adapter";
            layerNode.operation = std::make_shared<LLaMAAdapter7BLayerOperation>(opParam);
            ...
            layerNode.inTensors.at(inTensorId++) = &graph_.inTensors.at(IN_TENSOR_PASTKV_START + param_.layerNum + layerId);
            ...
            opParam.model = "llama_adapter_a";
            layerNode.operation = std::make_shared<LLaMAAdapter7BLayerAdapterOperation>(opParam);
            layerNode.inTensors.resize(layerNode.operation->GetInTensorCount());
...
AsdOps::Status LlamaAdapter7BDecoderModel::ParseVarintPackParam(const std::string &param, int nodeId,
