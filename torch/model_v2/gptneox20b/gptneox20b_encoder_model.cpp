 ...
 */
#include "gptneox20b_encoder_model.h"
...
void GptNeox20BEncoderModel::Param::FromString(const std::string &param)
{
    ...
    ASD_LOG(INFO) << "GptNeox20BEncoderModel param layerNormEps:" << layerNormEps << ", headNum:" << headNum
                  << ", transKey:" << transKey << ", dk:" << dk << ", layerNum:" << layerNum
                  << ", rotaryPct:" << rotaryPct << ", beginNormAxis:" << beginNormAxis;
}

GptNeox20BEncoderModel::GptNeox20BEncoderModel(const std::string &param) : Model("GptNeox20BEncoderModel", param)
{
    param_.FromString(param);
}

GptNeox20BEncoderModel::~GptNeox20BEncoderModel() {}

uint64_t GptNeox20BEncoderModel::GetInTensorCount() const { return graph_.inTensors.size(); }

uint64_t GptNeox20BEncoderModel::GetOutTensorCount() const { return graph_.outTensors.size(); }

AsdOps::Status GptNeox20BEncoderModel::InferShape(const std::vector<AsdOps::Tensor> &inTensors,
...
void GptNeox20BEncoderModel::BuildGraph()
{
    ...
    ASD_LOG(INFO) << "GptNeox20BEncoderModel nodeSize is " << nodeSize;
    ...
    for (int layerId = 0; layerId < param_.layerNum; ++layerId) {
        ...
        layerNode.operation = std::make_shared<GptNeox20BLayerEncoderOperation>(opParam);
