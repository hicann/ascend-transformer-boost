 ...
 */
#include "chatglm6b_decoder_dequant_flashattention_model.h"
...
void ChatGlm6BDecoderDequantFlashAttentionModel::Param::FromString(const std::string &param)
{
    ...
    ASD_LOG(INFO) << "CChatGlm6BDecoderDequantFlashAttentionModel param layerNormEps:" << layerNormEps << ", headNum:" << headNum
                  << ", transKey:" << transKey << ", dk:" << dk << ", layerNum:" << layerNum
                  << ", residualAddScale:" << residualAddScale 
                  << ", tokenOffset:" << tokenOffset << ", seqLen:" << seqLen;
}

ChatGlm6BDecoderDequantFlashAttentionModel::ChatGlm6BDecoderDequantFlashAttentionModel(const std::string &param) : Model("ChatGlm6BDecoderDequantFlashAttentionModel", param)
{
    param_.FromString(param);
}

ChatGlm6BDecoderDequantFlashAttentionModel::~ChatGlm6BDecoderDequantFlashAttentionModel() {}

uint64_t ChatGlm6BDecoderDequantFlashAttentionModel::GetInTensorCount() const { return graph_.inTensors.size(); }

uint64_t ChatGlm6BDecoderDequantFlashAttentionModel::GetOutTensorCount() const { return graph_.outTensors.size(); }

AsdOps::Status ChatGlm6BDecoderDequantFlashAttentionModel::InferShape(const std::vector<AsdOps::Tensor> &inTensors,
...
void ChatGlm6BDecoderDequantFlashAttentionModel::BuildGraph()
{
    ...
    for (int layerId = 0; layerId < param_.layerNum; ++layerId) {
        ...
        ChatGlm6BLayerDequantFlashAttentionParam opParam;
        ...
        opParam.perm = param_.perm;
        layerNode.operation = std::make_shared<ChatGlm6BLayerDecoderDequantFlashAttentionOperation>(opParam);
...
AsdOps::Status ChatGlm6BDecoderDequantFlashAttentionModel::ParseVarintPackParam(const std::string &param, int nodeId,
                                                           AsdOps::Any &variantPackParam)
{
    ...
    ASD_LOG(INFO) << "ChatGlm6BDecoderDequantFlashAttentionModel SelfAttentionKvCacheFusionVariantPackParam tokenOffset:"
