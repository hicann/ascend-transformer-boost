 ...
 */
#include "chatglm6b_encoder_without_fusion_model.h"
...
void ChatGlm6BEncoderWithoutFusionModel::Param::FromString(const std::string &param)
{
...
ChatGlm6BEncoderWithoutFusionModel::ChatGlm6BEncoderWithoutFusionModel(const std::string &param) : Model("ChatGlm6BEncoderWithoutFusionModel", param)
{
    param_.FromString(param);
}

ChatGlm6BEncoderWithoutFusionModel::~ChatGlm6BEncoderWithoutFusionModel() {}

uint64_t ChatGlm6BEncoderWithoutFusionModel::GetInTensorCount() const { return graph_.inTensors.size(); }

uint64_t ChatGlm6BEncoderWithoutFusionModel::GetOutTensorCount() const { return graph_.outTensors.size(); }

AsdOps::Status ChatGlm6BEncoderWithoutFusionModel::InferShape(const std::vector<AsdOps::Tensor> &inTensors,
                                                                std::vector<AsdOps::TensorDesc> &outTensorDescs)
{
    ...
    const AsdOps::Tensor &keyTensor = inTensors.at(0);
...
void ChatGlm6BEncoderWithoutFusionModel::BuildGraph()
{
    ...
    for (int layerId = 0; layerId < param_.layerNum; ++layerId) {
        ...
        layerNode.operation = std::make_shared<ChatGlm6BLayerEncoderWithoutFusionOperation>(opParam);
        ...
        if (layerId != param_.layerNum - 1) {
            layerNode.outTensors = {&graph_.internalTensors.at(layerId),
                                    ...
                                    &graph_.outTensors.at(param_.layerNum + layerId + 1)};
        } else {
...
AsdOps::Status ChatGlm6BEncoderWithoutFusionModel::ParseVarintPackParam(const std::string &param, int nodeId,
