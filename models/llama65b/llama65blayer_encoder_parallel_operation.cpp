 ...
 */
#include "llama65blayer_encoder_parallel_operation.h"
    ...
    : GraphOperation("LLaMA65BLayerEncoderOperation"), param_(param)
{
    ...
    mlpNode.operation.reset(new AclTransformer::MlpOperation({true, "llama65b"}));
...
LLaMA65BLayerEncoderOperation::~LLaMA65BLayerEncoderOperation() {}

uint64_t LLaMA65BLayerEncoderOperation::GetInTensorCount() const { return IN_TENSOR_COUNT; }

uint64_t LLaMA65BLayerEncoderOperation::GetOutTensorCount() const { return OUT_TENSOR_COUNT; }

AsdOps::Status LLaMA65BLayerEncoderOperation::InferShapeImpl(const AsdOps::SVector<AsdOps::Tensor> &inTensors,
