 ...
 */
#include "llama13blayer_fusion_quant_operation.h"
...
enum LLaMA13BLayerFusionQuantTensorId {
    IN_HIDDENSTATES = 0,
    IN_QMIXDWEIGHT,
    IN_QMIXDDEQSCALE,
    IN_QMIXDBIAS,
...
LLaMA13BLayerFusionQuantOperation::LLaMA13BLayerFusionQuantOperation(const LLaMA13BLayerFusionQuantParam &param)
    : GraphOperation("LLaMA13BLayerFusionQuantOperation"), param_(param)
{
    ...
    mixdQLinearNode.operation.reset(new AclTransformer::LinearQuantOperation({param_.transposeA, param_.transposeB}));
    mixdQLinearNode.inTensorIds = {INTERMIDATE_INPUTNORMOUT, IN_QMIXDWEIGHT, IN_QMIXDBIAS, IN_QMIXDDEQSCALE};
    mixdQLinearNode.outTensorIds = {INTERMIDATE_MIXEDQ};

    mixdKLinearNode.operation.reset(new AclTransformer::LinearQuantOperation({param_.transposeA, param_.transposeB}));
    mixdKLinearNode.inTensorIds = {INTERMIDATE_INPUTNORMOUT, IN_KMIXDWEIGHT, IN_KMIXDBIAS, IN_KMIXDDEQSCALE};
    mixdKLinearNode.outTensorIds = {INTERMIDATE_MIXEDK};

    mixdVLinearNode.operation.reset(new AclTransformer::LinearQuantOperation({param_.transposeA, param_.transposeB}));
    mixdVLinearNode.inTensorIds = {INTERMIDATE_INPUTNORMOUT, IN_VMIXDWEIGHT, IN_VMIXDBIAS, IN_VMIXDDEQSCALE};
    ...
    selfOutQuantNode.operation.reset(new AclTransformer::QuantOperation({param_.inputScale_2, param_.inputOffset_2}));
    selfOutQuantNode.inTensorIds = {INTERMIDATE_SELFOUT};
    selfOutQuantNode.outTensorIds = {INTERMIDATE_SELFQUANTOUT};

    selfOutLinearNode.operation.reset(new AclTransformer::LinearQuantOperation({param_.transposeA, param_.transposeB}));
    ...
    selfResidualAddNode.operation.reset(new AclTransformer::AddOperation({param_.scale}));
    ...
    mlpQuantParam.inputScale = param_.inputScale_4;
    mlpQuantParam.inputOffset = param_.inputOffset_4;
    mlpNode.operation.reset(new AclTransformer::MlpQuantOperation(mlpQuantParam));
    mlpNode.inTensorIds = {INTERMIDATE_SELFNORMOUT, IN_MLPGATEWEIGHT, IN_MLPGATEDEQSCALE, IN_MLPGATEBIAS, IN_MLPDOWNWEIGHT, IN_MLPDOWNDEQSCALE, IN_MLPDOWNBIAS, IN_MLPUPWEIGHT, IN_MLPUPDEQSCALE, IN_MLPUPBIAS};
    mlpNode.outTensorIds = {INTERMIDATE_MLPOUT};

    mlpResidualAddNode.operation.reset(new AclTransformer::AddOperation({param_.scale}));
...
LLaMA13BLayerFusionQuantOperation::~LLaMA13BLayerFusionQuantOperation() {}

uint64_t LLaMA13BLayerFusionQuantOperation::GetInTensorCount() const { return IN_TENSOR_COUNT; }

uint64_t LLaMA13BLayerFusionQuantOperation::GetOutTensorCount() const { return OUT_TENSOR_COUNT; }

AsdOps::Status LLaMA13BLayerFusionQuantOperation::InferShapeImpl(const AsdOps::SVector<AsdOps::Tensor> &inTensors,
