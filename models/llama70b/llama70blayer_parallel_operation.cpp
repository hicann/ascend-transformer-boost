 ...
 */
#include "llama70blayer_parallel_operation.h"
...
LLaMA70BLayerOperation::LLaMA70BLayerOperation(const LLaMA70BLayerParam &param)
    : GraphOperation("LLaMA70BLayerOperation"), param_(param)
{
    ...
    kPositionEmbeddingNode.operation.reset(new AclTransformer::PositionEmbedding1dSplitOperation({param_.kvHeadNum}));
    ...
    selfAttentionKvCacheNode.inTensorViewFuncs.at(2) = [=](const AsdOps::SVector<int64_t> &oldDims,
        ...
        newDims = {oldDims.at(0), oldDims.at(1), param_.kvHeadNum, oldDims.at(2) / param_.kvHeadNum};
    };

    selfOutLinearParallelNode.operation.reset(
        new AclTransformer::LinearParallelOperation(
                {false, param_.rank, param_.rankSize, 0, "None", "RowParallel", "hccl"}));
    ...
    mlpNode.operation.reset(new AclTransformer::MlpOperation({true, "llama70b"}));
...
LLaMA70BLayerOperation::~LLaMA70BLayerOperation() {}

uint64_t LLaMA70BLayerOperation::GetInTensorCount() const { return IN_TENSOR_COUNT; }

uint64_t LLaMA70BLayerOperation::GetOutTensorCount() const { return OUT_TENSOR_COUNT; }

AsdOps::Status LLaMA70BLayerOperation::InferShapeImpl(const AsdOps::SVector<AsdOps::Tensor> &inTensors,
