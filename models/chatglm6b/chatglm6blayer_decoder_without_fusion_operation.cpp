...
ChatGlm6BLayerDecoderWithoutFusionOperation::ChatGlm6BLayerDecoderWithoutFusionOperation(
    const ChatGlm6BLayerParam &param)
    : GraphOperation("ChatGlm6BLayerDecoderWithoutFusionOperation"), param_(param)
{
    ...
    inputNormNode.operation.reset(new AclTransformer::NormOperation({param_.layerNormEps, param_.beginNormAxis, 1}));
    ...
    positionEmbeddingNode.operation.reset(new AclTransformer::PositionEmbeddingOperation({true, param_.headNum}));
    ...
    selfNormNode.operation.reset(new AclTransformer::NormOperation({param_.layerNormEps, param_.beginNormAxis, 1}));
...
uint64_t ChatGlm6BLayerDecoderWithoutFusionOperation::GetInTensorCount() const { return IN_TENSOR_COUNT; }

uint64_t ChatGlm6BLayerDecoderWithoutFusionOperation::GetOutTensorCount() const { return OUT_TENSOR_COUNT; }

AsdOps::Status
ChatGlm6BLayerDecoderWithoutFusionOperation::InferShapeImpl(const AsdOps::SVector<AsdOps::Tensor> &inTensors,
