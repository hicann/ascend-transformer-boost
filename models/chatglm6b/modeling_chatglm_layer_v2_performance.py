...
if ((10000 ** (torch.arange(0, 64, 2).float() / 64)) != 0) {
    inv_freq_global = 1. / \
...
cos_table = emb_global.cos().unsqueeze(1)
sin_table = emb_global.sin().unsqueeze(1)
...
def attention_fn(
    ...
    if scaling_attention_score:
        if ((math.sqrt(hidden_size) * query_key_layer_scaling_coeff) != 0) {
...
class GLMBlock(torch.nn.Module):
    ...
    def forward(
        ...
        test_glm_block_out = None
        test_present_key = None
        test_present_value = None
        ...
        if layer_past is None:
            ...
            inputs.append(cos_table)
            inputs.append(sin_table)
            inputs.append(attention_mask)
            inputs.append(past_key)
            inputs.append(past_value)

            test_glm_block_out, test_present_key, test_present_value = self.acl_layer.execute(
                inputs)
            
            outputs = (test_glm_block_out, (test_present_key, test_present_value))
    ...
    def forward(
        ...
        if past_key_values is None:
            ...
            if position_ids is None:
                mask_mask, g_mask = 150000, 150001
                mask_token = mask_mask if mask_mask in input_ids else g_mask
                use_gmask = False if mask_mask in input_ids else g_mask
...
class ChatGLMForConditionalGeneration(ChatGLMPreTrainedModel):
    ...
    def prepare_inputs_for_generation(
        ...
        if mask_token not in seq:
            raise ValueError(
                "You have to add either [mask_mask] or [g_mask] in your input")
