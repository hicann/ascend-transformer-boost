...
class ChatGLMModel(ChatGLMPreTrainedModel):
    ...
    def __init__(self, config: ChatGLMConfig):
        ...
        acl_param = json.dumps({"transKey": True, "dk": 128, "headNum": 32, "layerNum": 28,
    ...
    def forward(
        ...
        if self.weightFlag is False:
            ...
            weights = [weight for i, weight in enumerate(weights) if ((i == 0) or (i % 2 != 0))]
        ...
        if past_key_values[0] is not None:
            ...
            present_keys = [torch.zeros(t.shape[0] + 1, *t.shape[1:]) for t in past_keys]
            present_values = [torch.zeros(t.shape[0] + 1, *t.shape[1:]) for t in past_values]
