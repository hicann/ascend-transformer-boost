...
for name, mod in model.named_modules():
    if type(mod).__name__ == "GLMBlock":
        ...
        mod.qkv_deq_scale = torch.nn.Parameter(mod.qkv_deq_scale.data.to(torch.float32))
        mod.dense_deq_scale = torch.nn.Parameter(mod.dense_deq_scale.data.to(torch.float32))
        mod.hto4h_deq_scale = torch.nn.Parameter(mod.hto4h_deq_scale.data.to(torch.float32))
        mod.fhtoh_deq_scale = torch.nn.Parameter(mod.fhtoh_deq_scale.data.to(torch.float32))
