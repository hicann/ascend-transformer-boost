...
def attention_fn(
    ...
    #     torch.save(key_layer.cpu(), 'intensor0.pth')
    #     torch.save(value_layer.cpu(), 'intensor1.pth')
    #     torch.save(query_layer.cpu(), 'intensor4.pth')
...
class GLMBlock(torch.nn.Module):
    def __init__(
        ...
        self.acl_input_layernorm_operation = torch.classes.OperationTorch.OperationTorch(
            "NormOperation")
        ...
        self.acl_ffn_linear_operation = torch.classes.OperationTorch.OperationTorch(
            "LinearOperation")
    ...
    def forward(
        ...
        if layer_past is not None and layer_id == 0:
            ...
            # torch.save(k_cache_input.cpu(), 'intensor2.pth')
            # torch.save(v_cache_input.cpu(), 'intensor3.pth')
            # torch.save(attention_mask.cpu(), 'intensor5.pth')
            # torch.save(seq_len.cpu(), 'intensor6.pth')
            # torch.save(token_offset.cpu(), 'intensor7.pth')
            # torch.save(layer_id_input.cpu(), 'intensor8.pth')
        ...
        if layer_past is not None and layer_id == 0:
            ...
            print("token num before:" + str(token_num[layer_id]))
            torch.save(attention_output.cpu(), 'outtensor0.pth')
            # torch.save(k_cache_input.cpu(), 'outtensor1.pth')
            # torch.save(v_cache_input.cpu(), 'outtensor2.pth')
            # exit()
        ...
        torch.save(output.cpu(), 'expect_jao.pth')
        ...
        if increment_flags[layer_id]:
            ...
            # torch.save(inputs[17].cpu(), 'k_cache_input_flash.pth')
            # torch.save(inputs[18].cpu(), 'v_cache_input_flash.pth')
            # torch.save(inputs[16].cpu(), 'attention_mask_flash.pth')
            # torch.save(inputs[19].cpu(), 'seq_len_flash.pth')
            ...
            # if torch.allclose(output, ffn_residual_add_out[0], rtol=0.02, atol=0.02):
            #     print("equal")
            # else:
            #     print("not euqal")
            print("!!!!!!!!!!!!!!output:" + str(output))
            torch.save(output.cpu(), 'expect_jao.pth')
            assert F.cosine_similarity(output.view(output.numel()), ffn_residual_add_out[0].view(
                ffn_residual_add_out[0].numel()), dim=0).item() >= 0.99, 'fail'
            print("success!")
            exit()
            # output = ffn_residual_add_out[0]
...
class ChatGLMForConditionalGeneration(ChatGLMPreTrainedModel):
    ...
    def stream_chat(self, tokenizer, query: str, history: List[Tuple[str, str]] = None, max_length: int = 2048,
                    do_sample=True, top_p=0.7, temperature=0.95, logits_processor=None, **kwargs):
        global increment_flags
        if increment_flags is not None:
            increment_flags = [False] * len(increment_flags)
        if history is None:
            history = []
        if logits_processor is None:
            logits_processor = LogitsProcessorList()
