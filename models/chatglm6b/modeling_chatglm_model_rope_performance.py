...
class GLMBlock(torch.nn.Module):
    ...
    def forward(
        ...
        if layer_past is None:
            test_glmBlockOut, test_presentKey, test_presentValue = self.acl_layer_encoder.execute(inputs)
        else:
            pastKey, pastValue = layer_past
            inputs.append(pastKey)
            inputs.append(pastValue)
            test_glmBlockOut, test_presentKey, test_presentValue = self.acl_layer.execute(inputs)
...
class ChatGLMModel(ChatGLMPreTrainedModel):
    ...
    def forward(
        ...
        global cosTable
        global sinTable
        seq_len = torch.tensor([hidden_states.shape[0]] * hidden_states.shape[1], device=hidden_states.device)
        if past_key_values[0] is None:
            acl_model_out = self.acl_encoder_operation.execute(hidden_states, position_ids, cosTable, sinTable, attention_mask, seq_len)
            ...
            acl_model_out = self.acl_decoder_operation.execute(hidden_states, position_ids, cosTable, sinTable, attention_mask, past_keys, past_values, seq_len)
            presents = tuple(zip(acl_model_out[1:self.num_layers + 1], acl_model_out[self.num_layers + 1:]))
