...
class GLMBlock(torch.nn.Module):
    def __init__(
        ...
        self.acl_add_norm = torch.classes.OperationTorch.OperationTorch("AddNormOperation")
        add_norm_param = {"layerNormEps": layernorm_epsilon}
        self.acl_add_norm.set_param(json.dumps(add_norm_param))
    ...
    def forward(
        ...
        hidden_states = attention_input * alpha
        mlp_input = self.post_attention_layernorm(hidden_states + attention_output)
        
        acl_mlp_input = self.acl_add_norm.execute(
            [hidden_states, attention_output, self.post_attention_layernorm.weight, self.post_attention_layernorm.bias])
        if torch.allclose(mlp_input, acl_mlp_input[0], rtol=0.02, atol=0.02):
            print("equal")
        else:
            print("not equal")
            exit()
