...
class GLU(torch.nn.Module):
    def __init__(self, hidden_size, inner_hidden_size=None,
        ...
        self.acl_linear = torch.classes.OperationTorch.OperationTorch(
            "LinearOperation")
        self.acl_linear.set_param('{"transposeA":false,"transposeB":true}')

        self.linear_testcase = TensorTestCase('GLULinear', in_tensor_num=3)
    ...
    def forward(self, hidden_states):
        ...
        acl_intermediate_parallels = self.acl_linear.execute([hidden_states,
        ...
        if torch.allclose(acl_intermediate_parallel, intermediate_parallel, rtol=0.02, atol=0.02):
            ...
            print("not euqal")
            self.linear_testcase.set_in_tensors(
                [hidden_states, self.dense_h_to_4h.weight, self.dense_h_to_4h.bias])
            self.linear_testcase.set_out_tensors([intermediate_parallel])
            self.linear_testcase.write(1)
            print("hidden_states = " + str(hidden_states))
            print("self.dense_h_to_4h.weight = " +
                  str(self.dense_h_to_4h.weight))
            print("self.dense_h_to_4h.bias = " + str(self.dense_h_to_4h.bias))
            print("acl_intermediate_parallel = " +
                  str(acl_intermediate_parallel))
            print("intermediate_parallel = " + str(intermediate_parallel))
            exit()
        intermediate_parallel2 = self.activation_func(
            acl_intermediate_parallel)

        output = self.dense_4h_to_h(intermediate_parallel2)
