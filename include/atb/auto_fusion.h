...
public:
    ...
    //!
    void DoAutoFusion(const std::set<std::string> &fusionClassArray = {});
private:
    //! \brief 判断id为传入参数的tensor是不是其他node的输入
    //!
    //! \param id 被判断的tensor id
    //!
    std::vector<uint32_t> findNodesWhichInputsIs(const uint32_t id);

    //! \brief 针对手机到的node进行分析，筛选出可以被融合的nodes
    //!
    std::vector<std::pair<std::string, std::vector<uint32_t>>> parseCollectNodes();
    ...
    //! return fusionclassAndIndex 需要更新的图结构中的node pair所对应的融合之后算子的存储路径
    //!
    std::vector<std::pair<std::string, std::vector<uint32_t>>> ParseFusion();
    ...
    //! \return 生成的二进制的存储路径
    //!
    std::string callBiShengIR(const std::pair<std::string, std::vector<uint32_t>> &subFusion);
