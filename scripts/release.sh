...
function fn_build_mki()
{
    ...
    bash scripts/build.sh release --namespace=AtbOps $build_options_atbops
    cd $THIRD_PARTY_DIR
    tar -xf mki.tar.gz
    rm -f mki.tar.gz

    cd $THIRD_PARTY_DIR/Mind-KernelInfra
    build_options_asdops="$build_options --output=$THIRD_PARTY_DIR/Mind-KernelInfra/output"
    [[ -d "$THIRD_PARTY_DIR"/Mind-KernelInfra/build ]] && rm -rf $THIRD_PARTY_DIR/Mind-KernelInfra/build
    bash scripts/build.sh release --namespace=AsdOps $build_options_asdops
    cd $THIRD_PARTY_DIR/Mind-KernelInfra/output
    tar -xf mki.tar.gz
    rm -f mki.tar.gz
}
...
function fn_build_asdops()
{
    ...
    cp -r $THIRD_PARTY_DIR/Mind-KernelInfra/output/mki 3rdparty
...
function fn_init_makeself()
{
    ...
    rm -rf makeself
    git clone -b 2.5.0-h0.computing.cann.r2 https://szv-open.codehub.huawei.com/OpenSourceCenter/megastep/makeself.git
}
