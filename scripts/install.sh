...
function install_torch_atb() {
    if [ "${torch_atb_flag}" == "y" ]; then
        if ! pip install ${sourcedir}/whl/torch_atb-0.0.1-py3-none-any.whl; then
            print "ERROR" "torch_atb install failed!"
            exit 1
        else
            print "INFO" "torch_atb install success!"
            install_torch_atb_dir=$(pip show torch_atb | grep Location | awk '{print $2}')/torch_atb
            if [ ! -d "$install_torch_atb_dir" ]; then
                echo "ERROR: torch_atb installation directory not found."
                exit 1
            fi
        fi
    fi
