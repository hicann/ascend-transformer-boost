...
do { \
    ...
    magicGt.SetGlobalBuffer((__gm__ int32_t *)commArgsTmp0>magics); \
    if (GetBlockIdx() == 0) { \
...
#define LCCL_TYPE_AIC_FUNC(fun) \
    ...
    fun(float, _mix_aic); fun(float16_t, _mix_aic); fun(bfloat16, _mix_aic);

#define LCCL_TYPE_AIV_FUNC(fun) \
    fun(int, _mix_aiv); fun(int8_t, _mix_aiv); fun(int16_t, _mix_aiv); fun(int64_t, _mix_aiv); \
    fun(float, _mix_aiv); fun(float16_t, _mix_aiv); fun(bfloat16, _mix_aiv);
