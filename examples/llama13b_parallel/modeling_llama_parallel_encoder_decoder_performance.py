...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        #     torch.save(query_states.cpu(), 'rank'+str(rank)+'_'+'1_out0.path')        
        key_states = self.k_proj(hidden_states).view(bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2)
        # if past_key_value is not None:
        #     torch.save(key_states.cpu(), 'rank'+str(rank)+'_'+'2_out0.path')
        value_states = self.v_proj(hidden_states).view(bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2)
        # if past_key_value is not None:
        #     torch.save(value_states.cpu(), 'rank'+str(rank)+'_'+'3_out0.path')
        ...
        #     torch.save(query_states.cpu().permute(2, 0, 1, 3),'rank'+str(rank)+'_'+ '4_out0.path')
        #     torch.save(key_states.cpu().permute(2, 0, 1, 3),'rank'+str(rank)+'_'+ '5_out0.path')
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig):
        ...
        head_num=config.num_attention_heads// self.world_size
        rank=torch.distributed.get_rank()
        rankSize=torch.distributed.get_world_size()
        param = json.dumps({"headNum": head_num, "rmsNormEps": config.rms_norm_eps,
                            "dk": headSize, "model": "llama13b", "rank": rank, "rankSize": rankSize})
        # print(param)
        self.acl_llama_encoder_layer.set_param(param)
...
#---------accelerate code---------------------
        ...
        if past_key_value is None:
            ...
            test_llama13bOut, test_presentKey, test_presentValue = self.acl_llama_encoder_layer.execute(
                inputs)

            residual = hidden_states
            # if test_llama13bOut is not None:
            #     torch.save(hidden_states.cpu(), 'rank'+str(rank)+'_'+'0_in0.path')
            hidden_states = self.input_layernorm(hidden_states)
            # if test_llama13bOut is not None:
            #     torch.save(self.input_layernorm.weight.cpu(), 'rank'+str(rank)+'_'+'0_in1.path')
            #     torch.save(hidden_states.cpu(), 'rank'+str(rank)+'_'+'0_out00.path')
            # Self Attention
            ...
            # if test_llama13bOut is not None:
            #     torch.save(hidden_states.cpu(), 'rank'+str(rank)+'_'+'8_out0.path')
            ...
            # if test_llama13bOut is not None:
            #     torch.save(hidden_states.cpu(), 'rank'+str(rank)+'_'+'10_out0.path')
            hidden_states = self.mlp(hidden_states)

            if self.world_size >=2:
                torch.distributed.all_reduce(hidden_states, op=torch.distributed.ReduceOp.SUM)
            # if test_llama13bOut is not None:
            #     torch.save(hidden_states.cpu(),'rank'+str(rank)+'_'+ '11_out0.path')
            #reduce
            hidden_states = residual + hidden_states
            ...
            if len(self.acl_weights) == 0:
                ...
                #     self.hidden_size//self.world_size).npu().half())
                self.acl_weights.append(weights[1].npu())
                # self.acl_weights.append(torch.zeros(
                #     self.hidden_size//self.world_size).npu().half())
                self.acl_weights.append(weights[2].npu())
            ...
            test_llama13bOut, test_presentKey, test_presentValue = self.acl_llama_layer.execute(
                inputs)
...
#-------------raw code---------------
        ...
        # if test_llama13bOut is not None:
        #     torch.save(hidden_states.cpu(), 'rank'+str(rank)+'_'+'12_out0.path')
