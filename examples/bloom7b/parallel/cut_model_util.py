...
def cut_weights(model,world_size,cut_row_keys=['dense_h_to_4h'],cut_col_keys=['dense','dense_4h_to_hh']):
    ...
    for key, tensor in model.state_dict().items():
        ...
        if key_short == 'query_key_value':
            ...
            for i in range(3):
                cut_tensor_list[0] = torch.cat((cut_tensor_list[0], torch.chunk(tensor_list[i], world_size, dim=0)[0]), 1)
                cut_tensor_list[1] = torch.cat((cut_tensor_list[1], torch.chunk(tensor_list[i], world_size, dim=0)[0]), 1)
            if key_type == "weight":
...
if __name__ == "__main__":
    ...
    tokenizer = AutoTokenizer.from_pretrained(args.input_path, use_fast=False) #load the tokrnnizer
    ...
    model = BloomForCausalLM.from_pretrained(args.input_path, torch_dtype=torch.float16) #load the raw model
    state_dict_list = cut_weights(model,args.world_size,args.cut_row_keys,args.cut_col_keys) #cut the weight
    ...
    for i in range(args.world_size):
        create_model.load_state_dict(state_dict_list[i]) #load the weights to the model
        create_model.save_pretrained(args.output_path+'/part_model/'+str(i)+'/') #save model
