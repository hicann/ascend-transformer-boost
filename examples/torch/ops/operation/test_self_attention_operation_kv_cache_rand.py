...
class TestAddNormal(unittest.TestCase):
    def test_2d(self):
        ...
        # out
        result = torch.rand(12, 1, 32, 128).npu()
        present_key = torch.rand(12, 1, 32, 128).npu()
        present_value = torch.rand(12, 1, 32, 128).npu()

        operation.execute("SelfAttentionKvCacheOperation", json.dumps(
            {"transKey":True, "dk":128, "headNum":32, "layerId":13}), [query, key, value, attention_mask, past_key, past_value], [result, present_key, present_value])
