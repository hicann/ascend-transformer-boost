...
class TensorTestCase:
    def __init__(self, test_name, in_tensor_num = 1, out_tensor_num = 1) -> None:
        ...
        # self.acl_path = os.getenv("ACLTRANSFORMER_HOME_PATH")
        # if not self.acl_path:
        #     print("env ACLTRANSFORMER_HOME_PATH not exist!")
    ...
    def read(self, test_idx):
        self.in_tensor_list = []
        self.out_tensor_list = []
        dir_path = os.path.join(self.acl_path, 'testcases', self.test_name, str(test_idx))
        for i in range(1, self.in_tensor_num + 1):
            self.in_tensor_list.append(torch.load(os.path.join(dir_path, f"inTensor{i}.pth")))
        for i in range(1, self.out_tensor_num + 1):
            self.out_tensor_list.append(torch.load(os.path.join(dir_path, f"outTensor{i}.pth")))
