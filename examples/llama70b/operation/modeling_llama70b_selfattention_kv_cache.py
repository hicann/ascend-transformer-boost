...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        # self_attention_kv_cache accelerate
        ...
        self.acl_attention_operation.set_param(
            json.dumps({"headNum": self.num_heads, "dk": self.head_dim, "model": "llama70b"})
        )
    ...
    def forward(
        ...
        # check attention kv cache operation
        context_layer = None
        if past_key_value is not None:
            context_layer, present_key, present_value = (
                self.acl_attention_operation.execute([query_states.permute(2, 0, 1, 3),
                                                      key_states.permute(2, 0, 1, 3),
                                                      value_states.permute(2, 0, 1, 3),
                                                      attention_mask,
                                                      past_key_value[0].permute(2, 0, 1, 3),
                                                      past_key_value[1].permute(2, 0, 1, 3)]))
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    _tied_weights_keys = ["lm_head_weight"]
    def __init__(self, config):
        super().__init__(config)
