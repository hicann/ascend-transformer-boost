...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig):
        ...
        self.acl_llama_layer = torch.classes.OperationTorch.OperationTorch(
            "LLaMA70BLayerOperation")
        headSize = config.hidden_size // config.num_attention_heads
        head_num = config.num_attention_heads // self.world_size
        kv_head_num = config.num_key_value_heads // self.world_size
        rank=torch.distributed.get_rank()
        rankSize=torch.distributed.get_world_size()
        param = json.dumps({"headNum": head_num, "kvHeadNum": kv_head_num, "rmsNormEps": config.rms_norm_eps,
                            "dk": headSize, "model": "llama70b", "rank": rank, "rankSize": rankSize})
    ...
    def forward(
        ...
        if past_key_value is not None:
            ...
            test_llama70bOut, test_presentKey, test_presentValue = self.acl_llama_layer.execute(
                inputs)
        #-------------raw code---------------
        ...
        if test_llama70bOut is not None:
            print('******************************compare layer*************************************')
            print("------------------------hidden_states----------------------- \n",hidden_states)
            print("------------------------test_llama70bOut----------------------- \n",test_llama70bOut)
            assert torch.allclose(hidden_states, test_llama70bOut, rtol=0.02, atol=0.02), "not equal"
            print("equal")

        # if test_llama70bOut is not None:
        #     outputs = (test_llama70bOut,)
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        cosTable, sinTable = rot_emb_global.forward(x, 1024)
