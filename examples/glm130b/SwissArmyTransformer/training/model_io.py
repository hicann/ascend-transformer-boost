...
def save_checkpoint(iteration, model, optimizer,
    ...
    if torch.distributed.get_rank() == 0:
        ...
        with open(os.path.join(args.save, 'model_config.json'), 'w') as f:
            ...
            # model specific args
            model_specific_args = extract_model_specific_args_from_model(
                args, module)
...
def save_ds_checkpoint(iteration, model, lr_scheduler, args):
    ...
    save_ds_checkpoint_no_optim(
        model, args.save, str(iteration), client_state=sd)
...
def load_checkpoint(model, args, load_path=None, prefix=''):
    ...
    # 使用sleep来避免多卡同时加载导致CPU内存不足报错
    # rank = torch.distributed.get_rank()
    # sleeptime = rank * 20
    # time.sleep(sleeptime) # avoid out of cpu memory
    # print('rank {} sleep for {} second to load checkpoint...'.format(rank, sleeptime))
