...
from SwissArmyTransformer.mpu.mappings import gather_from_model_parallel_region, copy_to_model_parallel_region, scatter_to_model_parallel_region
...
class BaseTransformer(torch.nn.Module):
    def __init__(self,
        ...
        self.acl_glm130b_decoder_operation_v2.set_param(acl_param)
        ...
        self.wordTableFlag=True
        self.wordTable=None
        acl_param_word = json.dumps({
                            "axis": 0,
                            "rank": self.rank,
                            "rankSize": self.rankSize,
                            "rankRoot": 0,
        ...
        self.acl_glm130b_word_embedding = torch.classes.OperationTorch.OperationTorch("EmbeddingParallelOperation")
    ...
    def forward(self, input_ids, position_ids, attention_mask, *,
        ...
        output_cross_layer = {}
        if self.wordTableFlag:
            wordTable = list(self.state_dict().values())[0] #[part_v,embedding_dim]
            # print("*************wordTable,raw********************",wordTable.shape,wordTable.device)
            wordTable = wordTable.T.contiguous() #[embedding_dim,part_v]
            # print("*************wordTable,T********************",wordTable.shape,wordTable.device)
            wordTable = gather_from_model_parallel_region(wordTable) #[embedding_dim,v]
            # print("*************wordTable,gather********************",wordTable.shape,wordTable.device)
            wordTable = wordTable.T.contiguous() #[v,embedding_dim]
            # print("*************wordTable,gather then T********************",wordTable.shape,wordTable.device)
            wordTable = scatter_to_model_parallel_region(wordTable) #[v,part_embedding_dim]
            # print("*************wordTable,scatter********************",wordTable.shape,wordTable.device)
            self.wordTable = wordTable
            self.wordTableFlag = False

        acl_embedding_output = self.acl_glm130b_word_embedding.execute([self.wordTable, input_ids])
        ...
        hidden_states = acl_embedding_output[0]
        ...
        if self.checkpoint_activations:
            ...
            if self.weight_flag:
                ...
                self.acl_glm130b_decoder_operation_v2.set_weight(self.acl_weights)
            ...
            if "mems" in kw_args:
                ...
                if mem is not None:                   
                    ...
                    acl_decoder_output_v2 = self.acl_glm130b_decoder_operation_v2.execute(acl_decoder_v2_input, json.dumps({}))
                    # print('acl decoder output[0][0,0,0] : ' + str(acl_decoder_output[0][0,0,0]))
                    # print('decoder takes (second) : ' + str(time.time() - start))

                    hidden_states = acl_decoder_output_v2[0]
                    for i in range(self.num_layers):
                        presentk = acl_decoder_output_v2[i + 1]
                        presentv = acl_decoder_output_v2[i + 1 + self.num_layers]
