...
class BaseTransformer(torch.nn.Module):
    def __init__(self,
        ...
        self.k_cache_input = torch.as_tensor(
            self.kv_cache[0], dtype=torch.half, device=self.kv_cache.device).view(self.num_layers, batch_num, self.max_sequence_length, self.hidden_size // self.rankSize)
        self.v_cache_input = torch.as_tensor(
            self.kv_cache[1], dtype=torch.half, device=self.kv_cache.device).view(self.num_layers, batch_num, self.max_sequence_length, self.hidden_size // self.rankSize)
        self.layerId_npu_list = [torch.tensor([i], device=self.kv_cache.device, dtype=torch.int32) for i in range(num_layers)]
        ...
        acl_param = json.dumps({
                            ...
                            "tokenOffset": [self.token_num]*batch_num, 
                            "seqLen": [1]*batch_num
                            })
    ...
    def forward(self, input_ids, position_ids, attention_mask, *,
        ...
        if self.checkpoint_activations:
            ...
            if self.weight_flag:
                ...
                acl_model_weights = list(self.state_dict().values())
                self.acl_weights.append(acl_model_weights[-3]) #final norm weight
            ...
            if self.decoder_flag:
                # if self.flash_acl_kv:
                #     self.flash_acl_kv = False
                #     mems = kw_args["mems"]
                #     for i in range(self.num_layers):
                #         layer_id = torch.tensor(i)
                #         # (batch, seqlen, 2*hidden_size // ranksize) => (batch, seqlen, hidden_size // ranksize)
                #         mem = mems[layer_id] if mems is not None else None
                #         if mem is not None:
                #             mem = mem.expand(batch_size, -1, -1).reshape(batch_size, mem.shape[1], 2, self.headNum // self.rankSize, self.head_size).permute(2, 0, 1, 3, 4)
                #             pastk, pastv = mem[0], mem[1]
                #             past_seqlen = pastk.shape[1]
                #             # only for batch=1
                #             self.kv_cache[0, i, 0, :past_seqlen, ...] = pastk
                #             self.kv_cache[1, i, 0, :past_seqlen, ...] = pastv
                ...
                acl_param = json.dumps({
                                    ...
                                    "rankSize": self.rankSize,
                                    "backend": "hccl",
                                    "residualAddScale": (2 * self.num_layers) ** 0.5,
            ...
            for i, layer in enumerate(self.layers):
                ...
                # if i == 0:
                #     torch.save(hidden_states, './' + str(self.rank) + '_' + str(seq_len) + '_' + str(i))
