...
class RMSNorm(torch.nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor):
        output_exec = self.rms_norm_operation.execute([hidden_states, self.weight])[0]
        input_dtype = hidden_states.dtype
        variance = hidden_states.to(torch.float32).pow(2).mean(-1, keepdim=True)
        hidden_states = hidden_states * torch.rsqrt(variance + self.eps)
        output = (self.weight * hidden_states).to(input_dtype)
        # torch.save(output.cpu(), "./tensors/torch/output.pth")
        # print(output)
        # print("=====================================================================================")
        # print(output_exec)
        # print(output_exec == output)
        assert torch.allclose(output, output_exec, rtol=0.001, atol=0.001)
        return output_exec
