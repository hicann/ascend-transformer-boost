...
class GLMBlock(torch.nn.Module):
    def __init__(
        ...
        self.acl_add_norm = torch.classes.OperationTorch.OperationTorch(
            "AddNormOperation")
        add_norm_param = {"layerNormEps": layernorm_epsilon,
                          "zoom_scale": self.alpha}
    ...
    def forward(
        ...
        # Residual connection.
        alpha_hidden_states = attention_input * self.alpha
        hidden_states = attention_input
        mlp_input = self.post_attention_layernorm(
            alpha_hidden_states + attention_output)
        acl_mlp_input = self.acl_add_norm.execute([attention_output,
                                                   hidden_states,
