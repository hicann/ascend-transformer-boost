...
class SelfAttention(torch.nn.Module):
    def __init__(self, hidden_size, num_attention_heads,
        ...
        self.acl_self_attention_kv_cache_operation.set_param(json.dumps({"transKey": True, "dk": hidden_size / num_attention_heads,
                                                                         "headNum": num_attention_heads, "layerId": layer_id}))
    ...
    def forward(
        ...
        if layer_past is not None:
            acl_result, acl_present_key, acl_present_value = self.acl_self_attention_kv_cache_operation.execute(
                [query_layer, key_layer, value_layer, attention_mask, layer_past[0], layer_past[1]])
        ...
        if layer_past is not None:
            if torch.allclose(acl_result, context_layer, rtol=0.02, atol=0.02) or \
                    torch.allclose(acl_present_key, present[0], rtol=0.02, atol=0.02) or \
                    torch.allclose(acl_present_value, present[1], rtol=0.02, atol=0.02):
                print("equal")
            else:
                print("not euqal")
                exit()
