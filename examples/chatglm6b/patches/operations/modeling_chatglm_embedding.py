...
class ChatGLMModel(ChatGLMPreTrainedModel):
    ...
    def forward(
        ...
        self.acl_embedding = torch.classes.OperationTorch.OperationTorch(
            "EmbeddingOperation")
        embedding_param = {"axis": 0}
        self.acl_embedding.set_param(json.dumps(embedding_param))

        if inputs_embeds is None:
            wordTable = list(self.state_dict().values())[0]
            inputs_embeds = self.word_embeddings(input_ids)
            acl_inputs_embeds = self.acl_embedding.execute([wordTable, input_ids])

        if torch.allclose(inputs_embeds, acl_inputs_embeds[0], rtol=0.02, atol=0.02):
            print("equal")
        else:
            print("not equal")
        exit()
