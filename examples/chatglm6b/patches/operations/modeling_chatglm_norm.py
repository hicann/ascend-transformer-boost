...
class GLMBlock(torch.nn.Module):
    def __init__(
        ...
        self.acl_norm_operation = torch.classes.OperationTorch.OperationTorch(
            "NormOperation")
        self.acl_norm_operation.set_param(
    ...
    def forward(
        ...
        acl_attention_inputs = self.acl_norm_operation.execute(
            [hidden_states, self.input_layernorm.weight, self.input_layernorm.bias])
        if torch.allclose(attention_input, acl_attention_inputs[0], rtol=0.02, atol=0.02):
            print("equal")
        else:
            print("not equal")
            exit()
