...
class ChatGLMModel(ChatGLMPreTrainedModel):
    ...
    def __init__(self, config: ChatGLMConfig):
        ...
        self.acl_decoder_operation = torch.classes.ChatGlm6BModelDecoderWithoutFusionTorch.ChatGlm6BModelDecoderWithoutFusionTorch()
    ...
    def forward(
        ...
        if not self.rotary_flag:
            ...
            temp_global = torch.arange(self.max_sequence_length, device='cpu').npu().half()
        ...
        if past_key_values[0] is None:
            ...
            acl_model_out = self.acl_decoder_operation.execute(
                hidden_states, position_ids, self.cos, self.sin, attention_mask, past_keys, past_values)
            presents = tuple(
                zip(acl_model_out[1:self.num_layers + 1], acl_model_out[self.num_layers + 1:]))
