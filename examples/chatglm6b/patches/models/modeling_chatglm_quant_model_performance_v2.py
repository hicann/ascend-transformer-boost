...
class ChatGLMModel(ChatGLMPreTrainedModel):
    ...
    def __init__(self, config: ChatGLMConfig):
        ...
        for layer_index in range(self.num_layers):
            query_key_value_name = "transformer.layers.{}.attention.query_key_value".format(layer_index)
            dense_name = "transformer.layers.{}.attention.dense".format(layer_index)
            dense_h_to_4h_name = "transformer.layers.{}.mlp.dense_h_to_4h".format(layer_index)
            dense_4h_to_h_name = "transformer.layers.{}.mlp.dense_4h_to_h".format(layer_index)
    ...
    def forward(
        ...
        if past_key_values[0] is None:
            acl_param = json.dumps({"transKey": True, "dk": self.hidden_size_per_attention_head, "headNum": self.num_attention_heads, "layerNum": self.num_layers,
                                "layerNormEps": self.layernorm_epsilon, "residualAddScale": math.sqrt(2 * self.num_layers)})
            self.acl_encoder_operation_inputs[0] = hidden_states
            self.acl_encoder_operation_inputs[1] = position_ids
            self.acl_encoder_operation_inputs[2] = self.cos
            self.acl_encoder_operation_inputs[3] = self.sin
            self.acl_encoder_operation_inputs[4] = attention_mask
            self.acl_encoder_operation_inputs[5] = seq_len
            acl_model_out = self.acl_encoder_operation.execute(
                self.acl_encoder_operation_inputs, acl_param)
            presents = tuple(
