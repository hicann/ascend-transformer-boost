...
class GLMBlock(torch.nn.Module):
    def __init__(
        ...
        self.qkv_weight_dequant = Parameter(quant_weight_dict[query_key_value_name].t().to(torch.int8).npu(), requires_grad=False)
        self.qkv_deq_scale_dequant = Parameter(weight_scale_dict[query_key_value_name].t().to(torch.float16).npu(), requires_grad=False)

        self.dense_weight_dequant = Parameter(quant_weight_dict[dense_name].t().to(torch.int8).npu(), requires_grad=False)
        self.dense_deq_scale_dequant = Parameter(weight_scale_dict[dense_name].t().to(torch.float16).npu(), requires_grad=False)

        self.hto4h_weight_dequant = Parameter(quant_weight_dict[dense_h_to_4h_name].t().to(torch.int8).npu(), requires_grad=False)
        self.hto4h_deq_scale_dequant = Parameter(weight_scale_dict[dense_h_to_4h_name].t().to(torch.float16).npu(), requires_grad=False)

        self.fhtoh_weight_dequant = Parameter(quant_weight_dict[dense_4h_to_h_name].t().to(torch.int8).npu(), requires_grad=False)
        self.fhtoh_deq_scale_dequant = Parameter(weight_scale_dict[dense_4h_to_h_name].t().to(torch.float16).npu(), requires_grad=False)
...
class ChatGLMModel(ChatGLMPreTrainedModel):
    ...
    def __init__(self, config: ChatGLMConfig):
        ...
        self.quant_weight_dict = np.load("/home/zcj/dequant_weight/0804/quant_weight.npy", allow_pickle=True).item()
        self.weight_scale_dict = np.load("/home/zcj/dequant_weight/0804/weight_scale.npy", allow_pickle=True).item()
        ...
        self.acl_decoder_operation = torch.classes.ModelTorch.ModelTorch(
            "ChatGlm6BDecoderDequantFlashAttentionModel")
        acl_param = json.dumps({"transKey": True, "dk": 128, "headNum": 32, "layerNum": self.num_layers,
                                "layerNormEps": self.layernorm_epsilon, "residualAddScale": math.sqrt(2 * self.num_layers),
                                "tokenOffset": [self.token_num], "seqLen": [1], "perm":[1,0]})
    ...
    def forward(
        ...
        if self.weightFlag is False:
            ...
            weights_dequant = [all_weights[0]]
            weights_encoder = [all_weights[0]]
            for i in range(self.num_layers):
                weights_t = list(self.layers[i].state_dict().values())
                del weights_t[10]
                weights_dequant.extend(weights_t)
                del weights_t[0:8]
                weights_encoder.extend(weights_t)
            weights_dequant.append(all_weights[-2])
            weights_dequant.append(all_weights[-1])
            weights_encoder.append(all_weights[-2])
            weights_encoder.append(all_weights[-1])

            self.acl_encoder_operation.set_weight(weights_encoder[1:-2])
            self.acl_decoder_operation.set_weight(weights_dequant)
