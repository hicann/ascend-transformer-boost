...
class ChatGLMForConditionalGeneration(ChatGLMPreTrainedModel):
    ...
    @torch.no_grad()
    def stream_chat_multi_batch(self, tokenizer, query: List, history: List[List[Tuple[str, str]]] = None, max_length: int = 2048,
                        do_sample=True, top_p=0.7, temperature=0.95, logits_processor=None, **kwargs):
        if history is None:
            history = [[]]
        if logits_processor is None:
            logits_processor = LogitsProcessorList()
        logits_processor.append(InvalidScoreLogitsProcessor())
        gen_kwargs = {"max_length": 128, "do_sample": do_sample, "top_p": top_p,
                      "temperature": temperature, "logits_processor": logits_processor, **kwargs}

        tmp_list = []
        if (len(history) < len(query)):
            for _ in range(len(query) - len(history)):
                tmp_list.append([])
            history += tmp_list

        response_list, new_history_list = [], []
        for query_item, history_item in zip(query, history):
            if not history_item:
                prompt = query_item
            else:
                prompt = ""
                for i, (old_query, response) in enumerate(history_item):
                    prompt += "用户：{}\n机器人：{}\n".format(old_query, response)
                prompt += "用户：{}\n机器人：".format(query_item)
            input_ids = tokenizer([prompt], return_tensors="pt", padding=True)
            input_ids = input_ids.to(self.device)

            outputs = self.stream_iter(**input_ids, **gen_kwargs)
            outputs = list(outputs)
            outputs = outputs[-1][0][len(input_ids["input_ids"][0]):]
            response = tokenizer.decode(outputs)
            response_list.append(response)
            new_history = history_item + [(query_item, response)]
            new_history_list.append(new_history)

        return response_list, new_history_list

    @torch.no_grad()
    def stream_chat_iter(self, tokenizer, query, history, max_length: int = 2048,
                        do_sample=True, top_p=0.7, temperature=0.95, logits_processor=None, **kwargs):
        if logits_processor is None:
            logits_processor = LogitsProcessorList()
        logits_processor.append(InvalidScoreLogitsProcessor())
        ...
        if isinstance(query, list):
            print("Processing multi-batch inputs.")
            response_list, new_history_list = self.stream_chat_multi_batch(tokenizer, query, history=history)
            yield response_list, new_history_list
            ...
            outputs = list(outputs)[0][0]
            response = tokenizer.decode(outputs)
            new_history = history + [(query, response)]
            yield response, new_history
        else:
            print("Input type might be incorrect.")

    @torch.no_grad()
    def stream_pre_processing(
            self,
            input_ids,
        ...
        return model_kwargs, generation_config, logits_processor, stopping_criteria, logits_warper, unfinished_sequences, scores

    @torch.no_grad()
    def stream_infer(self,**model_inputs):
        return self(
            **model_inputs,
            return_dict=True,
    ...
    def stream_post_processing(
        ...
        next_token_logits = infer_outputs.logits[:, -1, :]
        ...
        model_kwargs = self._update_model_kwargs_for_generation(
            infer_outputs, model_kwargs, is_encoder_decoder=self.config.is_encoder_decoder
        )
        ...
        return input_ids, unfinished_sequences
    ...
    def stream_iter(
        ...
        model_kwargs, generation_config, logits_processor, stopping_criteria, logits_warper, unfinished_sequences, scores = self.stream_pre_processing(input_ids)
        ...
        while True:
            ...
            outputs = self.stream_infer(**model_inputs)

            torch.npu.synchronize()

            input_ids, unfinished_sequences = self.stream_post_processing(input_ids, outputs, unfinished_sequences, model_kwargs, generation_config, logits_processor, logits_warper)
