...
class GLMBlock(torch.nn.Module):
    def __init__(
        ...
        query_key_value_name = "transformer.layers.{}.attention.query_key_value".format(
            layer_id)
        dense_name = "transformer.layers.{}.attention.dense".format(layer_id)
        dense_h_to_4h_name = "transformer.layers.{}.mlp.dense_h_to_4h".format(
            layer_id)
        dense_4h_to_h_name = "transformer.layers.{}.mlp.dense_4h_to_h".format(
            layer_id)
        ...
        self.qkv_weight = Parameter(quant_weight_dict[query_key_value_name].to(
            torch.int8).npu(), requires_grad=False)
        self.qkv_deq_scale = Parameter(padding_descale(
            deq_scale_dict[query_key_value_name]).to(torch.float32).npu(), requires_grad=False)
        self.qkv_bias = Parameter(quant_bias_dict[query_key_value_name].to(
            torch.int32).npu(), requires_grad=False)

        self.dense_weight = Parameter(quant_weight_dict[dense_name].to(
            torch.int8).npu(), requires_grad=False)
        self.dense_deq_scale = Parameter(padding_descale(
            deq_scale_dict[dense_name]).to(torch.float32).npu(), requires_grad=False)
        self.dense_bais = Parameter(quant_bias_dict[dense_name].to(
            torch.int32).npu(), requires_grad=False)

        self.hto4h_weight = Parameter(quant_weight_dict[dense_h_to_4h_name].to(
            torch.int8).npu(), requires_grad=False)
        self.hto4h_deq_scale = Parameter(padding_descale(
            deq_scale_dict[dense_h_to_4h_name]).to(torch.float32).npu(), requires_grad=False)
        self.hto4h_bais = Parameter(quant_bias_dict[dense_h_to_4h_name].to(
            torch.int32).npu(), requires_grad=False)

        self.fhtoh_weight = Parameter(quant_weight_dict[dense_4h_to_h_name].to(
            torch.int8).npu(), requires_grad=False)
        self.fhtoh_deq_scale = Parameter(padding_descale(
            deq_scale_dict[dense_4h_to_h_name]).to(torch.float32).npu(), requires_grad=False)
        self.fhtoh_bais = Parameter(quant_bias_dict[dense_4h_to_h_name].to(
            torch.int32).npu(), requires_grad=False)
...
class ChatGLMModel(ChatGLMPreTrainedModel):
    ...
    def __init__(self, config: ChatGLMConfig):
        ...
        import numpy as np
        self.input_scale_dict = np.load(
            "./chatglm_quant_param/input_scale.npy", allow_pickle=True).item()
        self.input_offset_dict = np.load(
            "./chatglm_quant_param/input_offset.npy", allow_pickle=True).item()
        self.quant_weight_dict = np.load(
            "./chatglm_quant_param/quant_weight.npy", allow_pickle=True).item()
        self.weight_scale_dict = np.load(
            "./chatglm_quant_param/weight_scale.npy", allow_pickle=True).item()
        self.deq_scale_dict = np.load(
            "./chatglm_quant_param/deq_scale.npy", allow_pickle=True).item()
        self.quant_bias_dict = np.load(
            "./chatglm_quant_param/quant_bias.npy", allow_pickle=True).item()
        ...
        for layer_index in range(self.num_layers):
            query_key_value_name = "transformer.layers.{}.attention.query_key_value".format(
                layer_index)
            dense_name = "transformer.layers.{}.attention.dense".format(
                layer_index)
            dense_h_to_4h_name = "transformer.layers.{}.mlp.dense_h_to_4h".format(
                layer_index)
            dense_4h_to_h_name = "transformer.layers.{}.mlp.dense_4h_to_h".format(
                layer_index)
            self.qkv_input_scale.append(
                float(1/self.input_scale_dict[query_key_value_name]))
            self.qkv_input_offset.append(
                int(self.input_offset_dict[query_key_value_name]))
            self.dense_input_scale.append(
                float(1/self.input_scale_dict[dense_name]))
            self.dense_input_offset.append(
                int(self.input_offset_dict[dense_name]))
            self.self_ln_input_scale.append(
                float(1/self.input_scale_dict[dense_h_to_4h_name]))
            self.self_ln_input_offset.append(
                int(self.input_offset_dict[dense_h_to_4h_name]))
            self.ffn_out_input_scale.append(
                float(1/self.input_scale_dict[dense_4h_to_h_name]))
            self.ffn_out_input_offset.append(
                int(self.input_offset_dict[dense_4h_to_h_name]))
        ...
        self.acl_decoder_operation = torch.classes.ChatGlm6BModelDecoderQuantFlashTorch.ChatGlm6BModelDecoderQuantFlashTorch()
    ...
    def forward(
        ...
        if full_flag:
            ...
            acl_model_out = self.acl_decoder_operation.execute(
                hidden_states, position_ids, self.cos, self.sin, self.attention_mask_max,
