...
class GLMBlock(torch.nn.Module):
    def __init__(
        ...
        if layer_id == 0:
            self.acl_encoder_quant_layer = torch.classes.OperationTorch.OperationTorch(
                "ChatGlm6BLayerEncoderFirstQuantOperation")
            self.acl_encoder_quant_layer.set_name(
                "ChatGlm6BLayerEncoderFirstQuantOperation_" + str(self.layer_id))
            self.acl_encoder_quant_layer.set_param(acl_param)

            self.acl_decoder_quant_layer = torch.classes.OperationTorch.OperationTorch(
                "ChatGlm6BLayerDecoderFirstQuantOperation")
            self.acl_decoder_quant_layer.set_name(
                "ChatGlm6BLayerDecoderFirstQuantOperation_" + str(self.layer_id))
            self.acl_decoder_quant_layer.set_param(acl_param)

        elif layer_id == num_layers - 1:
            self.acl_encoder_quant_layer = torch.classes.OperationTorch.OperationTorch(
                "ChatGlm6BLayerEncoderLastQuantOperation")
            self.acl_encoder_quant_layer.set_name(
                "ChatGlm6BLayerEncoderLastQuantOperation_" + str(self.layer_id))
            self.acl_encoder_quant_layer.set_param(acl_param)

            self.acl_decoder_quant_layer = torch.classes.OperationTorch.OperationTorch(
                "ChatGlm6BLayerDecoderLastQuantOperation")
            self.acl_decoder_quant_layer.set_name(
                "ChatGlm6BLayerDecoderLastQuantOperation_" + str(self.layer_id))
            self.acl_decoder_quant_layer.set_param(acl_param)

        else:
            self.acl_encoder_quant_layer = torch.classes.OperationTorch.OperationTorch(
                "ChatGlm6BLayerEncoderQuantOperation")
            self.acl_encoder_quant_layer.set_name(
                "ChatGlm6BLayerEncoderQuantOperation_" + str(self.layer_id))
            self.acl_encoder_quant_layer.set_param(acl_param)

            self.acl_decoder_quant_layer = torch.classes.OperationTorch.OperationTorch(
                "ChatGlm6BLayerDecoderQuantOperation")
            self.acl_decoder_quant_layer.set_name(
                "ChatGlm6BLayerDecoderQuantOperation_" + str(self.layer_id))
            self.acl_decoder_quant_layer.set_param(acl_param)
    ...
    def forward(
        ...
        if layer_past is None:
            if not self.input_full_flag:
                ...
                self.input_full.append(res_out)
            else:
                self.input_full[0] = hidden_states
                self.input_full[17] = position_ids
                self.input_full[20] = attention_mask
                self.input_full[21] = seq_len
                self.input_full[22] = res_out
            
            if layer_id == 27:
                test_glmBlockOut, test_presentKey, test_presentValue = self.acl_encoder_quant_layer.execute(self.input_full)
                outputs = (test_glmBlockOut, None, (test_presentKey, test_presentValue))
            else:
                acl_out = self.acl_encoder_quant_layer.execute(self.input_full)
                test_glmBlockOut, test_presentKey, test_presentValue, res_in = acl_out
                outputs = (test_glmBlockOut, res_in, (test_presentKey, test_presentValue))
            ...
            if not self.input_flag:
                ...
                self.input[23] = seq_len
                self.input[24] = res_out
            if layer_id == 27:
                test_glmBlockOut, test_presentKey, test_presentValue = self.acl_decoder_quant_layer.execute(self.input)
                outputs = (test_glmBlockOut, None, (test_presentKey, test_presentValue))
            else:
                test_glmBlockOut, test_presentKey, test_presentValue, res_in = self.acl_decoder_quant_layer.execute(self.input)
                outputs = (test_glmBlockOut, res_in, (test_presentKey, test_presentValue))
...
class ChatGLMModel(ChatGLMPreTrainedModel):
    ...
    def __init__(self, config: ChatGLMConfig):
        ...
        import numpy as np
        # dict{tensor},key module name, value tensor
        ...
        self.zero_tensor = torch.zeros([1,1,4096]).to(torch.float16).npu()
...
class ChatGLMForConditionalGeneration(ChatGLMPreTrainedModel):
    ...
    def stream_generate(
        ...
        while True:
            ...
            stream.synchronize()
            prof = torch.npu.profile('./profiling/')
            prof.__enter__()
