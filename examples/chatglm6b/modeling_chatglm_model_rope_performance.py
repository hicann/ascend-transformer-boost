    ...
    def forward(
        ...
        if past_key_values[0] is None:
            acl_model_out = self.acl_encoder_operation.execute(
                hidden_states, position_ids, cosTable, sinTable, attention_mask, seq_len)
            presents = tuple(
            ...
            acl_model_out = self.acl_decoder_operation.execute(
                hidden_states, position_ids, cosTable, sinTable, attention_mask, past_keys, past_values, seq_len)
            presents = tuple(
                zip(acl_model_out[1:self.num_layers + 1], acl_model_out[self.num_layers + 1:]))
