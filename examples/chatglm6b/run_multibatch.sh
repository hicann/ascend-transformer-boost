...
# 之后开始测试，输入测试用例（五条直线相交，最多能有多少个交点？ 同一平面内，五条直线相交，最多能有多少个交点？）后等待测试结果即可（注意两个问题用空格分开）
...
RUN_OPTION_LIST="--run --performance --webdemo --zhipu --profiling --run-iter"
RUN_OPTION="--run"

cp -rf $SCRIPT_DIR/patches/models/modeling_chatglm_iter_multibatch.py $SCRIPT_DIR
SCRIPT_PATH=$SCRIPT_DIR/modeling_chatglm_iter_multibatch.py
...
function fn_main()
{
    ...
    case "${RUN_OPTION}" in
        "--run")
            ...
            ;;
	"--run-iter")
	    python3 $SCRIPT_DIR/main_iter.py
	    ;;
        "--performance")
        ...
        "--profiling")
            ...
            ;;
        "--help")
            echo "run.sh [--run|--performance|--webdemo|--zhipu|--profiling|--run-iter] [model script path]"
            ;;
        *)
