 ...
 */
#include "chatglm6b_fusion_layerv2.h"
...
ChatGlm6BFusionLayerV2::ChatGlm6BFusionLayerV2(const nlohmann::json &paramJson) : Layer("ChatGlm6BFusionLayerV2", paramJson)
{
    BuildGraph();
    BuildPlan();
}

ChatGlm6BFusionLayerV2::~ChatGlm6BFusionLayerV2() {}

AsdOps::Status ChatGlm6BFusionLayerV2::InferShape(const AsdOps::SVector<AsdOps::Tensor> &inTensors,
...
void ChatGlm6BFusionLayerV2::BuildGraph()
{ // in
    ...
    ASD_LOG(FATAL) << "ChatGlm6BFusionLayerV2 setup graph";
    ...
    for (auto item : paramJson_["tokenOffset"]) {
        selfAttentionKvCacheParam.tokenOffset.push_back(item.get<int>());
        ASD_LOG(FATAL) << "token offset:" << selfAttentionKvCacheParam.tokenOffset.at(0);

    }
    for (auto item : paramJson_["seqLen"]) {
        selfAttentionKvCacheParam.seqLen.push_back(item.get<int>());
        ASD_LOG(FATAL) << "seqLen:" << selfAttentionKvCacheParam.seqLen.at(0);
    }
