...
class Model(PreTrainedModel):
    ...
    def forward(
        ...
        if self.weight_flag is False:
            ...
            self.acl_encoder_operation.set_weigth(weights)
        ...
        # do encoder
        is_prefill = True if past_key_values is None
        if is_prefill:
            ...
            acl_model_out = self.acl_encoder_operation.execute(self.acl_encoder_operation_inputs, self.acl_param)
            ...
            acl_model_out = self.acl_decoder_operation.execute(self.acl_decoder_operation_inputs, self.acl_param)
            acl_presents = tuple(zip(acl_model_out[1:self.num_layers+1], acl_model_out[self.num_layers+1:]))
            acl_hidden_state = acl_model_out[0]

        hidden_states = acl_hidden_state
        next_decoder_cache = acl_presents
