    ...
    def forward(
        ...
        if past_key_value is not None:
            if len(self.acl_weights) == 0:
                ...
                print(f"weights_pack is {weights_pack}, shape is {weights_pack.shape}")
                weight_q, weight_k, weight_v = torch.split(weights_pack, self.hidden_size, dim=0)
                print(f"weight_q is {weight_q}, shape is {weight_q.shape}")
                print(f"weight_k is {weight_k}, shape is {weight_k.shape}")
                print(f"weight_v is {weight_v}, shape is {weight_v.shape}")
