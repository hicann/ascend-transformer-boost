...
tokenizer_path = '/data/models/llama-13b-part_model_test'+'/tokenizer'
...
part_model_path = '/data/models/llama-13b-part_model_test/part_model/' + \
    str(local_rank)+'/'
model = LlamaForCausalLM.from_pretrained(
    part_model_path, torch_dtype=torch.float16).npu()
...
inputs_warm_up = tokenizer(
    test_prompt, return_tensors="pt", max_length=SEQ_LEN_IN, truncation=True)
...
print(
    f"Output generated in {(end_time-start_time):.2f} s ({new_tokens/(end_time-start_time):.2f} tokens/s, {new_tokens} tokens)")
