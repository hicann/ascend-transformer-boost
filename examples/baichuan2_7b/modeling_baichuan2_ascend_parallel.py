...
class BaichuanModel(BaichuanPreTrainedModel):
    ...
    def init_ascend_operations(self, config: BaichuanConfig):
        ...
        self.acl_encoder_operation = torch.classes.ModelTorch.ModelTorch("BaiChuan27BEncoderParallelModel")
        self.acl_decoder_operation = torch.classes.ModelTorch.ModelTorch("BaiChuan27BDecoderParallelModel")
    ...
    def prepare_inputs_for_ascend(self, input_ids, position_ids, seq_length, batch_size, attention_mask=None,
                                  past_key_values=None):
        cos_table, sin_table = self.ascend_rotary_embedding(input_ids, seq_length)
        ...
        if not past_key_values:
            ...
            seq_len_tensor = torch.tensor([seq_length] * batch_size, dtype=torch.int32, device=input_ids.device)
            ...
            inputs = [input_ids, position_ids, cos_embed, sin_embed, attention_mask] + past_keys + past_values
        return inputs

    def execute_ascend_operator(self, acl_model, input_ids, position_ids, seq_length_with_past, attention_mask=None,
                                past_key_values=None):
        batch_size, seq_length = input_ids.shape
        acl_inputs = self.prepare_inputs_for_ascend(input_ids, position_ids, seq_length_with_past,
                                                    batch_size,
                                                    attention_mask, past_key_values)
        acl_model_out = acl_model.execute(acl_inputs, self.acl_param)
    ...
    def forward(
        ...
        if past_key_values is not None:  # [bs, sq, hn, hs]
        ...
        hidden_states, next_decoder_cache = self.execute_ascend_operator(operation,
                                                                         input_ids,
                                                                         position_ids,
