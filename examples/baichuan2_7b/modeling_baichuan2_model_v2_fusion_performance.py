...
class LlamaModel(PreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.rmsNormEps = config.rms_norm_eps
        ...
        self.k_cache_input.data = self.k_cache_input.data.npu_format_cast(29)

        self.v_cache_input = torch.as_tensor(self.kv_cache[1], dtype=torch.half, device=self.kv_cache.device).view(
            self.num_layers, self.batch_num, self.max_seq_len, self.hidden_size
        )
        self.v_cache_input.data = self.v_cache_input.data.npu_format_cast(29)
        ...
        self.acl_model_operation = torch.classes.ModelTorch.ModelTorch(
            "BaiChuan27BFusionModel")
        acl_param_fa = json.dumps({"rmsNormEps": self.rmsNormEps,
                                   "headNum": self.head_num,
                                   "dk": self.head_size,
                                   "layerNum": self.num_layers,
                                   "tokenOffset": [self.token_num] * self.batch_num,
                                   "seqLen": [1] * self.batch_num})
        self.acl_model_operation.set_param(acl_param_fa)


        self.acl_model_operation_inputs = [None] * (9 + self.num_layers)
        for i in range(self.num_layers):
            self.acl_model_operation_inputs[9 + i] = torch.tensor([i], dtype=torch.int32).npu()
    ...
    def forward(
        ...
        print("===past key_values", past_key_values_length, "seq_length_with past", seq_length_with_past)
        ...
        if self.weight_flag is False:
            ...
            self.acl_model_operation.set_weight(weights)
        ...
        self.acl_model_operation_inputs[0] = input_ids
        self.acl_model_operation_inputs[1] = position_ids
        self.acl_model_operation_inputs[2] = cosTable
        self.acl_model_operation_inputs[3] = sinTable
        self.acl_model_operation_inputs[4] = maskAttenCache
        self.acl_model_operation_inputs[5] = self.k_cache_input
        self.acl_model_operation_inputs[6] = self.v_cache_input
        self.acl_model_operation_inputs[7] = self.token_offset
        self.acl_model_operation_inputs[8] = self.seq_len_tensor

        acl_param_here = json.dumps({"rmsNormEps": self.rmsNormEps,
                                   "headNum": self.head_num,
                                   "dk": self.head_size,
                                   "layerNum": self.num_layers,
                                   "tokenOffset": [self.token_num] * self.batch_num,
                                   "seqLen": [seq_length] * self.batch_num})

        acl_model_out = self.acl_model_operation.execute(self.acl_model_operation_inputs, acl_param_here)
