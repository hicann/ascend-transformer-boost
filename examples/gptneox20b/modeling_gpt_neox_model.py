...
class GPTNeoXAttention(nn.Module):
    ...
    def forward(
        ...
        query_rot = query[..., : self.rotary_ndims]  # [bs, hn, sq, rd]
    ...
    def _attn_fp16(self, query, key, value, attention_mask=None, head_mask=None):
        scaling_factor = 0.1
        test_query = query.half()
        test_key = key.half()
        ...
        mask_value_tmp = torch.zeros(size=(batch_size, num_attention_heads, query_length, key_length)).npu()
        mask_value = torch.finfo(torch.float32).min
        attn_mask_tmp = torch.masked_fill(mask_value_tmp, ~causal_mask, mask_value)
        attn_mask = attn_mask_tmp + attention_mask

        test_query = test_query.view(batch_size * num_attention_heads, query_length, attn_head_size)
        test_key = test_key.view(batch_size * num_attention_heads, key_length, attn_head_size)

        scaled_test_query = self.alpha * test_query * scaling_factor
        scaled_test_key = test_key * scaling_factor
        test_attn_scores = torch.bmm(scaled_test_query, scaled_test_key.transpose(1, 2))

        test_attn_scores = test_attn_scores.view(batch_size, num_attention_heads, query_length, key_length)

        test_attn_scores = test_attn_scores.float()
        test_attn_scores = test_attn_scores * (1/ (scaling_factor * scaling_factor))
        test_attn_scores += attn_mask

        test_attn_weights = nn.functional.softmax(test_attn_scores, dim=-1)

        test_attn_weights = test_attn_weights.half()

        attn_out = torch.matmul(test_attn_weights, value)
        return attn_out, test_attn_weights
...
class GPTNeoXModel(GPTNeoXPreTrainedModel):
    def __init__(self, config):
        ...
        self.acl_decoder_operation = torch.classes.GptNeox20BModelDecoderTorch.GptNeox20BModelDecoderTorch()
    ...
    def forward(
        ...
        if past_key_values[0] is None:
            ...
            acl_model_out = self.acl_encoder_operation.execute(hidden_states, position_ids, cosTable, sinTable,
                                                               attention_mask_tmp, seq_len_tensor)
            ...
            acl_model_out = self.acl_decoder_operation.execute(hidden_states, position_ids, cosTable, sinTable,
                                                               attention_mask_tmp, seq_len_tensor, past_keys, past_values)
            acl_presents = tuple(zip(acl_model_out[1:self.num_layers+1], acl_model_out[self.num_layers+1:]))
        ...
        for i in range(len(presents)):
            ...
            if np.allclose(acl_key.cpu(), test_key.cpu(), rtol=0.02, atol=0.02):
                ...
                print("!!!!not equal key of", i, "acl", acl_key, "output", test_key)
            if np.allclose(acl_value.cpu(), test_value.cpu(), rtol=0.02, atol=0.02):
                print("****equal value of ", i)
            else:
                print("!!!!not equal value of", i, "acl", acl_value, "output", test_value)
