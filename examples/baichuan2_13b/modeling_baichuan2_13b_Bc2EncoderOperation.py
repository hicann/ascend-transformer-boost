...
class BaichuanLayer(torch.nn.Module):
    def __init__(self, config: BaichuanConfig):
        ...
        self.acl_encoder_layer = torch.classes.OperationTorch.OperationTorch(
            "BaiChuan213BLayerEncoderOperation")
        ...
        self.acl_encoder_layer.set_param(param)

        self.acl_weights = []

        self.forward = self.forward_cmp
        self.equal_count = 0

    def init_acl_encoder_param(self,
                               hidden_states,
                               attention_mask=None,
                               past_key_value=None):
    ...
    def execute_encoder_acl(self, hidden_states: torch.Tensor,
        ...
        :return:
        """
        acl_input = self.init_acl_encoder_param(hidden_states, attention_mask, past_key_value)
        # acl返回的  presentkey value[bs, sq, hn, hs]
        hidden_states, test_presentKey, test_presentValue = self.acl_encoder_layer.execute(acl_input)
        test_presentKey = test_presentKey.permute(0, 2, 1, 3)  #
        test_presentValue = test_presentValue.permute(0, 2, 1, 3)
    ...
    def forward_cmp(
        ...
        if past_key_value is None:
            acl_out, acl_key_value = self.execute_encoder_acl(
    ...
    def forward(
        ...
        if past_key_value is None:
            hidden_states, present_key_value = self.execute_encoder_acl(
                hidden_states, attention_mask, past_key_value, use_cache)
        else:
