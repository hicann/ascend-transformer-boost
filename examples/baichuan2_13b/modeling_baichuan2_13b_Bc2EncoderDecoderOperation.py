...
class BaichuanLayer(torch.nn.Module):
    def __init__(self, config: BaichuanConfig):
        ...
        self.acl_encoder_layer = torch.classes.OperationTorch.OperationTorch("BaiChuan213BLayerEncoderOperation")
    ...
    def execute_encoder_acl(self, hidden_states: torch.Tensor,
        ...
        hidden_states, test_present_key, test_present_value = self.acl_encoder_layer.execute(acl_input)
...
class BaichuanModel(BaichuanPreTrainedModel):
    ...
    def forward(
        ...
        if past_key_values is not None:
            ...
            # print(seq_length_with_past)
            # past_key_values_length = past_key_values[0][0].shape[2]
            # 由于我们返回的kv cache 的shape 是 [batch_size,seq_len,nh,ns],同时接encoder和decoder时修改下顺序
