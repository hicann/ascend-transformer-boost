...
position_ids_cache = torch.arange(2048, device='cpu').long().unsqueeze(0).npu()
...
class LlamaRotaryEmbedding(torch.nn.Module):
    def __init__(self, dim, max_position_embeddings=2048, base=10000, device=None):
        ...
        inv_freq = 1.0 / (torch.tensor(base).double() **
                          (torch.arange(0, dim, 2).float().to(device) / dim).double())
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        #     torch.save(query_states.cpu(), 'save/1_linear_out0.pth')
        # else:
        #     torch.save(query_states.cpu(), 'save/full_1_linear_out0.pth')
        query_states = query_states.view(
            bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2)

        key_states = self.k_proj(hidden_states)
        # if past_key_value is not None:
        #     torch.save(key_states.cpu(), 'save/2_linear_out0.pth')
        # else:
        #     torch.save(key_states.cpu(), 'save/full_2_linear_out0.pth')
        key_states = key_states.view(
            bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2)

        value_states = self.v_proj(hidden_states)
        # if past_key_value is not None:
        #     torch.save(value_states.cpu(), 'save/3_linear_out0.pth')
        # else:
        #     torch.save(value_states.cpu(), 'save/full_3_linear_out0.pth')
        ...
        # if past_key_value is not None:
        #     torch.save(query_states.cpu(), 'save/4_rope_out0.pth')
        #     torch.save(key_states.cpu(), 'save/4_rope_out1.pth')
        # else:
        #     torch.save(query_states.cpu(), 'save/full_4_rope_out0.pth')
        #     torch.save(key_states.cpu(), 'save/full_4_rope_out1.pth')
        # [bsz, nh, t, hd]

        if past_key_value is not None:
            # reuse k, v, self_attention    [1,32,129,128]
        ...
        # if past_key_value is not None:
        #     torch.save(attn_output.cpu(), 'save/5_fa_out0.pth')
        # else:
        #     torch.save(attn_output.cpu(), 'save/full_5_fa_out0.pth')

        attn_output = attn_output.transpose(1, 2)
        attn_output = attn_output.reshape(bsz, q_len, self.hidden_size)

        attn_output = self.o_proj(attn_output)  # [11008,4096]
        # if past_key_value is not None:
        #     torch.save(attn_output.cpu(), 'save/6_linear_out0.pth')
        # else:
        #     torch.save(attn_output.cpu(), 'save/full_6_linear_out0.pth')
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # if past_key_value is not None:
        #     torch.save(hidden_states.cpu(), 'save/0_rms_in0.pth')
        # else:
        #     torch.save(hidden_states.cpu(), 'save/full_0_rms_in0.pth')
        hidden_states = self.input_layernorm(hidden_states)
        # if past_key_value is not None:
        #     torch.save(hidden_states.cpu(), 'save/0_rms_out0.pth')
        # else:
        #     torch.save(hidden_states.cpu(), 'save/full_0_rms_out0.pth')
        ...
        # if past_key_value is not None:
        #     torch.save(hidden_states.cpu(), 'save/7_add_out0.pth')
        # else:
        #     torch.save(hidden_states.cpu(), 'save/full_7_add_out0.pth')

        # Fully Connected
        residual = hidden_states
        hidden_states = self.post_attention_layernorm(hidden_states)
        # if past_key_value is not None:
        #     torch.save(hidden_states.cpu(), 'save/8_rms_out0.pth')
        # else:
        #     torch.save(hidden_states.cpu(), 'save/full_8_rms_out0.pth')
        hidden_states = self.mlp(hidden_states)
        # if past_key_value is not None:
        #     torch.save(hidden_states.cpu(), 'save/9_mlp_out0.pth')
        # else:
        #     torch.save(hidden_states.cpu(), 'save/full_9_mlp_out0.pth')
        hidden_states = residual + hidden_states
        # if past_key_value is not None:
        #     torch.save(hidden_states.cpu(), 'save/10_add_out0.pth')
        # else:
        #     torch.save(hidden_states.cpu(), 'save/full_10_add_out0.pth')
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.token_num = 0
        self.token_offset = torch.full((self.batch_num,), 0, dtype=torch.int32).npu()
        self.acl_operation_inputs = [None] * (9 + self.num_layers)

        for i in range(self.num_layers):
            self.acl_operation_inputs[9 + i] = torch.tensor([i], dtype=torch.int32).npu()

        self.acl_operation = torch.classes.ModelTorch.ModelTorch("Llama13BFusionModel")
        acl_param = json.dumps({"headNum": self.num_heads, "rmsNormEps": self.rms_norm_eps,
                                "dk": self.headSize, "tokenOffset": [self.token_num] * self.batch_num,
                                "seqLen": [1] * self.batch_num, "layerNum": self.num_layers,
                                "rotaryCoeff": 2, "model": "llama13b"})
        
        self.acl_operation.set_param(acl_param)
        self.seq_len = torch.full((self.batch_num,), 0, dtype=torch.int32).npu()
    ...
    def forward(
        ...
        global maskAttenCache_incre
        global biasCache
        global mask_value
        if batch_size != self.batch_num or self.kv_cache is None or past_key_values is None:
            ...
            self.kv_cache = torch.zeros(2,
                                        ...
                                        self.num_heads,
                                        self.headSize,
                                        device="cpu").half().npu().contiguous()
        ...
        if past_key_values is not None:
            past_key_values_length = self.token_offset[0]
        ...
        if past_key_values is None:
            ...
            attention_mask = self._prepare_decoder_attention_mask(
                attention_mask, (batch_size,
                                seq_length), self.kv_cache, past_key_values_length
            ) 
        ...
        if self.weightFlag is False:
            ...
            self.weights_a.append(all_weights[0])
            for i in range(self.num_layers):
                weights = list(self.layers[i].state_dict().values())
                weights_p.append(weights[8].npu())
                qkv_weight = torch.concat([weights[0].npu(), weights[1].npu(), weights[2].npu()], dim=0)
                weights_p.append(qkv_weight.npu())
                weights_p.append(weights[3].npu())
                weights_p.append(weights[9].npu())
                weights_p.append(weights[5].npu())
                weights_p.append(weights[6].npu())
                weights_p.append(weights[7].npu())
                self.weights_a.extend(weights_p)
                weights_p.clear()
            self.weights_a.append(self.state_dict()["norm.weight"])
            self.weights_a.append(lm_head_weight)
            self.acl_operation.set_weight(self.weights_a)
        ...
        global sinTable
        acl_param = json.dumps({"headNum": self.num_heads, "rmsNormEps": self.rms_norm_eps,
                                "dk": self.headSize, "layerNum": self.num_layers,
                                "tokenOffset": [int(self.token_offset[0])] * self.batch_num,
                                "seqLen": [int(self.seq_len[0])] * self.batch_num, 
                                "rotaryCoeff": 2, "model": "llama13b"})
        self.acl_operation_inputs[0] = input_ids
        self.acl_operation_inputs[1] = position_ids
        self.acl_operation_inputs[2] = cosTable.npu_format_cast(2)
        self.acl_operation_inputs[3] = sinTable.npu_format_cast(2)
        if past_key_values is None:
            for i in range(self.batch_num):
                maskAttenCache[i][:seq_length, :seq_length] = attention_mask[i]
                count = torch.eq(attention_mask[i][0][seq_length - 1], 0).sum().item()
                decoder_mask = torch.full((2048, seq_length - count), -65504, dtype=torch.half).npu()
                maskAttenCache_incre[i][:, count:seq_length] = decoder_mask
            self.acl_operation_inputs[4] = maskAttenCache
        else:
            self.acl_operation_inputs[4] = maskAttenCache_incre
        ...
        acl_model_out = self.acl_operation.execute(self.acl_operation_inputs, acl_param)
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def prepare_inputs_for_generation(
        ...
        global position_ids_cache

        input_shape = input_ids.shape
        if input_shape[0] != position_ids_cache.shape[0]:
            position_ids_cache = position_ids_cache.repeat(input_shape[0], 1)

        position_ids = kwargs.get("position_ids", None)
        if position_ids is None:
            position_ids = position_ids_cache[:, :input_shape[1]]
