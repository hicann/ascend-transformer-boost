...
class LlamaDecoderLayer(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_id: int):
        ...
        param = json.dumps({"headNum": config.num_attention_heads, "rmsNormEps": config.rms_norm_eps,
                            "dk": self.headSize, "tokenOffset": [self.token_num], "seqLen": [1], "layerId": self.layerId, "model": "llama7b", "rotaryCoeff": 2})
        # print(param)
        ...
        self.token_offset = torch.full((1,), 0, dtype=torch.int32, device=self.kv_cache.device) 
    ...
    def forward(
        ...
        if past_key_value is not None:
            ...
            acl_param = json.dumps({"headNum": self.num_heads, "rmsNormEps": self.rms_norm_eps,
                            "dk": self.headSize, "tokenOffset": [token_num], "seqLen": [1], "layerId": self.layerId, "model": "llama7b"})
            # self.acl_llama_layer.set_param(acl_param)
            #NEW
            print(acl_param)
            test_llama7bOut = self.acl_llama_layer.execute_with_param(inputs, acl_param)
            #OLD
        ...
        if past_key_value is None:  # [1,32,128,128]
            self.kv_cache[0, self.layerId, 0, :hidden_states.shape[1], ...] = present_key_value[0].permute(0, 2, 1, 3)  # batch = 1
            self.kv_cache[1, self.layerId, 0, :hidden_states.shape[1], ...] = present_key_value[1].permute(0, 2, 1, 3)  # batch = 1
        # if test_llama7bOut is not None:
        #     torch.save(hidden_states.cpu(), '8_out0.path')
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.layers = nn.ModuleList(
            [LlamaDecoderLayer(config, id) for id in range(config.num_hidden_layers)])
