...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        # if past_key_value is not None:
        #     torch.save(value_states.cpu(), '3_LinearOperation_out0.pth')
        ...
        #     # [seq_len, batch, head_num, head_size]
            # torch.save(query_states.cpu(), 'save/4_PositionEmbedding1dSplitOperation_out0.pth')
            # torch.save(key_states.cpu(), 'save/5_PositionEmbedding1dSplitOperation_out0.pth')
        # [bsz, nh, t, hd]
...
class LlamaDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.acl_llama_layer = torch.classes.OperationTorch.OperationTorch(
            "LLaMA13BLayerFusionQuantOperation")
        #NEW
        self.q_name = q_name
        self.o_name = o_name
        ...
        self.alpha = 1.0
        param = json.dumps({
            "inputScale_1": float(1 / self.input_scale_dict[self.q_name]),
            "inputOffset_1": int(self.input_offset_dict[self.q_name]),
            "transposeA": False, "transposeB": False,
            "headNum": self.num_heads, "dk": self.head_dim,
            "inputScale_2": float(1 / self.input_scale_dict[self.o_name]),
            "inputOffset_2": int(self.input_offset_dict[self.o_name]),
            "scale": self.alpha,
            "inputScale_3": float(1 / self.input_scale_dict[self.gate_name]),
    ...
    def forward(
        ...
        if past_key_value is not None:
            ...
            #NEW
            self.seq_len = torch.tensor([1] * hidden_states.shape[1], device=self.k_cache_input.device)
            # print(f"seq_len is {self.seq_len}")
            if len(self.acl_weights) == 0:
                self.acl_weights = list(self.state_dict().values())
                del self.acl_weights[21:29]
            ...
            #NEW
            inputs.append(self.attention_mask_max)
            #NEW
            inputs.append(self.k_cache_input)
            #NEW
            inputs.append(self.v_cache_input)
            #NEW
            ...
            #NEW
            inputs.append(self.beta_tensor)
            ...
            #NEW
            print(acl_param)
            hidden_states = self.acl_llama_layer.execute_with_param(inputs, acl_param)
            ...
            # torch.save(self.input_layernorm.weight.cpu(), '0_in1.path')
                # torch.save(attention_mask.cpu(), 'save/attention_mask.pth')
                # torch.save(hidden_states.cpu(), 'save/0_RmsNormOperation_out0.pth')
            ...
            #NEW
            if past_key_value is None:  # [1,32,128,128]
                pass
        ...
        if test_llama7bOut is not None:
            ...
            assert torch.allclose(
                test_llama7bOut[0], hidden_states, rtol=0.05, atol=0.05), 'not equal'
