...
class LlamaRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        ...
        self.rms_norm_operation = torch.classes.OperationTorch.OperationTorch(
            "RmsNormOperation")
    ...
    def forward(self, hidden_states):
        ...
        output_exec = self.rms_norm_operation.execute(
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        acl_qkv: Optional[List[torch.Tensor]] = None,
        ...
        # compare
        acl_q, acl_k, acl_v = acl_qkv
        acl_q = acl_q[0].view(bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2)
        acl_k = acl_k[0].view(bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2)
        acl_v = acl_v[0].view(bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2)
        assert torch.allclose(acl_q, query_states, atol=0.02, rtol=0.02), "Not equal q"
        assert torch.allclose(acl_k, key_states, atol=0.02, rtol=0.02), "Not equal k"
        assert torch.allclose(acl_q, value_states, atol=0.02, rtol=0.02), "Not equal q"
