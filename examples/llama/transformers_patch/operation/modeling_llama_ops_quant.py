...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if past_key_value is None:
            ...
            in_norm_out = self.acl_in_norm_quant_operation.execute([inputs[0], inputs[22], self.beta_tensor])
            ...
            o_linear_quant_out = self.acl_o_linear_quant_operation.execute([
                ...
                self.o_weight,
                self.o_bias,
                self.o_deq_scale.to(torch.float32)
            ])
            ...
            down_linear_quant_out = self.acl_mlp_quant_operation.execute([
                post_norm_out[0],
                self.gate_weight,
                self.gate_deq_scale.to(torch.float32),
                self.gate_bias,
                self.down_weight,
                self.down_deq_scale.to(torch.float32),
                self.down_bias,
                self.up_weight,
                self.up_deq_scale.to(torch.float32),
                self.up_bias,
            ])
