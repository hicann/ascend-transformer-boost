...
class LlamaMLP(nn.Module):
    def __init__(
        ...
        self.mlp_operation = torch.classes.OperationTorch.OperationTorch(
            "MlpOperation")
        
        self.mlp_operation.set_param(json.dumps({"model": "llama13b"}))
    ...
    def forward(self, x):
        ...
        feedForwardOutput = self.act_fn(self.gate_proj(x)) * self.up_proj(x)
        output = self.down_proj(feedForwardOutput)
        output_exec = self.mlp_operation.execute([x,
                                                  self.gate_proj.weight,
                                                  self.up_proj.weight])
        
        if torch.allclose(feedForwardOutput, output_exec[0], rtol=0.02, atol=0.02):
            print("equal")
        else:
            print("not equal")
        return output
