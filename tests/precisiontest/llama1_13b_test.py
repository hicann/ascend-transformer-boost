...
# GOLDEN_SCRIPT_PATH = os.path.join(LLAMA1_13B_PATH, "patches/models/modeling_chatglm_model_precision.py")
...
class Llama1_13b_ModelTest(model_test.ModelTest):
    ...
    def generate_precision_performance(self):
        os.system(f"bash {RUN_SHELL_PATH} --precision --llama1-13b")
        # os.system(f"bash {RUN_SHELL_PATH} {GOLDEN_SCRIPT_PATH} --precision")
        self.append_precision_golden(LLAMA1_13B_PATH + "/hidden_states_golden.pth")
        self.append_precision_result(LLAMA1_13B_PATH + "/hidden_states.pth")
