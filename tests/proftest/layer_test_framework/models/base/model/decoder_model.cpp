...
void DecoderModel::BindFlashcommHostTensor()
{
    std::map<std::string, DynamicParam<std::vector<int64_t>>&> bindFlashCommTensorMap = {
        {"send_counts", this->sendCounts},
        {"sdispls", this->sdispls},
        {"send_count", this->sendCount},
        {"recv_counts", this->recvCounts},
        {"rdispls", this->rdispls},
        {"recv_count", this->recvCount},
    };
    uint32_t tensorIdx = 0;
    for (auto item : bindFlashCommTensorMap) {
        tensorIdx = atb_speed::common::GetTensorIdx(this->inTensorMap, item.first);
        if (tensorIdx != UINT32_MAX) {
            graph_.inTensors.at(tensorIdx).hostData = item.second.Get().data();
        }
    }
}
