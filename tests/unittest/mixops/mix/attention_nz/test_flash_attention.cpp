...
#include "mixops/unpad_flash_attention_nz/tiling/flash_attention_nz_tiling.h"
...
namespace AtbOps {
Status FlashAttentionNzGolden(void *gt, void *gt_high, const GoldenContext &context)
{
    ...
    return MatchAtTensor(atOutTensor, atGtTensor, atGtHighTensor, dtype);
}
} // namespace AtbOps
...
TEST(TestFlashAttentionNz, FlashAttentionTestKVPtr_normmask_dim3)
{
    ...
    PrepareScalarParams(scalars, opTest, atbopsHomeDir, subDir);
    ...
    PrepareVectors(q_seqlen, kv_seqlen, opTest, atbopsHomeDir, subDir);
    uint64_t qnTokens = 0;
    ...
    UtOpDesc opDesc = {"UnpadFlashAttentionNzOperation", opParam};
