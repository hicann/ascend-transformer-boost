...
#include "mixops/unpad_flash_attention/tiling/flash_attention_tiling.h"
...
namespace AtbOps {
Status FlashAttentionNdGolden(void *gt, void *gt_high, std::string &dtype, const GoldenContext &context)
{
    ...
    return MatchAtTensor(atOutTensor_float, atGtTensor_float, atGtHighTensor, dtype);
}
}
...
TEST(TestFlashAttentionNd, TestFlashAttentionNdCanSupport)
{
    ...
    UtOpDesc opDesc = {"UnpadFlashAttentionOperation", opParam};
...
TEST(TestFlashAttentionNd, FlashAttentionTestKVPtr_mask1_fp16)
{
    ...
    std::string dataGen = "python3 " + atbopsHomeDir + "/../../tests/unittest/mix/" +
    ...
    SVector<std::string> inDataFiles = GetInDataFiles(atbopsHomeDir, subDir);
...
TEST(TestFlashAttentionNd, FlashAttentionTestKVPtr_relayAttention_case1)
{
    ...
    std::ofstream shareIdxBin(atbopsHomeDir + "/../../tests/unittest/mix/" + subDir + "/data/share_idx.bin", std::ios::binary);
    shareIdxBin.write((const char *)shareIdx.data(), shareIdx.size() * sizeof(int));
    shareIdxBin.close();
    std::ofstream shareLenBin(atbopsHomeDir + "/../../tests/unittest/mix/" + subDir + "/data/share_len.bin", std::ios::binary);
