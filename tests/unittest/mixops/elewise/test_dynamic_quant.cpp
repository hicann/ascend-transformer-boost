...
Status DQMatchAtTensorFloat(float atol, float rtol, at::Tensor &out, at::Tensor &gt)
{
    ...
    for (int i = 0; i < out.numel(); i++) {
        ...
        if (!FloatUtil::FloatJudgeEqual(expect, actual, ATOL, RTOL)) {
...
Status DQMatchAtTensorInt(float atol, float rtol, at::Tensor &out, at::Tensor &gt)
{
    ...
    if (!FloatUtil::FloatJudgeEqual(float(out.numel()), float(countRight + countFalse), atol, rtol)) {
        std::string msg =
...
Status DynamicQuantGolden(float atol, float rtol, OpParam::Elewise opParam, const GoldenContext &context)
{
...
TEST(DynamicQuantFP16, DynamicQuantFP16Case1)
{
    ...
    uint32_t c = 32;
    MkiOpTest opTest;
    AtbOps::OpParam::Elewise opParam;
    UtOpDesc opDesc;
    opDesc.opName = "ElewiseOperation";
    opParam.elewiseType = AtbOps::OpParam::Elewise::ELEWISE_DYNAMIC_QUANT;
