...
Status SinF32Compare(float atol, float rtol, const GoldenContext &context)
{
    ...
    for (size_t i = 0; i < inTensor.desc.Numel(); ++i) {
        if (!FloatUtil::FloatJudgeEqual(SinRef(indata[i]), outdata[i], atol, rtol)) {
...
Status SinF16Compare(float atol, float rtol, const GoldenContext &context)
{
    const Tensor &inTensor = context.hostInTensors.at(0);
    const Tensor &outTensor = context.hostOutTensors.at(0);
    ...
    for (size_t i = 0; i < inTensor.desc.Numel(); ++i) {
        if (!FloatUtil::FloatJudgeEqual(SinRef(static_cast<float>(indata[i])), static_cast<float>(outdata[i]), atol,
                                        rtol)) {
            return Status::FailStatus(-1, "float judge not equal");
        }
    }
