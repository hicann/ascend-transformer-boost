...
Status FastGeluF16Compare(float atol, float rtol, const GoldenContext &context)
{
    ...
    for (int64_t i = 0; i < outTensor.Numel(); i++) {
        ...
        if (!FloatUtil::FloatJudgeEqual(static_cast<float>(expect), static_cast<float>(result), atol, rtol)) {
