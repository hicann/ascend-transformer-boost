...
#include "torch/operation/operation_creator.h"
...
AsdOps::Status OpTest::Prepare(AclTransformer::Operation *operation ,const AsdOps::SVector<AsdOps::Tensor> &inTensorLists)
{
    ...
    if (inTensorLists.size() != operation_->GetInTensorCount()) {
        return AsdOps::Status::FailStatus(-1, "InTensors Count not equal " + std::string(operation_->GetInTensorCount()));
    }

    AsdOps::Status st = operation_->BuildPlan(&plan_);
    if (!st.Ok()) {
        return AsdOps::Status::FailStatus(-1, opName + " build plan fail");
    }
...
{
    AsdOps::SVector<AsdOps::TensorDesc> outTensorDescs(operation_->GetOutTensorCount());
    AsdOps::Status st = operation_->InferShape(hostInTensors, outTensorDescs);
    if (!st.Ok()) {
...
AsdOps::Status OpTest::RunOperation()
{
    ...
    st = plan_Execute(handle_, variantPack_);
    if (!st.Ok()) {
...
Status OpTest::Run(AclTransformer::Operation *operation, const AsdOps::SVector<AsdOps::TensorDesc> &inTensorDescs)
{
    AsdOps::SVector<AsdOps::Tensor> inTensors(inTensorDescs.size());
    GenerateRandomTensors(inTensorDescs, inTensors);
    return RunImpl(operation, inTensors);
}

Status OpTest::Run(AclTransformer::Operation *operation, const AsdOps::SVector<AsdOps::TensorDesc> &inTensorDescs, const AsdOps::Any &varaintPackParam)
{
    AsdOps::SVector<AsdOps::Tensor> inTensors(inTensorDescs.size());
    GenerateRandomTensors(inTensorDescs, inTensors);
...
Status OpTest::Run(AclTransformer::Operation *operation, const AsdOps::SVector<AsdOps::Tensor> &inTensorLists)
{
    return RunImpl(operation, inTensorLists);
}

Status OpTest::Run(AclTransformer::Operation *operation, const AsdOps::SVector<AsdOps::Tensor> &inTensorLists, const AsdOps::Any &varaintPackParam)
{
    return RunImpl(operation, inTensorLists, varaintPackParam);
}

Status OpTest::RunImpl(AclTransformer::Operation *operation, const AsdOps::SVector<AsdOps::Tensor> &inTensorLists, const AsdOps::Any &varaintPackParam)
{
    Cleanup();
    Init();
...
Status OpTest::RunImpl(AclTransformer::Operation *operation, const AsdOps::SVector<AsdOps::Tensor> &inTensorLists)
{
    Cleanup();
    Init();
