...
AsdOps::Status AddGolden0(const GoldenContext &context)
{
    ...
    atInRefTensor2 = atInRefTensor2.view(
        {atInRefTensor2.sizes()[0], atInRefTensor2.sizes()[1] * paramHeadNum, mixedKey.sizes()[2] / paramHeadNum});
...
TEST(TestSelfAttentionOperation, TestSelfAttention)
{
    ...
    AsdOps::SVector<AsdOps::TensorDesc> inTensorDescs = {
        ...
        {AsdOps::TENSOR_DTYPE_FLOAT, AsdOps::TENSOR_FORMAT_ND, {4, 5, 6, 7}}};
    OpTest opTest0(3);
    opTest0.Golden(&AddGolden0);
    AsdOps::Status status0 = opTest0.Run(&op, inTensorDescs);
    ASSERT_EQ(status0.Ok(), true);
}