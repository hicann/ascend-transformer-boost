...
AsdOps::Status MlpGolden(const GoldenContext &context)
{
    ...
    at::Tensor atInRefTensor4 =
        at::from_blob(inTensor4.data, ToIntArrayRef(inTensor4.desc.dims), at::kHalf).to(at::kFloat);

    const AsdOps::Tensor outTensor = context.hostOutTensors.at(0);
    at::Tensor atOutTensor = at::from_blob(outTensor.data, ToIntArrayRef(outTensor.desc.dims), at::kHalf);

    atInRefTensor1 =
        at::silu(at::matmul(atInRefTensor1, atInRefTensor2.t())) * at::matmul(atInRefTensor1, atInRefTensor4.t());
    at::Tensor refOutTensor = at::matmul(atInRefTensor1, atInRefTensor3.t()).to(at::kHalf).contiguous();
