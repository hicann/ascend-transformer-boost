...
TEST(TestPositionEmbedding1dSplitOperation, InferShape)
{
    ...
    AsdOps::SVector<AsdOps::Tensor> inTensors = {{AsdOps::TENSOR_DTYPE_FLOAT, AsdOps::TENSOR_FORMAT_ND, {8, 4, 512}},
                                                     {AsdOps::TENSOR_DTYPE_FLOAT, AsdOps::TENSOR_FORMAT_ND, {8, 4, 512}}};
...
AsdOps::Status PosEmb1dSplitGolden(AclTransformer::PositionEmbedding1dSplitParam param, const GoldenContext &context)
{
    ...
    at::Tensor atInRefinput = at::from_blob(input.data, ToIntArrayRef(input.desc.dims), at::kHalf);
    const Tensor &positionIds = context.hostInTensors.at(1);
    at::Tensor atInRefpositionIds = at::from_blob(positionIds.data, ToIntArrayRef(positionIds.desc.dims), at::kHalf);
    const Tensor &cosTable = context.hostInTensors.at(2);
    at::Tensor atInRefcosTable = at::from_blob(cosTable.data, ToIntArrayRef(cosTable.desc.dims), at::kHalf);
    const Tensor &sinTable = context.hostInTensors.at(3);
    at::Tensor atInRefsinTable = at::from_blob(sinTable.data, ToIntArrayRef(sinTable.desc.dims), at::kHalf);

    // // [batch, seqlen, embeddim]
    // AsdOps::SVector<AsdOps::Tensor> inTensors = {{AsdOps::TENSOR_DTYPE_FLOAT, AsdOps::TENSOR_FORMAT_ND, {8, 8, 512}},
    //                                             {AsdOps::TENSOR_DTYPE_INT64, AsdOps::TENSOR_FORMAT_ND, {8, 4, 1}},
    //                                             {AsdOps::TENSOR_DTYPE_FLOAT, AsdOps::TENSOR_FORMAT_ND, {1, 1, 8, 64}},
    //                                             {AsdOps::TENSOR_DTYPE_FLOAT, AsdOps::TENSOR_FORMAT_ND, {1, 1, 8, 64}}};
    // torch::Tensor input = TorchUtil::AsdOpsTensor2AtTensor(handle, inTensors[0]);
    // torch::Tensor positionIds = TorchUtil::AsdOpsTensor2AtTensor(handle, inTensors[1]);
    // torch::Tensor cosTable = TorchUtil::AsdOpsTensor2AtTensor(handle, inTensors[2]);
    // torch::Tensor sinTable = TorchUtil::AsdOpsTensor2AtTensor(handle, inTensors[3]);
...
TEST(TestPositionEmbedding1dSplitOperation, TestPositionEmbedding1dSplit)
{
    ...
    AsdOps::SVector<AsdOps::TensorDesc> inTensorDescs = {{AsdOps::TENSOR_DTYPE_FLOAT, AsdOps::TENSOR_FORMAT_ND, {8, 8, 512}},
