...
TEST(TestQuantOperation, InferShape)
{
    AclTransformer::QuantParam param;
    AclTransformer::QuantOperation op(param);
    ...
    AsdOps::SVector<int64_t> expectDims1 = {1, 8};
...
AsdOps::Status QuantGolden(const GoldenContext &context){
    ...
    at::Tensor refOutTensor=atInRefinput;
    //refOutTensor.type(torch.uint8);
...
TEST(TestQuantOperation, QuantOperation)
{
    AclTransformer::QuantParam opParam;
