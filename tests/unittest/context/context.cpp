...
Context::Context()
{
    ...
    ASD_LOG(FATAL) << "Context hosttiling bufferRing:" << bufferRing << ", bufferSize:" << bufferSize;
    ...
    ASD_LOG(FATAL) << "Context tiling bufferRing:" << bufferRing << ", bufferSize:" << bufferSize;
    ...
    ASD_LOG(FATAL) << "Context workspace bufferRing:" << bufferRing << ", bufferSize:" << bufferSize;
    ...
    ASD_LOG(FATAL) << "Context intermediate bufferRing:" << bufferRing << ", bufferSize:" << bufferSize;
...
{
    const char *envStr = std::getenv("ACLTRANSFORMER_CONTEXT_HOSTTILING_RING");
    if (envStr == nullptr) {
        return 1;
    }
    return atoll(envStr);
...
{
    const char *envStr = std::getenv("ACLTRANSFORMER_CONTEXT_HOSTTILING_SIZE");
    if (envStr == nullptr) {
        return 0;
    }
    return atoll(envStr);
...
{
    const char *envStr = std::getenv("ACLTRANSFORMER_CONTEXT_TILING_RING");
    if (envStr == nullptr) {
        return 1;
    }
    return atoll(envStr);
...
{
    const char *envStr = std::getenv("ACLTRANSFORMER_CONTEXT_TILING_SIZE");
    if (envStr == nullptr) {
        return 0;
    }
    return atoll(envStr);
...
{
    const char *envStr = std::getenv("ACLTRANSFORMER_CONTEXT_WORKSPACE_RING");
    if (envStr == nullptr) {
        return 1;
    }
    return atoll(envStr);
...
{
    const char *envStr = std::getenv("ACLTRANSFORMER_CONTEXT_WORKSPACE_SIZE");
    if (envStr == nullptr) {
        return 0;
    }
    return atoll(envStr);
...
{
    const char *envStr = std::getenv("ACLTRANSFORMER_CONTEXT_INTERMEDIATE_RING");
    if (envStr == nullptr) {
        return 1;
    }
    return atoll(envStr);
...
{
    const char *envStr = std::getenv("ACLTRANSFORMER_CONTEXT_INTERMEDIATE_SIZE");
    if (envStr == nullptr) {
        return 0;
    }
    return atoll(envStr);
