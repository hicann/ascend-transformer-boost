...
GPTNEOX_20B_PATH = os.path.join(model_test.ACLTRANSFORMER_HOME_PATH, "examples/gptneox20b")
RUN_SHELL_PATH = os.path.join(GPTNEOX_20B_PATH, "run.sh")
MODEL_SCRIPT_PATH = os.path.join(GPTNEOX_20B_PATH, "patches/models/modeling_gpt_neox_model_flashattention_performance_v2.py")

class GptNeox_20b_ModelTest(model_test.ModelTest):
    ...
    def generate_time_performance(self):
        ...
        performance_file_path = os.path.join(GPTNEOX_20B_PATH, f"zhiputest_{self.device_type}_gptneox_20b.csv")
        print("-----gptneox20b_test-----")
        print(f"[MODEL]: gptneox20b")
        ...
        os.system(f"bash {RUN_SHELL_PATH} --zhipu")
...
def main():
    test_body = GptNeox_20b_ModelTest("gptneox20b")
