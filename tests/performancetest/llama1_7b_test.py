...
class Llama1_7b_ModelTest(model_test.ModelTest):
    ...
    def generate_time_performance(self):
        ...
        performance_file_path = os.path.join(LLAMA1_7B_PATH, f"zhiputest_{self.device_type}_llama1-7b.csv")
        print("-----llama_test-----")
        print(f"[MODEL]: llama1_7b")
        print(f"[RUN_SHELL_PATH]: {RUN_SHELL_PATH}")
        print(f"[MODEL_SCRIPT_NAME]: {MODEL_SCRIPT_NAME}")
        os.system(f"bash {RUN_SHELL_PATH} --zhipu --llama1-7b {MODEL_SCRIPT_NAME}")
