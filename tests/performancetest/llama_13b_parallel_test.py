...
LLAMA1_13B_PATH = os.path.join(model_test.ACLTRANSFORMER_HOME_PATH, "examples/llama13b_parallel")
RUN_SHELL_PATH = os.path.join(LLAMA1_13B_PATH, "run.sh")
# MODEL_SCRIPT_PATH = os.path.join(LLAMA1_13B_PATH, "transformers_patch/layer/modeling_layer_performance.py")
...
class Llama1_13b_ModelTest(model_test.ModelTest):
    ...
    def generate_time_performance(self):
        ...
        # performance_file_path = os.path.join(LLAMA1_13B_PATH, f"zhiputest_{self.device_type}_llama1-13b_parallel.csv")
        performance_file_path = os.path.join(LLAMA1_13B_PATH, f"zhiputest_llama1-13b_parallel.csv")
        print("-----llama_test-----")
        print(f"[MODEL]: llama1_13b_parallel")
        print(f"[RUN_SHELL_PATH]: {RUN_SHELL_PATH}")
        # print(f"[MODEL_SCRIPT_PATH]: {MODEL_SCRIPT_PATH}")
        # os.system(f"bash {RUN_SHELL_PATH} --zhipu --llama1-13b {MODEL_SCRIPT_PATH} ")
        os.system(f"bash {RUN_SHELL_PATH} --zhipu --llama1-13b_parallel")
