...
zero_tensor_data = torch.from_numpy(np.random.uniform(low=-2, high=2, size=shape1).astype(np.float16)).npu()

def layer_norm_compute(input_x, input_y, input_gamma, input_beta,
    ...
    input_x = np.add(input_x, input_y) 
    reduce_elts = 1.0
