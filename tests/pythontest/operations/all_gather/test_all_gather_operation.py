...
# torchrun --standalone --nnodes=1 --nproc_per_node=8 test_all_gather_operation.py
# attention: when you use lccl backend, unset HCCL_MTE_ENABLE.
...
acl_param = json.dumps(
    {"rank": local_rank, "rankSize": world_size, "rankRoot": 0, "backend": "lccl"})
...
outTensors = acl_allgather_operation.execute([oneTensor])
print('acl all gather output is ' + str(outTensors[0]))
print('acl all gather output shape is ' + str(outTensors[0].shape))
