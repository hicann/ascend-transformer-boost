...
Status MkiOpTest::RunImpl(const UtOpDesc &opDesc, const SVector<TensorDesc> &inTensorDescs, const std::string &kernelName)
{
    Timer timer;
    Cleanup();
    Init();
...
void MkiOpTest::PrepareKVcacheBatchwiseTensors(uint32_t batchNum, const TensorDesc &kvTensorDescIn, const std::vector<std::string> &kvfiles, uint32_t share_len)
{
    Cleanup();
    Init();
    kvFile_ = kvfiles;
...
void MkiOpTest::CreateTensorListForKVPtr(const uint32_t batchNum, std::vector<uint8_t *>& inTensorPtrs, Tensor &deviceTensor) {
