                         ...
                         const std::string &atbopsHomeDir, const std::string &subDir)
{
    ...
    opTest.ReadFile(batch_num.data(), batch_num.size() * sizeof(uint32_t),
                    atbopsHomeDir + "/../../tests/unittest/mix/" + subDir + "/data/batch_num.bin");
    opTest.ReadFile(embd_dim.data(), embd_dim.size() * sizeof(uint32_t),
                    atbopsHomeDir + "/../../tests/unittest/mix/" + subDir + "/data/embd_dim.bin");
    opTest.ReadFile(q_heads.data(), q_heads.size() * sizeof(uint32_t),
                    atbopsHomeDir + "/../../tests/unittest/mix/" + subDir + "/data/q_heads.bin");
    opTest.ReadFile(kv_heads.data(), kv_heads.size() * sizeof(uint32_t),
                    atbopsHomeDir + "/../../tests/unittest/mix/" + subDir + "/data/kv_heads.bin");
    opTest.ReadFile(max_seqlen.data(), max_seqlen.size() * sizeof(uint32_t),
                    atbopsHomeDir + "/../../tests/unittest/mix/" + subDir + "/data/max_seqlen.bin");
    opTest.ReadFile(is_batch_dynamic.data(), is_batch_dynamic.size() * sizeof(uint32_t),
                    atbopsHomeDir + "/../../tests/unittest/mix/" + subDir + "/data/is_batch_dynamic.bin");
                    ...
                    MkiOpTest &opTest, const std::string &atbopsHomeDir, const std::string &subDir)
{
    opTest.ReadFile(q_seqlen.data(), q_seqlen.size() * sizeof(int32_t),
                    atbopsHomeDir + "/../../tests/unittest/mix/" + subDir + "/data/q_seqlen.bin");
    opTest.ReadFile(kv_seqlen.data(), kv_seqlen.size() * sizeof(int32_t),
                    atbopsHomeDir + "/../../tests/unittest/mix/" + subDir + "/data/kv_seqlen.bin");
    opTest.ReadFile(batch_state.data(), batch_state.size() * sizeof(int32_t),
                    atbopsHomeDir + "/../../tests/unittest/mix/" + subDir + "/data/batchRunStatus.bin");
}
                   ...
                   const std::string &atbopsHomeDir, const std::string &subDir)
{
    opTest.ReadFile(expect.data(), expect.size() * sizeof(uint16_t),
                    atbopsHomeDir + "/../../tests/unittest/mix/" + subDir + "/data/expect.bin");
    opTest.ReadFile(expect_high.data(), expect_high.size() * sizeof(uint32_t),
                    atbopsHomeDir + "/../../tests/unittest/mix/" + subDir + "/data/expect_high.bin");
}
                    ...
                    const std::string &subDir, bool isRelay, uint32_t share_len)
{
    for (size_t i = 0; i < batch; ++i) {
        std::string file = atbopsHomeDir + "/../../tests/unittest/mix/" + subDir + "/data/kvBatches/input2_" +
                           std::to_string(i) + ".bin";
        kvFiles.push_back(file);
    }
    for (size_t i = 0; i < batch; ++i) {
        std::string file = atbopsHomeDir + "/../../tests/unittest/mix/" + subDir + "/data/kvBatches/input3_" +
                           std::to_string(i) + ".bin";
        kvFiles.push_back(file);
    }
    if (isRelay) {
        for (size_t i = 0; i < share_len; ++i) {
            std::string file = atbopsHomeDir + "/../../tests/unittest/mix/" + subDir + "/data/kvBatches/share_input2_" +
                               std::to_string(i) + ".bin";
            kvFiles.push_back(file);
        }
        for (size_t i = 0; i < share_len; ++i) {
            std::string file = atbopsHomeDir + "/../../tests/unittest/mix/" + subDir + "/data/kvBatches/share_input3_" +
                               std::to_string(i) + ".bin";
            kvFiles.push_back(file);
        }
    }
...
SVector<std::string> GetInDataFiles(const std::string &atbopsHomeDir, const std::string &subDir, bool isRelay)
{
    ...
    if (isRelay) {
        inDataFiles = {
            atbopsHomeDir + "/../../tests/unittest/mix/" + subDir + "/data/input1.bin",
            atbopsHomeDir + "/../../tests/unittest/mix/" + subDir + "/data/input4.bin",
        };
    } else {
        inDataFiles = {
            atbopsHomeDir + "/../../tests/unittest/mix/" + subDir + "/data/input1.bin",
            atbopsHomeDir + "/../../tests/unittest/mix/" + subDir + "/data/input2.bin",
            atbopsHomeDir + "/../../tests/unittest/mix/" + subDir + "/data/input3.bin",
            atbopsHomeDir + "/../../tests/unittest/mix/" + subDir + "/data/layerId.bin",
            atbopsHomeDir + "/../../tests/unittest/mix/" + subDir + "/data/input4.bin",
        };
    }
