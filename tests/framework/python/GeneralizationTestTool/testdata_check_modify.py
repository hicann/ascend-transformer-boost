...
class LinearOperation(MatmulCommon):
    ...
    def tensor_desc_check_modify(op_param, tensor_desc: TensorDesc):
        ...
        m = random.randint(1, 3) * 32
        k = random.randint(3, 5) * 32
        n = random.randint(1, 3) * 32
        batch = random.choice(range(2, 4))
        ...
        if (outDataType == 1 or outDataType == 27) and tensor_desc.formats[1] == "fractal_nz" and get_soc_version() == "Ascend910B":
            return False
        if transposeA is True:
