...
err_enum_dict = {0: "NO_ERROR", 1: "ERROR_INVALID_PARAM", 2: "ERROR_INVALID_GRAPH", 3: "ERROR_INTERNAL_ERROR",
                 4: "ERROR_RT_FAIL",
                 5: "ERROR_INVALID_IN_TENSOR_NUM", 6: "ERROR_INVALID_TENSOR_DTYPE", 7: "ERROR_INVALID_TENSOR_FORMAT",
                 8: "ERROR_INVALID_TENSOR_DIM",
                 9: "ERROR_INVALID_TENSOR_SIZE", 10: "ERROR_OPERATION_NULL_RUNNER",
                 11: "ERROR_GRAPH_INFERSHAPE_FUNC_FAIL", 12: "ERROR_CANN_ERROR",
                 13: "ERROR_INVALID_TENSOR_INI_MATCH", 14: "ERROR_INVALID_TENSOR_ADDR", 15: "ERROR_INVALID_TENSOR_NUM",
                 16: "ERROR_INVALID_TENSOR_DIM_NUM",
                 17: "ERROR_INVALID_SINGLE_OPERATION_PARAM", 18: "ERROR_GRAPH_NODE_RESHAPE_FUNC_FAIL",
                 19: "ERROR_INVALID_GRAPH_NODE_CHUNK",
                 20: "ERROR_INVALID_CONTEXT_ADDR", 21: "ERROR_INVALID_STREAM", 22: "ERROR_INVALID_WORKSPACE_SIZE",
                 23: "ERROR_INVALID_WORKSPACE_ADDR",
                 24: "ERROR_INVALID_OPERATION_ADDR", 25: "ERROR_HCCL_FAIL", 26: "ERROR_OUT_OF_DEVICE_MEMORY",
                 27: "ERROR_OUT_OF_HOST_MEMORY",
...
class CsvOpsTest():
    ...
    def __init__(self, operation_info_file, args):
        ...
        self.operation_name = ''
        self.op_param_str = ''
        self.file_data = pd.read_csv(operation_info_file, sep='|', skipinitialspace=True,
                                     dtype={'InShape': str, 'OutShape': str, 'TestType': str})
    ...
    def need_to_run_case(self, case_index, args, card_type):
        ...
        if args.operation_name != '' and not re.search(op_name, args.operation_name, re.I):
            logging.warning("CaseNum %d, Case %s will not run because OpName %s differs from input OperationName %s",
                            self.case_num.loc[case_index], self.case_name.loc[case_index], self.op_name.loc[case_index],
                            args.operation_name)
            logging.info("")
            return False
        if card_type == 'single_card' and op_name in ['AllGatherOperation', 'AllReduceOperation', 'BroadcastOperation',
                                                      'ReduceScatterOperation', 'LinearParallelOperation',
                                                      'AllToAllVOperation', 'AllToAllVV2Operation',
                                                      'AllToAllOperation']:
            return False
        if card_type == 'multi_card':
            if op_name not in ['AllGatherOperation', 'AllReduceOperation', 'BroadcastOperation',
                               'ReduceScatterOperation', 'LinearParallelOperation', 'AllToAllVOperation',
                               'AllToAllVV2Operation', 'AllToAllOperation']:
                return False
            elif self.args.world_size != json.loads(self.op_param.loc[case_index])['rankSize']:
                logging.warning(
                    "CaseNum %d, Case %s will not run because args.world_size %s differs from testcase's rank_size %s",
                    self.case_num.loc[case_index], self.case_name.loc[case_index], self.args.world_size,
                    json.loads(self.op_param.loc[case_index])['rankSize'])
                logging.info("")
                return False
        if args.test_type != '' and self.test_type.loc[case_index] != args.test_type:
            logging.warning("CaseNum %d, Case %s will not run because TestType %s differs from input TestType %s",
                            self.case_num.loc[case_index], self.case_name.loc[case_index],
                            self.test_type.loc[case_index], args.test_type)
            logging.info("")
            return False
        if args.test_level != '' and not pd.isnull(self.test_level.loc[case_index]) and self.test_level.loc[
            case_index] != args.test_level:
            logging.warning("CaseNum %d, Case %s will not run because TestLevel %s differs from input TestLevel %s",
                            self.case_num.loc[case_index], self.case_name.loc[case_index],
                            self.test_level.loc[case_index], args.test_level)
            logging.info("")
            return False
        if args.model != '' and self.from_model.loc[case_index] != args.model:
            logging.warning("CaseNum %d, Case %s will not run because FromModel %s differs from input FromModel %s",
                            self.case_num.loc[case_index], self.case_name.loc[case_index],
                            self.from_model.loc[case_index], args.model)
            logging.info("")
            return False
        if args.soc_version != '' and not pd.isnull(self.soc_version.loc[case_index]):
            ...
            if not args.soc_version in soc_version_list:
                logging.warning(
                    "CaseNum %d, Case %s will not run because SocVersion %s differs from input SocVersion %s",
                    self.case_num.loc[case_index], self.case_name.loc[case_index], self.soc_version.loc[case_index],
                    args.soc_version)
                logging.info("")
                return False
        # CSV中soc_version为空的用例不在910A/310B上执行
        if pd.isnull(self.soc_version.loc[case_index]) and (
                args.soc_version == "Ascend910A" or args.soc_version == "Ascend310B"):
            logging.warning("CaseNum %d, Case %s will not run because SocVersion %s differs from input SocVersion %s",
                            self.case_num.loc[case_index], self.case_name.loc[case_index],
                            self.soc_version.loc[case_index], args.soc_version)
            logging.info("")
            return False
        return True
    ...
    def __dump_tensor(self, tensor, tensor_data_type, index, info):
        if self.args.save_tensor:
            dump_path = self.data_save_path + self.case_name.loc[self.index] + '_' + \
                        str(self.case_num.loc[self.index]) + '_deviceid_' + str(
                torch.npu.current_device()) + '_' + tensor_data_type + '_index_' + str(index)
        ...
        for i in range(self.in_num.loc[self.index]):
            if intensor_file_list[i] != '':
                ...
                if data_gen_type_list[i] in ['one', 'zero', 'random']:
                    input_tensor = eval(tensor_gen_func)(shapes[i], dtype, input_formats_list[i],
                                                         data_gen_ranges_list[i], self.op_param_str)
                elif data_gen_type_list[i] == 'customize':
                    input_tensor = eval(tensor_gen_func)(shapes, i, dtype, input_formats_list[i],
                                                         data_gen_ranges_list[i], self.op_param_str)
                else:
                    logging.error("DataGenType only supports one/zero/random/customize but for this case it is %s",
                                  data_gen_type_list[i])
                    quit(1)
    ...
    def analyse_result(self):
        ...
        if self.args.precision_standard == 'new' and self.op_type == data_generation.OpTypes.NA:
            logging.error(
                f"{self.operation_name}'s precision_standard is 'new', so its op_type(from get_op_type method) cannot be 'OpTypes.NA'!")
            exit(1)
    ...
    def get_json_result(self, json_result, running_stage, times):
        ...
        if self.file_data.loc[self.index, 'ActualError'] != "NO_ERROR":
            result_flag = False
        if self.expected_error.loc[self.index] != "NO_ERROR" and self.expected_error.loc[self.index].startswith(
                running_stage):
            result_flag = False
        if result_flag:
            ...
            logging.info("CaseNum " + str(self.case_num.loc[self.index]) + ", Case " + self.case_name.loc[
                self.index] + " times " + str(times) + " end, "
                                                       "result: " + self.file_data.loc[self.index, 'Result'])
        logging.debug("Stage " + running_stage + " end, ActualError: " + self.file_data.loc[
            self.index, 'ActualError'] + ", ExpectedError: " + self.expected_error.loc[self.index])
        return result_flag

    def run_one_case(self, index, times, output_file):
        logging.info(
            "CaseNum " + str(self.case_num.loc[index]) + ", Case " + self.case_name.loc[index] + " times " + str(
                times) + " start...")
        self.index = index
        ...
        execute_result = self.operation.execute_sync(self.input_tensor_list, self.output_tensor_list,
                                                     self.workspace_size)
    ...
    def generate_golden_output_tensors(self):
        ...
        if self.args.precision_standard == 'new' and self.op_type == data_generation.OpTypes.CV_FUSION:
            ...
            self.gpu_golden_output_tensor_list = eval('proxy.' + self.operation_name)(golden_input_tensors,
                                                                                      self.op_param_str)
    ...
    def __precision_eb_percent(self, i, actual_output, golden_output, precision_threshold, eb_threshold):
        actual_output = actual_output if actual_output.dtype != torch.bool else actual_output.long()
        golden_output = golden_output if golden_output.dtype != torch.bool else golden_output.long()
        if self.op_type in [data_generation.OpTypes.COMPUTE_FLOAT, data_generation.OpTypes.COMPUTE_FLOAT_HIGH_PRECISION,
                            data_generation.OpTypes.VECTOR_FUSION] and actual_output.dtype in [torch.float16,
                                                                                               torch.bfloat16]:
        ...
        if self.op_type in [data_generation.OpTypes.CV_FUSION]:
            ...
            if actual_output.dtype in [torch.float16, torch.bfloat16]:
                ...
                gpu_golden_output = torch.where(torch.isnan(gpu_golden_output), torch.full_like(gpu_golden_output, 0),
                                                gpu_golden_output)
                gpu_golden_output = torch.where(torch.isinf(gpu_golden_output), torch.full_like(gpu_golden_output, 0),
                                                gpu_golden_output)
            ...
            self.__dump_tensor(tolerance, 'tolerance', i,
                               'index {}, precision_threshold: {}'.format(i, precision_threshold))
...
class CsvOpsResult():
    ...
    def reset(self, index):
        self.result = {"succ": 0, "fail": 0, "SetupTime(us)": 0, "ExecuteTime(us)": 0, "SyncTime(us)": 0,
                       ...
                       "PrecisionPercent": '', "EBPercent": '',
                       "FirstErrType": self.opsTest.file_data.loc[index, 'ExpectedError']}
    ...
    def add_result(self, opsTest):
        if (opsTest.file_data.loc[opsTest.index, 'Result'] == 'succ'):
            if self.result['succ'] == 0 and not opsTest.args.skip_verify:
                self.compare_tensor_num = opsTest.out_num.loc[opsTest.index] if opsTest.out_num.loc[
                                                                                    opsTest.index] > 0 else \
                opsTest.in_num.loc[opsTest.index]
                self.compare_dtypes = opsTest.out_dtype.loc[opsTest.index] if opsTest.out_num.loc[
                                                                                  opsTest.index] > 0 else \
                opsTest.in_dtype.loc[opsTest.index]
                for i in range(self.compare_tensor_num):
            ...
            if opsTest.args.skip_verify:
                return
            if self.opsTest.test_type.loc[opsTest.index] == "Performance" and self.result[
                'succ'] <= CsvOpsResult.SKIP_PERFORMANCE_CASE_NUM:
                return
    ...
    def __precision_check(self, case_sum):
        ...
        for i in range(len(compare_dtype_list)):
            if self.opsTest.args.precision_standard == 'new':
                precision_percent = [(float(percent[:-1])) for percent in
                                     self.opsTest.file_data.loc[self.opsTest.index, 'PrecisionPercent'].split(";")]
                eb_percent = [(float(percent[:-1])) for percent in
                              self.opsTest.file_data.loc[self.opsTest.index, 'EBPercent'].split(";")]
                if self.opsTest.op_type == data_generation.OpTypes.RAND:
                    ...
                    if precision_percent[i] < case_pass_percent:
                        ...
                        self.opsTest.file_data.loc[
                            self.opsTest.index, 'ActualError'] = 'precision_percent {}% under case_pass_percent {}%, op type: {}'.format(
                            precision_percent[i], str(case_pass_percent)[:5], self.opsTest.op_type)
                        return False
                elif precision_percent[i] < 100:
                    self.opsTest.file_data.loc[self.opsTest.index, 'CasePassed'] = 'No'
                    self.opsTest.file_data.loc[
                        self.opsTest.index, 'ActualError'] = 'precision under 100%, dtype: {}, op type: {}'.format(
                        compare_dtype_list[i], self.opsTest.op_type)
                    return False
                if eb_percent[i] > 100:
                    self.opsTest.file_data.loc[self.opsTest.index, 'CasePassed'] = 'No'
                    self.opsTest.file_data.loc[
                        self.opsTest.index, 'ActualError'] = 'eb over 100%, dtype: {}, op type: {}'.format(
                        compare_dtype_list[i], self.opsTest.op_type)
                    return False
            else:
                error_percent = [(float(percent[:-1])) for percent in self.opsTest.file_data.loc[
                    self.opsTest.index, CsvOpsResult.precision_standard[compare_dtype_list[i]][0]].split(";")]
                if error_percent[i] < CsvOpsResult.precision_standard[compare_dtype_list[i]][1]:
                    self.opsTest.file_data.loc[self.opsTest.index, 'CasePassed'] = 'No'
                    self.opsTest.file_data.loc[
                        self.opsTest.index, 'ActualError'] = 'precision check fail, dtype: {}, standard: {}'.format(
                        compare_dtype_list[i], CsvOpsResult.precision_standard[compare_dtype_list[i]])
                    return False
        return True

    def __performance_check(self):
        performance_threshold = eval('data_generation.' + self.opsTest.operation_name + '.performance_threshold')(
            self.opsTest.op_param.loc[self.opsTest.index])
        for key, thre in performance_threshold.items():
            value = self.opsTest.file_data.loc[self.opsTest.index, key]
            if int(value) > thre:
                self.opsTest.file_data.loc[self.opsTest.index, 'CasePassed'] = 'No'
                self.opsTest.file_data.loc[
                    self.opsTest.index, 'ActualError'] = 'performance check fail, stage: {}, value: {}, threshold: {}'.format(
                    key, value, thre)
                return False
        return True
    ...
    def compute_result(self, case_index):
        ...
        if (case_sum > 0 and not self.opsTest.args.skip_verify):
            ...
            self.opsTest.file_data.loc[case_index, 'ExecuteTime(us)'] = str(
                int(self.result['ExecuteTime(us)'] / case_sum))
            self.opsTest.file_data.loc[case_index, 'SyncTime(us)'] = str(int(self.result['SyncTime(us)'] / case_sum))
            self.opsTest.file_data.loc[case_index, 'TotalTime(us)'] = str((int(self.result['SetupTime(us)']) + int(
                self.result['ExecuteTime(us)']) + int(self.result['SyncTime(us)'])) / case_sum)
            if self.opsTest.args.precision_standard == 'new':
        ...
        logging.info("CaseNum " + str(self.opsTest.case_num.loc[case_index]) + ", Case " + self.opsTest.case_name.loc[
            case_index] + " end, "
                          "Case Passed: " + self.opsTest.file_data.loc[case_index, 'CasePassed'])
        if self.opsTest.file_data.loc[case_index, 'CasePassed'] == 'No':
            logging.error(
                "CaseNum " + str(self.opsTest.case_num.loc[case_index]) + ", Case " + self.opsTest.case_name.loc[
                    case_index] +
    ...
    def __need_to_save_case(self, data, index, args, card_type):
        ...
        if card_type == 'single_card' and op_name in ['AllGatherOperation', 'AllReduceOperation', 'BroadcastOperation',
                                                      'ReduceScatterOperation', 'LinearParallelOperation',
                                                      'AllToAllVOperation', 'AllToAllOperation',
                                                      'AllToAllVV2Operation']:
            return False
        if card_type == 'multi_card' and op_name not in ['AllGatherOperation', 'AllReduceOperation',
                                                         'BroadcastOperation', 'ReduceScatterOperation',
                                                         'LinearParallelOperation', 'AllToAllVOperation',
                                                         'AllToAllOperation', 'AllToAllVV2Operation']:
            return False
        if args.test_type != '' and data.loc[index, "TestType"] != args.test_type:
            return False
        if args.test_level != '' and not pd.isnull(data.loc[index, "TestLevel"]) and data.loc[
            index, "TestLevel"] != args.test_level:
            return False
    ...
    def save_result_to_csv(self, actual_output_file, from_case_index, to_case_index, args, card_type):
        ...
        max_widths = [
            max([len(str(row[i])) for row in self.data_to_export.values] + [len(str(self.data_to_export.columns[i]))])
            for i in range(len(self.data_to_export.columns))]
...
class CsvOpsTestUtil:
    ...
    def main(args, input_file, card_type):
        ...
        logging.info('')
        logging.info(
            f"-------------------------------------AtbCsvopstest Begins, op type: {card_type}-------------------------------------")
        ...
        logging.info(
            f"--------------------------------------AtbCsvopstest Ends, op_type: {card_type}--------------------------------------")
        return cases_passed
    ...
    def run_one_file(args, input_file):
        ...
        with open(input_file) as f:
            content = f.read()
            if re.search(
                    "AllGatherOperation|AllReduceOperation|BroadcastOperation|ReduceScatterOperation|LinearParallelOperation|AllToAllVOperation|AllToAllOperation|AllToAllVV2Operation",
                    content, re.I):
                if args.world_size <= 1:
                    args.world_size = 2
                if soc_version == "Ascend310B":
                    args.world_size = 1
                no_space_file, actual_output_file, lines = CsvOpsTestUtil.rm_space_from_input_file(input_file,
                                                                                                   args.output)
                rank_root = random.randint(0, args.world_size - 1)
                os.environ["random_seed"] = str(random.randint(1, 1000))
                mp.spawn(CsvOpsTestUtil.main_worker, nprocs=args.world_size,
                         args=(rank_root, args, no_space_file, actual_output_file))
                os.remove(no_space_file)
        return True
...
def init_args():
    ...
    parser.add_argument('-i', '--input', default='../../../apitest/opstest/csv/testcase_example.csv',
                        help='the input path or csv file to be tested, default: testcase_example.csv')
    parser.add_argument('-o', '--output', default='',
                        help='output path to save the test result csv file, default: input csv file path')
    ...
    parser.add_argument('-t', '--times', default=1, type=int,
                        help='times that testcases should be run, default: 1(performance case\'s default is 101)')
    parser.add_argument('-ll', '--log_level', default='info', choices=['info', 'debug'],
                        help='log level, default: info')
    parser.add_argument('-f', '--format', default='csv', choices=['csv', 'excel', 'html'],
                        help='export result to output file in addtional format, default: csv')
    parser.add_argument('-tt', '--test_type', default='', choices=['Function', 'Performance', 'Generalization'],
                        help='run testcases of specific test type, default: any test type')
    parser.add_argument('-tl', '--test_level', default='', choices=['Level0', 'Level1', 'Level2'],
                        help='run testcases of specific test level, default: any test level')
    parser.add_argument('-m', '--model', default='',
                        choices=['LLaMA-65B', 'ChatGLM-6B', 'LLaMA-Adapter', 'Baichuan2-13B', 'ChatGLM2-6B',
                                 'LLaMA2-13B', 'LLaMA2-7B', 'GPT-NeoX-20B', 'ChatGLM-130B', 'BLOOM-7B'],
                        help='run testcases from specific model, default: any model')
    parser.add_argument('-s', '--soc_version', default='', choices=['Ascend310P', 'Ascend910B'],
                        help='run testcases of specific ascend soc platform, default: local soc platform')
    parser.add_argument('-op', '--operation_name', default='',
                        help='run testcases of specific operations, default: any operation name')
    parser.add_argument('-ws', '--world_size', default=2, type=int,
                        help='the number of cards used for multicard op test, default: 2')
    parser.add_argument('-st', '--save_tensor', action='store_true',
                        help='save tensor data to "output path"/csvopstest')
    parser.add_argument('-ps', '--precision_standard', default='new', choices=['old', 'new'],
                        help='choose precision standard version')
    parser.add_argument('-sv', '--skip_verify', action='store_true',
                        help='skip verifying precision and preformance for op tests')
    parser.add_argument('-gpu', '--gpu_info', default='', help='gpu info to run golden on gpu, format: ip:port')
    parser.add_argument('-ea', '--exit_after', action='store_true',
                        help='exit after running all testcase files rather than exit immediately when running testcase file fails')
    parser.add_argument('-nf', '--no_file', action='store_true',
                        help='not load intensors from paths in column InTensorFile and OutTensorFile')
    return parser
