    ...
    def __precision_check(self, case_sum):
        ...
        for i in range(len(compare_dtype_list)):
            if self.opsTest.args.precision_standard == 'new':
                ...
                if precision_percent[i] >= 99.9 and (compare_dtype_list[i] == torch.float8_e5m2 or compare_dtype_list[i] == torch.float8_e4m3fn or compare_dtype_list[i] == torch.bits8):
                    precision_percent[i] = 100
