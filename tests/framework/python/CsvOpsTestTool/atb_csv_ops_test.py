...
class CsvOpsResult():
    ...
    def __need_to_save_case(self, data, index, args, card_type):
        ...
        if args.operation_name != '' and not re.search(op_name, args.operation_name, re.I):
            return False
        if card_type == 'single_card' and op_name in ['AllGatherOperation', 'AllReduceOperation', 'BroadcastOperation', 'ReduceScatterOperation', 'LinearParallelOperation','AllToAllVOperation','AllToAllOperation']:
            return False
        if card_type == 'multi_card' and op_name not in ['AllGatherOperation', 'AllReduceOperation', 'BroadcastOperation', 'ReduceScatterOperation', 'LinearParallelOperation','AllToAllVOperation','AllToAllOperation']:
            return False
...
class CsvOpsTestUtil:
    ...
    def run_one_file(args, input_file):
        ...
        with open(input_file) as f:
            content = f.read()
            if re.search("AllGatherOperation|AllReduceOperation|BroadcastOperation|ReduceScatterOperation|LinearParallelOperation|AllToAllVOperation|AllToAllOperation", content, re.I):
