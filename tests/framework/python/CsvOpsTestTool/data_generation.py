...
def get_precision_and_eb_threshold(op_type, dtype):
    ...
    if op_type in [OpTypes.CV_FUSION]:
        precision_threshold = 1022 #最大相对误差5/平均相对误差2/均方根误差2
        if dtype in [torch.float16]:
            eb_threshold = 2**(-10)
        if dtype in [torch.bfloat16]:
            eb_threshold = 2**(-7)
...
class LinearOperation(DataGen):
    ...
    def customize(shapes, i, datatype, format, data_gen_ranges, op_params):
        ...
        if i == 2:
            ...
            return MatmulCommon.gen_deq(shapes, i, data_gen_ranges, out_data_type)
        if i == 3:
            out_data_type = MatmulCommon.get_param_value(op_params, "outDataType", -1)
            return MatmulCommon.gen_deq(shapes, i, data_gen_ranges, out_data_type)
...
class GatherOperation(DataGen):
    ...
    def customize(shapes, i, datatype, format, data_gen_ranges, op_params):
        ...
        if i == 0:
            ...
            indexs_data = ((high - low) * torch.rand(indexs_shape) + low).type(dtype_dict[datatype]).long()
...
class AllReduceOperation(DataGen):
    ...
    def golden(in_tensors, op_params):
        ...
        if "quantType" in json_data:   
            return AllReduceOperation.sum_cal_quant(AllReduceOperation.intensors, op_params)
        else:
...
class AllToAllOperation(DataGen):
    ...
    def golden(in_tensors, op_params):
        ...
        for i in range(rankSize):
            golden_out = []
            for j in range(rankSize):
                golden_out_list = AllToAllOperation.intensors[j].reshape(-1).tolist()
...
class BlockCopyOperation(DataGen):
    ...
    def customize(shapes, i, datatype, format, data_gen_ranges, op_params):
        ...
        if i < 2:
            return random(shape, datatype, format, data_gen_ranges, op_params)
        elif i == 2:
    ...
    def golden(in_tensors, op_params):
        ...
        if topktopp_sampling_type == 0:
            ...
            if topktopp_sampling_type == 2:
                ...
                return [outtensor_idx.to(torch.int32), outtensor_probs.to(in_tensors[0].dtype)]
...
class RopeQConcatOperation(DataGen):
    ...
    def golden(in_tensors, op_params):
        ...
        rope_res = rope_res.reshape(token_num, head_num, head_dim)
...
class GatingOperation(DataGen):
    ...
    def golden(in_tensors, op_params):
        json_data = json.loads(op_params)
        topk_expert_num = json_data["topkExpertNum"] if "topkExpertNum" in json_data else 0
        cum_sum_num = json_data["cumSumNum"] if "cumSumNum" in json_data else 0
        device_expert = json_data["deviceExpert"] if "deviceExpert" in json_data else []
        is_ep = len(device_expert) > 0
        if cum_sum_num == 0:
            expert_num = 1
        elif is_ep:
            expert_num = len(device_expert)
            device_expert.sort()
        else:
            expert_num = cum_sum_num

        in_tensor_topk = in_tensors[0].clone()
        out_tensor_token_index = torch.zeros(in_tensor_topk.shape[0], dtype=torch.int32)
        out_tensor_cum_sum = torch.zeros(expert_num, dtype=torch.int32)
        out_tensor_original_index = in_tensors[1].clone()

        cum_sum_pre = torch.zeros(cum_sum_num, dtype=torch.int32)
        cum_sum_full = torch.zeros(cum_sum_num, dtype=torch.int32)

        if cum_sum_num > 0:
            for i in range(in_tensor_topk.numel()):
                expert_index = in_tensor_topk[i]
                cum_sum_pre[expert_index] += 1

            accumulator = 0
            for i in range(cum_sum_num):
                accumulator += cum_sum_pre[i]
                cum_sum_full[i] = accumulator

            accumulator = 0
            for i in range(expert_num):
                if is_ep:
                    accumulator += cum_sum_pre[device_expert[i]]
                else:
                    accumulator += cum_sum_pre[i]
                out_tensor_cum_sum[i] = accumulator

        for i in range(in_tensor_topk.numel() - 1, 0, -1):
            for j in range(0, i):
                if in_tensor_topk[j] > in_tensor_topk[j + 1]:
                    tmp0 = in_tensor_topk[j].clone()
                    in_tensor_topk[j] = in_tensor_topk[j + 1]
                    in_tensor_topk[j + 1] = tmp0
                    tmp1 = out_tensor_original_index[j].clone()
                    out_tensor_original_index[j] = out_tensor_original_index[j + 1]
                    out_tensor_original_index[j + 1] = tmp1

        if cum_sum_num > 0:
            out_tensor_token_index = torch.floor(out_tensor_original_index / topk_expert_num).to(torch.int32)

        if is_ep:
            out_tensor_valid_index = torch.tensor([0], dtype=torch.int32)
            p = 0
            for i in range(expert_num):
                out_tensor_valid_index[0] += cum_sum_pre[device_expert[i]]
                left = 0
                right = cum_sum_full[device_expert[i]]
                if device_expert[i] > 0:
                    left = cum_sum_full[device_expert[i] - 1]
                for j in range(left, right):
                    out_tensor_original_index[p] = out_tensor_original_index[j]
                    out_tensor_token_index[p] = out_tensor_token_index[j]
                    p += 1
            out_tensor_token_index[out_tensor_valid_index.item():] = 0
            out_tensor_original_index[out_tensor_valid_index.item():] = 0
            return [out_tensor_token_index, out_tensor_cum_sum, out_tensor_original_index, out_tensor_valid_index]

        return [out_tensor_token_index, out_tensor_cum_sum, out_tensor_original_index]
    ...
    def customize(shapes, i, datatype, format, data_gen_ranges, op_params):
        ...
        if soc_version == 'Ascend910B':
            if i == 0:
                low = float(data_gen_ranges.split(',')[0])
                high = float(data_gen_ranges.split(',')[1])
                input0 = torch.rand((shapes[0][0], shapes[0][1], shapes[0][2]), dtype=dtype_dict[datatype]).npu()
                return input0
            if i == 1:
                low = float(data_gen_ranges.split(',')[0])
                high = float(data_gen_ranges.split(',')[1])
                input1 = torch.rand((shapes[1][0],), dtype=dtype_dict[datatype]).npu()
                return input1
            if i == 2:
                low = float(data_gen_ranges.split(',')[0])
                high = float(data_gen_ranges.split(',')[1])
                input2 = torch.rand((shapes[2][0], shapes[2][1]), dtype=dtype_dict[datatype]).npu()
                return input2
            if i == 3:
                low = float(data_gen_ranges.split(',')[0])
                high = float(data_gen_ranges.split(',')[1])
                input3 = torch.rand((shapes[3][0], shapes[3][1]), dtype=dtype_dict[datatype]).npu()
                return input3
            if i == 4:
                low = float(data_gen_ranges.split(',')[0])
                high = float(data_gen_ranges.split(',')[1])
                input4 = torch.rand((shapes[4][0], shapes[4][1]), dtype=dtype_dict[datatype]).npu()
                return input4
            if i == 5:
                low = float(data_gen_ranges.split(',')[0])
                high = float(data_gen_ranges.split(',')[1])
                input5 = torch.tensor(np.random.choice(100, shapes[5][0], replace=False), dtype=dtype_dict[datatype]).npu()
                return input5
 
            input6 = torch.zeros((shapes[6][0], shapes[6][1], shapes[6][2], shapes[6][3]), dtype=dtype_dict[datatype]).npu()
            return input6
    ...
    def random(shape, datatype, format, data_gen_ranges, op_params):
        ...
        if soc_version == 'Ascend910B':
            if NormRopeReshapeOperation.random_idx == 1:
                input0 = torch.rand((shape[0], shape[1], shape[2]), dtype=dtype_dict[datatype]).npu()
                return input0
            if NormRopeReshapeOperation.random_idx == 2:
                input1 = torch.rand((shape[0],), dtype=dtype_dict[datatype]).npu()
                return input1
            if NormRopeReshapeOperation.random_idx == 3:
                input2 = torch.rand((shape[0], shape[1]), dtype=dtype_dict[datatype]).npu()
                return input2
            if NormRopeReshapeOperation.random_idx == 4:
                input3 = torch.rand((shape[0], shape[1]), dtype=dtype_dict[datatype]).npu()
                return input3
            if NormRopeReshapeOperation.random_idx == 5:
                input4 = torch.rand((shape[0], shape[1]), dtype=dtype_dict[datatype]).npu()
                return input4
...
class FusedAddTopkDivOperation(DataGen):
    ...
    def golden(in_tensors, op_params):
        ...
        add_output = torch.reshape(add_output, (token_num, group_num, expert_num // group_num))
        ...
        if is_norm:
            ...
            y = gather_output
        return y, sort_output.indices[:, 0:k].to(torch.int32)
