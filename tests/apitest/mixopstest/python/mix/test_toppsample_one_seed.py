...
class TestToppSample(op_test.OpTest):
    ...
    def golden_compare(self, out_tensors, golden_out_tensors):
        rest = torch.nonzero(out_tensors[0] - golden_out_tensors[0])

        if(torch.nonzero(out_tensors[0] - golden_out_tensors[0]).size(0) > 0):
            logging.debug("rest[0]",rest[0][0])
            logging.debug(f"out_tensors {rest[0][0]}",out_tensors[0][rest[0][0]])
            logging.debug(f"golden_out_tensors{rest[0][0]}",golden_out_tensors[0][rest[0][0]])
