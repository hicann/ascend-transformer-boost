...
def process_deq_scale(deq_scale: torch.Tensor) -> np.ndarray:
    ret = torch.frombuffer(deq_scale.numpy().tobytes(), dtype=torch.int32).to(
        torch.int64
    )
    return ret
...
class TestMLAPrepross(op_test.OpTest):
    ...
    def RopeConcatGolden(self, q, sin, cos, concatInput):
        ...
        rope_res = rope_res.reshape(
            self.input_token_num, self.headNum, self.rope_hidden_size
        )
    ...
    def RmsNormAndRopeAndReshapeAndCacheGolden(
        self, x, gamma, keyRope, cos, sin, slotMapping, keycachein
    ):
    ...
    def calc_vec_mm_atb_data(self, N, headNum, data_type):
        ...
        self.input1 = torch.from_numpy(np.random.uniform(-2.0, 2.0, size=(N, 7168))).to(
            data_type
        )  #
        self.gamma1 = torch.from_numpy(
            np.random.uniform(-1.0, 1.0, size=(hiddenStrate))
        ).to(data_type)
        self.quantScale1 = torch.from_numpy(np.random.uniform(-2.0, 2.0, size=(1))).to(
            data_type
        )
        self.quantOffset1 = torch.from_numpy(
            np.random.uniform(-128.0, 127.0, size=(1))
        ).to(torch.int8)
        self.wdqkv = torch.from_numpy(
            np.random.uniform(-2.0, 2.0, size=(2112, 7168))
        ).to(
            torch.int8
        )  #

        self.deScale1 = torch.rand((2112), dtype=torch.float32) / 1000
        self.gamma2 = torch.from_numpy(np.random.uniform(-1.0, 1.0, size=(1536))).to(
            data_type
        )
        self.quantScale2 = torch.from_numpy(np.random.uniform(-2.0, 2.0, size=(1))).to(
            data_type
        )
        self.quantOffset2 = torch.from_numpy(
            np.random.uniform(-128.0, 127.0, size=(1))
        ).to(torch.int8)

        self.wuq = torch.from_numpy(
            np.random.uniform(-2.0, 2.0, size=(headNum * 192, 1536))
        ).to(
            torch.int8
        )  #
        ...
        self.gamma3 = torch.from_numpy(np.random.uniform(-1.0, 1.0, size=(512))).to(
            data_type
        )
        self.sin1 = torch.from_numpy(np.random.uniform(-1.0, 1.0, size=(N, 64))).to(
            data_type
        )
        self.cos1 = torch.from_numpy(np.random.uniform(-1.0, 1.0, size=(N, 64))).to(
            data_type
        )
        self.keyCache = torch.from_numpy(
            np.random.uniform(-1.0, 1.0, size=(blockNum, blockSize, 1, headdim))
        ).to(data_type)

        self.slotMapping = torch.from_numpy(
            np.random.choice(192 * 128, N, replace=False).astype(np.int32)
        ).to(torch.int32)

        self.wuk = torch.from_numpy(
            np.random.uniform(-2.0, 2.0, size=(headNum, 128, 512))
        ).to(
            data_type
        )  #
        self.sin2 = torch.from_numpy(np.random.uniform(-1.0, 1.0, size=(N, 64))).to(
            data_type
        )
        self.cos2 = torch.from_numpy(np.random.uniform(-1.0, 1.0, size=(N, 64))).to(
            data_type
        )

        self.bias1 = torch.from_numpy(
            np.random.randint(-10, 10, (1, 2112)).astype(np.int32)
        ).to(torch.int32)
        self.bias2 = torch.from_numpy(
            np.random.randint(-10, 10, (1, headNum * 192)).astype(np.int32)
        ).to(torch.int32)

        self.beta1 = torch.from_numpy(
            np.random.randint(-2, 2, (hiddenStrate)).astype(np.float16)
        ).to(data_type)
        self.beta2 = torch.from_numpy(
            np.random.randint(-2, 2, (1536)).astype(np.float16)
        ).to(data_type)
        ...
        out_tensors_npu = [
            out_tensors[i] if isinstance(i, int) else i.npu() for i in out_tensors
        ]
        ...
        for size in splitSize:
            ...
            Split1_out_tensors_npu.append(
                torch.zeros(slice_shape, dtype=data_type).npu()
            )
        ...
        OP_PARAM0 = {}
        self.set_param(OP_NAME, OP_PARAM0)
        Split2_out_tensors_npu[1] = (
            Split2_out_tensors_npu[1].cpu().reshape(N, headNum * 64)
        )
        in_tensors = [
            Split2_out_tensors_npu[1],
            self.cos2,
            self.sin2,
            Einsumout_tensors_npu[0],
        ]
        qOut_tensors = [self.qOut_npu]
        in_tensors_npu = [tensor.npu() for tensor in in_tensors]
        qout_tensors_npu = [
            qOut_tensors[i] if isinstance(i, int) else i.npu() for i in qOut_tensors
        ]
        self.mki.execute(in_tensors_npu, qout_tensors_npu)
        qout_tensors_npu[0] = torch.cat(
            [
                qout_tensors_npu[0].cpu()[:, :, 64:],
                qout_tensors_npu[0].cpu()[:, :, :64],
            ],
            dim=2,
        )
        ...
        in_tensors = [
            RmsNormOut_tensors_npu[0],
            Ropeout_tensors_npu[0].cpu().reshape(N, 1, 64),
        ]
    ...
    def calc_vec_mm_data(self, N, headNum, data_type):
        mm1In = self.rms_norm_quant_calc(
            self.input1, self.gamma1, self.beta1, self.quantScale1, self.quantOffset1
        )

        self.rmsquantOut1 = mm1In.clone()
        mm1Out = torch.matmul(
            mm1In.to(torch.float32), self.wdqkv.transpose(0, 1).to(torch.float32)
        )
        ...
        mm2Out = torch.matmul(
            rms2Out.to(torch.float32), self.wuq.transpose(0, 1).to(torch.float32)
        )
        ...
        self.bmmOut = torch.permute(
            torch.matmul(
                torch.permute(mm2Out[:, :, :128], (1, 0, 2)).float(), self.wuk.float()
            ),
            (1, 0, 2),
        )
        self.gg = mm2OutSplit2.clone()
        qOut = self.RopeConcatGolden(
            mm2OutSplit2.reshape(N, headNum * 64), self.sin2, self.cos2, self.bmmOut
        )
    ...
    def compare_data(self, tensor1, tensor2):
        ...
        logging.info(
            "accuracy is correct: %r", (float(strict_error_count) / out_len) <= 0.001
        )

    def golden_compare(self, out_tensors, golden_out_tensors):
        result_double1 = compare_cv(
            self.qOut_npu.reshape(self.qOut.shape[0], -1),
            self.qOut.reshape(self.qOut.shape[0], -1),
            out_tensors[0],
        )
    ...
    @op_test.only_910b
    def test_mla_preprocess_fp16(self):
        N = 32
        headNum = 32
        OP_PARAM = {"N": N, "headNum": headNum}
        ...
        for i in range(1):
            self.execute(
                ...
                [torch.zeros((N, headNum * 576), dtype=data_type), self.keyOutTensor],
            )

    @op_test.only_910b
    def test_mla_preprocess_largeN(self):
        N = 1024
        headNum = 128
        OP_PARAM = {"N": N, "headNum": headNum}
        data_type = torch.float16
    ...
    def test_mla_preprocess_bf16(self):
        ...
        for i in range(1):
            self.execute(
                ...
                [
                    torch.zeros((N, headNum * 576), dtype=torch.bfloat16),
                    self.keyOutTensor,
                ],
            )

    @op_test.only_910b
    def test_mla_preprocess_bf16_largeN(self):
        N = 1024
        headNum = 32
        OP_PARAM = {"N": N, "headNum": headNum}
        data_type = torch.bfloat16
    ...
    def test_mla_preprocess_fp16_gemv(self):
        ...
        headNum = 32
        OP_PARAM = {"N": N, "headNum": headNum}
        self.calc_vec_mm_atb_data(N, headNum, torch.float16)
        self.set_param(OP_NAME, OP_PARAM)
        self.execute(
            [
                self.input1,
            ...
            [
                torch.zeros((N, headNum * 576), dtype=torch.float16),
                self.keyOutTensor,
            ],
        )
    ...
    def test_mla_preprocess_bf16_gemv(self):
        ...
        headNum = 32
        OP_PARAM = {"N": N, "headNum": headNum}
        self.calc_vec_mm_atb_data_bf16(N, headNum, torch.bfloat16)
        self.set_param(OP_NAME, OP_PARAM)
        self.execute(
            [
                self.input1,
