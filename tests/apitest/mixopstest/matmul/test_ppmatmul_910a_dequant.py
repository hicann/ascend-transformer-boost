...
class TestPpMatmul910aDequant(op_test.OpTest):
    ...
    def golden_compare(self, out_tensors, golden_out_tensors):
        ...
        return torch.allclose(out_tensors[0].to(torch.half), golden_out_tensors[0].to(torch.half), rtol=0.002,
                              atol=0.002)
    ...
    def testcase0(self):
        ...
        for bsize, msize, ksize, nsize in case_list:
            ...
            self.execute([torch.tensor(self.bat_A).to(torch.int8), torch.tensor(self.bat_B).to(torch.int8),
                          torch.tensor(self.bias).to(torch.int32), torch.tensor(self.scale).to(torch.float32)],
    ...
    def testcase1(self):
        ...
        logging.debug(self.bat_A.shape)
        self.execute([torch.tensor(self.bat_A).to(torch.int8), torch.tensor(self.bat_B).to(torch.int8),
                      torch.tensor(self.bias).to(torch.int32), torch.tensor(self.scale).float()],
