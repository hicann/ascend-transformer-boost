...
MASK_TYPE_TRIU = 1
RING_ATTENTION_TYPE = 6

FIRST_STEP=True
SECOND_STEP=False
THIRD_STEP=False

class TestRingMLA(operation_test.OperationTest):
    ...
    def set_data_params(self, dynamic_batch=False, batch_state=None, window_size=0, cache_type=0,
                        is_mask=False, is_decoder=False, is_alibi=False, is_razor_fusion = False, alibi_dim=4,
    ...
    def gen_mask(self, batch, heads, data_type, mask_type, window_size, is_compress, cache_type=0):
        ...
        mask_type_dict = {
            # 四维的alibi mask
            MASK_TYPE_ALIBI_WITH_BATCH : ((batch, heads, q_max_seq, kv_max_seq), (lambda mask, idx, q_s, kv_s: (mask[idx, :, kv_s-q_s:kv_s, :kv_s]))),
            ...
            MASK_TYPE_NO_BATCH_WITH_PREFIX : ((1, q_max_seq, kv_max_seq), (lambda mask, idx, q_s, kv_s: (mask[:, kv_s-q_s:kv_s, :kv_s]))),  # this way
        ...
        # kernel中mask的系数
        if data_type == torch.float16:   # this way
            post_mask_coff = 1
        ...
        # print("golden mask shape is: ", self.mask)
    ...
    def softmax(self, x):
        # 对输入 x 进行 softmax 操作，保证在最后一维上进行softmax
        return F.softmax(x, dim=-1)
 
    def compute_wcd(self):
        # 计算 Q * K.T（矩阵乘法）q
        Q = self.q.to("npu")
        K = self.k.to("npu")
        V = self.v.to("npu")
        batch_total = K.shape[0]
 
        head_dim = int(Q.shape[-1]/self.kv_head)
        result = None
        ...
        for batch_id in range(batch_total):
            q_s = self.q_seqlen[batch_id]
            kv_s = self.kv_seqLen[batch_id]
            q_batch = Q[q_offset:q_offset + q_s][:]
            q_batch = q_batch.view(q_s, self.heads, self.embeddim)
            q_batch = torch.permute(q_batch, (1, 0, 2))
 
            k_batch = K[0][batch_id][:kv_s][:]
            k_batch = k_batch.view(kv_s, self.kv_head, self.embeddim)
            k_batch_t = torch.permute(k_batch, (1, 2, 0))
 
            v_batch = V[0][batch_id][:kv_s][:]
            v_batch = v_batch.view(kv_s, self.kv_head, self.embeddimv)
            v_batch = torch.permute(v_batch, (1, 0, 2))
 
 
            tmp_score = None
            for i in range(self.heads):
                qdotk = torch.matmul(q_batch[i:(i + 1), :, :],k_batch_t[i:(i + 1), :, :])
                if tmp_score is None:
                    tmp_score = qdotk
                else:
                    tmp_score = torch.cat((tmp_score, qdotk), 0)
            tmp_score = tmp_score * self.tor
 
            mask = self.mask
            # print(self.kv_seqLen[batch_id])
            mask = mask[:self.kv_seqLen[batch_id], :self.kv_seqLen[batch_id]]
            mask = mask[(self.kv_seqLen[batch_id] - self.q_seqlen[batch_id]):, :].to("npu") * self.post_mask_coff
 
            if tmp_score.shape[0] != mask.shape[0]:
                for i in range(tmp_score.shape[0]):
                    tmp_score[i] = tmp_score[i] + mask
 
            for j in range(self.heads):
                tmp_score[j] = self.softmax(tmp_score[j])
 
 
            result_qkv = None
            for i in range(self.heads):
                qdotk = torch.matmul(tmp_score[i:(i + 1), :, :], v_batch[i:(i + 1), :, :])
                if result_qkv is None:
                    result_qkv = qdotk
                else:
                    result_qkv = torch.cat((result_qkv, qdotk), 0)
 
            result_qkv = result_qkv.view(self.heads, q_s, self.embeddimv)
            result_qkv = torch.permute(result_qkv, (1, 0, 2))
            if out is None:
                out = result_qkv
            else:
                out = torch.cat((out, result_qkv), 0)
        q_offset += q_s
 
        return out.reshape(out.shape[0], self.heads*self.embeddimv)
    ...
    def update_out(self, out, lse):
        ...
        for batch_idx in range(self.batch):
            ...
            # out = out.view(self.q_seqlen[batch_idx], self.heads, self.embeddimv)
            out_i = out[q_start_idx:(q_start_idx+q_len)]
            # self.last_o = self.last_o.view(self.q_seqlen[batch_idx], self.heads, self.embeddimv)  # [q_len, head, embeddimv]
        ...
        # print("out prevo * prev lse shape is: ", tmp_o.shape)
        # new_tmp_o = tmp_o.reshape(3, 2048)
        # print("prevo * prev lse is: ", new_tmp_o)#[0][:512])
 
        # 更新lse  new_lse= lse_max + log(exp(lse_prev - lse_max)+exp(lse_curr - lse_max))    lse_max=max(lse_prev, lse_curr)
        for batch_idx in range(self.batch):
            q_start_idx = sum(self.q_seqlens[:batch_idx])
            q_len = self.q_seqlens[batch_idx]
            lse_i = lse[q_start_idx:(q_start_idx+q_len)].permute(1, 0).unsqueeze(0)  # (1, heads, bs)
            ...
            lse_max = torch.max(lse_i, lse_old)
            new_lse = lse_max + torch.log(torch.exp(lse_old - lse_max) + torch.exp(lse_i - lse_max))  # (1, heads, bs)
            new_lse = new_lse.permute(2, 1, 0)
            if result_lse is None:
                result_lse = new_lse
            else:
                result_lse = torch.cat((result_lse, new_lse), 0)
        # result_lse
        self.new_les = result_lse
        # print("result_lse is: ", result_lse)
        ...
        # print("result_lse_new is: ", result_lse_new)
    ...
    def gen_out_tensor(self, online=False):
        ...
        for idx in range(batch):
            ...
            # print("q_slice shape is: ", q_slice.shape)
            # print("k_slice shape is: ", k_slice.shape)
            ...
            if self.fav3:
                ...
                score *= self.tor  #0.08838834764831843
            ...
            #     if score.shape[0] != temp_mask.shape[0]:
            #         for i in range(score.shape[0]):
            #             score[i] = score[i] + temp_mask[0]
            #     else:
            #         score = score + temp_mask
            # print("score shape is: ", score.shape)
            if is_mask:
                mask2 = torch.triu(torch.ones(size=(score.shape[-1], score.shape[-1]), dtype=torch.float16), 1) * -10000.0
                score = score + mask2
            ...
            score_max = np.max(score, axis=-1)  #  m
            score = score - score_max.reshape((heads, q_s, 1))
            score_exp = np.exp(score)
            score_sum = np.sum(score_exp, axis=-1)  # l   shape is (heads, q_s)
            lse = np.log(score_sum) + score_max
            # print("lse shape is: ", lse.shape)
        ...
        print("self.new lse shap is: ", self.new_les.shape)
    ...
    def gen_out_tensor_bnsd_splitm(self):
        ...
        out = out.view(q_ntokens, heads * embedv)
        out_true = out_true.view(q_ntokens, heads * embedv)
        self.golden_out_o = obnsd.to(self.data_type)
        self.golden_out_true = out_true_bnsd.to(torch.float32)
        logging.debug(f"golden_out shape: {self.golden_out_o.shape}")
    ...
    def compare_output_data(self, out, golden, ratios):
        ...
        # compare_result, compare_result_index = torch.max((torch.abs(golden - out)), dim=-1)
    ...
    def group_mm_torch(self, heads, group_num, A, B, dtype=torch.float16):
        group_head = heads // group_num
        score = None
        for i in range(group_num):
            group_score = torch.matmul(A[i * group_head: (i + 1) * group_head, :, :].npu().to(torch.float16), B[i:(i + 1), :, :].npu().to(torch.float16))
            if score is None:
                score = group_score
            else:
    ...
    #     print("calcute out_tensors first shape is: ", out_tensors[0].shape)
    #     print("calcute out_tensors first is: ", out_tensors[0])#[0][:512])
    #     print("calcute out_tensors second shape is: ", out_tensors[1].shape)
    #     print("calcute out_tensors second is 1: ", out_tensors[1].reshape(-1)[:512])
    #     print("golden_tensors is: ", golden_tensors[0])
    #     print("golden_tensors lse is: ", self.new_les.reshape(-1)[:512])
    ...
    #     # intensor_list, outtensor_list = get_dump_tensor("/home/w30062832/RingMLA/ascend-op-common-lib/tests/pythontest/optest/RingMLA/RingAttn_dump/isring0")
    #     # read_q = intensor_list[0]
    #     # read_k = intensor_list[1]
    #     # read_v = intensor_list[2]
    #     # read_out1 = outtensor_list[0]
    #     # read_out2 = outtensor_list[1]
    #     # print("read_out1 first is: ", read_out1[:512])
    #     # print("read_out2 first is: ", read_out2[:512])
    #     #
    #     # result_single = self.compare_output_data(out_tensors[0].half(), read_out1.reshape(-1).half(), [0.001, 0.001, 0.005, 0.005])
    #     # result_single_lse = self.compare_output_data(out_tensors[1].half(), read_out2.reshape(-1).half(), [0.001, 0.001, 0.005, 0.005])
    ...
    #     atol = 1e-5
    #     etol = 1e-3
    #     is_all_zeros = torch.all(out_tensors[0]==0)
    #     if is_all_zeros:
    #         print("result is all zeros!!!!!")
    #     output = out_tensors[0].to(torch.float32).reshape((-1))
    #     golden = golden_tensors[0].to(torch.float32).reshape((-1))
    ...
    def golden_compare(self, out_tensors, golden_tensors):
        ...
        # print("golden_tensors lse is: ", self.new_les.reshape(-1)[:512])
        # print("self.heads is:", self.heads)
        # print("self.kvhead is:", self.kv_head)
        result_single = self.compare_output_data(out_tensors[0].half(), golden_tensors[0].half(), [0.001, 0.001, 0.005, 0.005])
        result_single_lse = self.compare_output_data(out_tensors[1].half(), golden_tensors[1].half(), [0.001, 0.001, 0.005, 0.005])
        ...
        if not result_single_lse:
            print("softmaxLse wrong precision!")
        return result_single and result_single_lse


    def test_flash_attention_case_fa_encoder_nocache_nomask_template(self):
        '''
        embeddim_v = embeddim, no mask, q_seqlen = kv_seqLen, isring = 0
        '''
        ...
        shape_out_1 = (sum(kv_seqLen), heads, embeddim)  # embeddimV
        shape_out_2 = (sum(kv_seqLen)*heads, 1)
        data_type = torch.float16
        calcType = 0
        if isring:
            old_out = torch.rand(shape_out_1).to(torch.float16)
            old_lse = (torch.rand(shape_out_2) * 10).to(torch.float16)
        else:
            old_out = torch.zeros(shape_out_1, dtype=data_type)
            old_lse = torch.zeros(shape_out_2, dtype=data_type)
            calcType = 1

        output_lse = torch.zeros(shape_out_2, dtype=data_type)
        ...
        self.set_data_params(dynamic_batch = dynamic_batch,
                             ...
                             op_type = RING_ATTENTION_TYPE, mask_type = MASK_TYPE_NO_MASK, no_cache = True, tor = tor,
                             lse=old_lse, last_o=old_out, isring=isring)
        self.gen_out_tensor()
        self.mask = np.reshape(self.mask[:256, :256], (256, 256))
        param_seqlen = self.kv_seqLen
        PARAM = json.dumps(
                {"headNum": heads, "calcType": calcType, "maskType": 0,
                "kvHeadNum": kv_head, "qkScale": tor})
        self.kv_seqLen = torch.from_numpy(np.array(self.kv_seqLen)).to(torch.int32)
        seqlen = torch.stack([self.kv_seqLen])

        kv_ntokens = sum(self.kv_seqLen)
        q_ntokens = kv_ntokens
        embeddimV = embeddim
 
        RUN_PARAM = json.dumps({"seqLen": param_seqlen})
        print("param_seqlen: ", seqlen, "seqlen", seqlen)
    ...
    def test_ring_attention_outlse_head12_fp16(self):
        '''
        embeddim = embeddim, no mask, q_seqlen > kv_seqLen, isring = 0
        '''
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        ...
        kv_head = 2  # kv_head num
        isdecoder = 0  # prefill or decoder
        heads = 2  #e llama7b  hidden_size 4096
        embeddim = 128
        max_seq = 256  # 2048
        tor = 1.0 / math.sqrt(1.0 * embeddim)
        dynamic_batch = False
        kv_seqLen = [256] * batch#,2048]#, 1024, 1500, 2048] * 4
        q_seqLen = [512] * batch#,1024]#, 768, 732, 1024] * 4
        is_clamp = 0
        clamp_min = 0
        clamp_max = 0
        dynamic_batch = False
        ...
        print("param_seqlen: ", seqlen, "seqlen", seqlen)
        self.execute_with_param(OP_NAME, PARAM, RUN_PARAM, [self.q.reshape(q_ntokens, heads, embeddim).npu(),
                                                            self.k.reshape(kv_ntokens, kv_head, embeddim).npu(), self.v.reshape(kv_ntokens, kv_head, embeddim).npu(),
          self.mask.to(data_type).npu(), seqlen.npu()])

    def test_ring_attention_outlse_head1_fp16_diff_embeddim(self):
        '''
        ...
        dynamic_batch = False
        kv_seqLen = [256] * batch #,2048]#, 1024, 1500, 2048] * 4
        q_seqLen = [256] * batch #,1024]#, 768, 732, 1024] * 4
        is_clamp = 0
        clamp_min = 0
        clamp_max = 0
        dynamic_batch = False
        ...
        PARAM = json.dumps(
                {"headNum": heads, "calcType": calcType, "maskType": 0,
                "kvHeadNum": kv_head, "qkScale": tor})
        param_seqlen = np.array(self.q_seqlen).tolist()
        self.kv_seqLen = torch.from_numpy(np.array(self.kv_seqLen)).to(torch.int32)
        self.q_seqlen = torch.from_numpy(np.array(self.q_seqlen)).to(torch.int32)
        seqlen = torch.stack([self.q_seqlen])
        ...
        RUN_PARAM = json.dumps({"seqLen": param_seqlen})
        print("param_seqlen: ", seqlen, "seqlen", seqlen)
        print("old_out.shape", old_out.shape)
        self.execute_with_param(OP_NAME, PARAM, RUN_PARAM, [self.q.reshape(q_ntokens, heads, embeddim).npu(),
                                                            self.k.reshape(kv_ntokens, kv_head, embeddim).npu(), self.v.reshape(kv_ntokens, kv_head, embeddimV).npu(),
                                                            self.mask.to(data_type).npu(), seqlen.npu(),
                                                            old_out.reshape(q_ntokens, heads, embeddimV).npu(), old_lse.reshape(q_ntokens, heads).npu()])

    # def test_ring_attention_outlse_head1_fp16_isring(self):
    #     '''
    #     embeddim == embeddimV, mask (max_seq, max_seq), q_seqlen < kv_seqLen, isring = 1
    #     '''
    #     if not operation_test.get_soc_version() == 'Ascend910B':
    #         print("this testcase only supports Ascend910B")
    #         return
    ...
    #     dynamic_batch = False
    #     kv_seqLen = [1024] * batch#,2048]#, 1024, 1500, 2048] * 4
    #     q_seqLen = [1024] * batch#,1024]#, 768, 732, 1024] * 4
    #     is_clamp = 0
    #     clamp_min = 0 
    #     clamp_max = 0
    #     isring = 1
    ...
    #     if isring:
    #         old_lse = (torch.rand(shape_out_2) * 10).to(data_type)
    ...
    #                          is_clamp=is_clamp, clamp_max=clamp_max, clamp_min=clamp_min,
    #                          data_type=data_type, is_alibi=False,
    #                          op_type=RING_ATTENTION_TYPE, mask_type=MASK_TYPE_NO_BATCH_WITH_PREFIX,
    #                          tor=tor, lse=old_lse, last_o=old_out, isring=isring)
    #     self.gen_out_tensor()
    #     # self.mask = np.reshape(self.mask, (max_seq, max_seq))
    #     mask2 = torch.triu(torch.ones(size=(128, 128), dtype=torch.float16), 1).npu() * -10000.0
    #     PARAM = json.dumps(
    #             {"headNum": heads, "calcType": calcType, "maskType": MASK_TYPE_TRIU,
    #             "kvHeadNum": kv_head, "qkScale": tor})
    #     # param_seqlen = np.concatenate((self.q_seqlen, self.kv_seqLen), 0).tolist()
    #     # param_seqlen = [self.q_seqlen, self.kv_seqLen]
    #     param_seqlen = np.concatenate((self.q_seqlen, self.kv_seqLen), 0).tolist()
    #     self.kv_seqLen = torch.from_numpy(np.array(self.kv_seqLen)).to(torch.int32)
    #     self.q_seqlen = torch.from_numpy(np.array(self.q_seqlen)).to(torch.int32)
    #     seqlen = torch.stack([self.q_seqlen, self.kv_seqLen])
    ...
    #     RUN_PARAM = json.dumps({"seqLen": param_seqlen})
    #     print("param_seqlen: ", param_seqlen, "seqlen", self.q_seqlen, "kv_seqLen", self.kv_seqLen)
    #     self.execute_with_param(OP_NAME, PARAM, RUN_PARAM, [self.q.reshape(q_ntokens, heads, embeddim).npu(),
    #                                                         self.k.reshape(kv_ntokens, kv_head, embeddim).npu(), self.v.reshape(kv_ntokens, kv_head, embeddimV).npu(),
    #                                                         mask2.to(data_type).npu(), seqlen.npu(),
    #                                                         old_out.reshape(q_ntokens, heads, embeddimV).npu(), old_lse.reshape(q_ntokens, heads).npu()])


    # def test_ring_attention_outlse_head8_fp16(self):
    #     if not operation_test.get_soc_version() == 'Ascend910B':
    #         print("this testcase only supports Ascend910B")
    #         return
    #     # unpad encoder
    ...
    #     shape_out_1 = (sum(q_seqLen), heads * embeddimV)
    #     shape_out_2 = (sum(q_seqLen)*heads, 1)
    #     data_type = torch.float16
    #     calcType = 0
    #     if isring:
    #         old_lse = (torch.rand(shape_out_2) * 10).to(torch.float16)
    ...
    #     OP_NAME = "RingMLAOperation"
    #     OP_PARAM = {"type": 6, "qSeqLen": q_seqLen, "kvSeqLen": kv_seqLen,
    #                 "headDimV": embeddimV, "headSize": heads,"isTriuMask":1,
    #                 "tor": tor, "isRing":isring}
    ...
    #     PARAM = json.dumps(
    #                     {"headNum": heads, "calcType": calcType, "maskType": 1,
    #                     "kvHeadNum": kv_head, "qkScale": tor})
    #     param_seqlen = np.concatenate((self.q_seqlen, self.kv_seqLen), 0).tolist()
    #     self.kv_seqLen = torch.from_numpy(np.array(self.kv_seqLen)).to(torch.int32)
    #     self.q_seqlen = torch.from_numpy(np.array(self.q_seqlen)).to(torch.int32)
    ...
    #     RUN_PARAM = json.dumps({"seqLen": param_seqlen})
    #     print("param_seqlen: ", seqlen, "seqlen", self.q_seqlen, "kv_seqLen", self.kv_seqLen)
    #     self.execute_with_param(OP_NAME, PARAM, RUN_PARAM, [self.q.npu(), self.k.npu(), self.v.npu(),
    #       self.mask.to(data_type).npu(), seqlen.npu(), old_out.npu(), old_lse.npu()])
        

# TODO: test the rest


    # def test_ring_attention_update_head8_fp16(self):
    #     if not operation_test.get_soc_version() == 'Ascend910B':
    #         print("this testcase only supports Ascend910B")
    #         return
    #     # unpad encoder
    ...
    #     self.set_input_formats([self.format_nd] * 14)
    ...
    #     return self.execute(
    #         [self.q, self.k, self.v, self.layer_id, self.mask.to(data_type), torch.tensor([], dtype=torch.float),
    ...
    # def test_ring_attention_update_head16_fp16(self):
    #     if not operation_test.get_soc_version() == 'Ascend910B':
    #         print("this testcase only supports Ascend910B")
    #         return
    #     # unpad encoder
    ...
    # def test_ring_attention_update_head32_fp16(self):
    #     if not operation_test.get_soc_version() == 'Ascend910B':
    #         print("this testcase only supports Ascend910B")
    #         return
    #     # unpad encoder
    ...
    # def test_ring_attention_update_head64_fp16(self):
    #     if not operation_test.get_soc_version() == 'Ascend910B':
    #         print("this testcase only supports Ascend910B")
    #         return
    #     # unpad encoder
    #     batch = 1  # 16
    #     kv_head = 64  # kv_head num
    #     isdecoder = 0  # prefill or decoder
    ...
    # def test_ring_attention_update_head128_fp16(self):
    #     if not operation_test.get_soc_version() == 'Ascend910B':
    #         print("this testcase only supports Ascend910B")
    #         return
    #     # unpad encoder
    #     batch = 1  # 16
    #     kv_head = 128  # kv_head num
    #     isdecoder = 0  # prefill or decoder
    #     heads = 128  # llama7b  hidden_size 4096
    #     embeddim = 576
    #     embeddimV = 512
    #     max_seq = 1024  # 2048
    #     tor = 1.0 / math.sqrt(1.0 * embeddim)
    ...
    # def test_ring_attention_update_head1_fp16(self):
    #     if not operation_test.get_soc_version() == 'Ascend910B':
    #         print("this testcase only supports Ascend910B")
    #         return
    #     # unpad encoder
    ...
    # def test_ring_attention_outlse_head1_fp16(self):
    #     if not operation_test.get_soc_version() == 'Ascend910B':
    #         print("this testcase only supports Ascend910B")
    #         return
    #     # unpad encoder
    ...
    #     tor = 1.0 / math.sqrt(1.0 * embeddim)
    #     dynamic_batch = False
    #     kv_seqLen = [1024, 1024, 1500, 2048]# * batch#,2048]#, 1024, 1500, 2048] * 4
    #     q_seqLen = [512, 512, 988, 1024] #* batch#,1024]#, 1024, 1500, 2048] * 4
    #     is_clamp = 0
    #     clamp_min = 0
    #     clamp_max = 0
    #     isring = 1
    ...
    # def test_ring_attention_outlse_head128_kvhead1_fp16(self):
    #     if not operation_test.get_soc_version() == 'Ascend910B':
    #         print("this testcase only supports Ascend910B")
    #         return
    #     # unpad encoder
    ...
    #     tor = 1.0 / math.sqrt(1.0 * embeddim)
    #     dynamic_batch = False
    #     kv_seqLen = [1024] #* batch# , 1024, 1500, 2048]# * batch#,2048]#, 1024, 1500, 2048] * 4
    #     q_seqLen = [512] #* batch# , 512, 988, 1024] #* batch#,1024]#, 1024, 1500, 2048] * 4
    #     is_clamp = 0
    #     clamp_min = 0
    #     clamp_max = 0
    #     isring = 1
    ...
    #     OP_NAME = "RingMLAOperation"
    #     OP_PARAM = {"type": 6, "qSeqLen": q_seqLen, "kvSeqLen": kv_seqLen,
    #                 "headDimV": embeddimV, "headSize": heads, "kvHead": kv_head, "isTriuMask":0,
    #                 "tor": tor, "isRing":isring}
    #     self.set_param(OP_NAME, OP_PARAM)
    #     self.set_input_formats([self.format_nd] * 14)
    #     self.set_output_formats([self.format_nd] * 2)
    ...
    # def test_ring_attention_outlse_head32_kvhead1_fp16(self):
    #     if not operation_test.get_soc_version() == 'Ascend910B':
    #         print("this testcase only supports Ascend910B")
    #         return
    #     # unpad encoder
    ...
    # def test_ring_attention_outlse_head32_kvhead4_fp16(self):
    #     # unpad encoder
    #     batch = 1  # 16
    ...
    # def test_ring_attention_outlse_batch3_head8_kvhead8_fp16(self):
    #     if not operation_test.get_soc_version() == 'Ascend910B':
    #         print("this testcase only supports Ascend910B")
    #         return
    #     # unpad encoder
    ...
    # def test_ring_attention_outlse_batch3_head128_kvhead1_fp16(self):
    #     if not operation_test.get_soc_version() == 'Ascend910B':
    #         print("this testcase only supports Ascend910B")
    #         return
    #     # unpad encoder
