...
# 真值计算
def generate_data(m, n, k, calc_dtype=torch.float32):
    ...
    matmul_result = torch.empty(size=(m, n), dtype=calc_dtype)
    res = torch.matmul(x1.to(torch.float32), weight1.to(torch.float32)).to(calc_dtype)
    res *= scale1[None, :].to(calc_dtype)
    res *= per_token_scale1[:, None].to(calc_dtype)
    matmul_result = res
    ...
    x2 = torch.round(quant_result).to(torch.int8)
    per_token_scale2 = (out_max / 127.).to(torch.float32)
    ...
    res = torch.matmul(x2.to(torch.float32), weight2.to(torch.float32)).to(calc_dtype)
    res *= scale2[None, :].to(calc_dtype)
    res *= per_token_scale2[:, None].to(calc_dtype)
    true_value = res
    ...
    return (x1, weight1, scale1, per_token_scale1, weight2, scale2, true_value)

def generate_data_safe(m, n, k, calc_dtype=torch.float32):
    max_attempt_times = 5
    for _ in range(max_attempt_times):
        data = generate_data(m, n, k, calc_dtype)
        if data is not None:
            return data
...
class TestMmDeqSwigluQuantMmDeq(operation_test.OperationTest):
    ...
    def test_mm_deq_swiglu_quant_mm_deq_1(self):
        ...
        (x1, weight1, scale1, per_token_scale1, weight2, scale2,
            self.true_value) = generate_data_safe(m, n, k)

        self.golden = calculate_golden(x1.npu(), weight1.npu(), scale1.npu(), per_token_scale1.npu(),
