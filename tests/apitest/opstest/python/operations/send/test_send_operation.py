...
def main_worker(rank, world_size, inTensorDtypes, sizes, random_seed):
    ...
    for inTensorDtype in inTensorDtypes:
        ...
        if rank // 2 == 0:
            acl_send_operation = torch.classes.OperationTorch.OperationTorch("SendOperation")
            acl_param = json.dumps({"rank": rank, "rankSize": world_size, "rankRoot": 0, "destRank": rank + 2})
            ...
            acl_recv_operation = torch.classes.OperationTorch.OperationTorch("RecvOperation")
            acl_param2 = json.dumps({"rank": rank, "rankSize": world_size, "rankRoot": 0, "srcRank": rank - 2})
