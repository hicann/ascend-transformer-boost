...
#最大绝对误差：max relative error，MAAE
def get_maae(golden:torch.Tensor, actual:torch.Tensor):
    golden = golden.to(torch.float32)
    abs_error = torch.abs(actual.to(torch.float32) - golden) / (torch.abs(golden) + MIN_ERR)
    mare = torch.max(abs_error.flatten())
    return mare

#平均绝对误差：mean relative error，MEAE
def get_meae(golden:torch.Tensor, actual:torch.Tensor):
    golden = golden.to(torch.float32)
    abs_error = torch.abs(actual.to(torch.float32) - golden) / (torch.abs(golden) + MIN_ERR)
    mere = torch.mean(abs_error)
    return mere
...
def compare_cv(golden:torch.Tensor, gpu:torch.Tensor, actual:torch.Tensor):
    ...
    maae_npu = get_maae(golden, actual)
    maae_gpu = get_maae(golden, gpu)

    meae_npu = get_meae(golden, actual)
    meae_gpu = get_meae(golden, gpu)
    ...
    maae_rate = maae_npu / max(maae_gpu, err_threshold)
    meae_rate = meae_npu / max(meae_gpu, err_threshold)
    rmse_rate = rmse_npu / max(rmse_gpu, err_threshold)

    EB = get_eb(gpu, actual)
    result =(maae_rate < 1) and (meae_rate < 1) and (mere_rate < 2) and (rmse_rate < 2) and (EB < eb_threshold)
