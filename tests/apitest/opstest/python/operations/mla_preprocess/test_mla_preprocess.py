...
def transdata(nd_mat: np.ndarray, block_size: tuple = (16, 16)) -> np.ndarray:
...
class TestMLAPrepross(operation_test.OperationTest):
    ...
    def golden_compare(self, out_tensors, golden_tensors):
        ...
        return True
        # return torch.allclose(out_tensors[0].reshape(out_tensors[0].shape[0], 128, 576).float(), golden_out_tensors[0].float(), rtol=0.001, atol=0.001)
    ...
    def test_mla_preprocess(self):
        ...
        self.execute_out(OP_NAME, PARAM,
                    ...
                    torch_npu.npu_format_cast(torch.from_numpy(transdata(self.wdqkv.numpy(), (16, 32))).contiguous().npu(),29),
                    self.deScale1.npu(),
                    self.bias1.npu(),
                    self.gamma2.npu(),
                    self.beta2.npu(),
                    ...
                    torch_npu.npu_format_cast(torch.from_numpy(transdata(self.wuq.numpy(), (16, 32))).contiguous().npu(),29),
                    self.deScale2.npu(),
                    self.bias2.npu(),
                    self.gamma3.npu(),
                    self.cos1.npu(),
    ...
    def test_mla_preprocess_1(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        N = 56
    ...
    def test_mla_preprocess_2(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        N = 435
