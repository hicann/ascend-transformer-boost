...
class TestMLAPrefill(op_test.OpTest):
    ...
    def test_flash_attention_mla_fp16_case1(self):
        ...
        OP_NAME = "RINGMLAOperation"
        OP_PARAM = {"type": 1, "qSeqLen":kv_seqLen, "kvSeqLen": kv_seqLen, "headDimV": embeddimV,"headSize": heads, "tor": tor, "isTriuMask": 1, "maskType": 1, "isRing":isring}
        self.set_param(OP_NAME, OP_PARAM)
        self.set_input_formats([self.format_nd] * 16)
        self.set_output_formats([self.format_nd] * 2)
        ...
        print("input_lse :", old_lse.shape)
        for i in range(1):
            self.execute([self.q_split1, self.q_split2, self.k_split1, self.k_split2, self.v, self.layer_id, self.mask.to(data_type), torch.tensor([], dtype=torch.float),
                          torch.tensor([], dtype=torch.float), torch.tensor([], dtype=torch.int32),
                          torch.tensor([], dtype=torch.float), torch.tensor([], dtype=torch.int32),
                          torch.tensor([], dtype=torch.float), torch.tensor([], dtype=torch.float),
                          old_out,old_lse],
