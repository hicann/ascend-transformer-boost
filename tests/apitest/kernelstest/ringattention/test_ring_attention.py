...
class TestFlashAttention(op_test.OpTest):
    ...
    def set_data_params(self, dynamic_batch=False, batch_state=None, window_size=0, cache_type=0,
                        is_mask=ISMASK, is_decoder=False, is_alibi=False, is_razor_fusion = False, alibi_dim=4,
    ...
    def update_out(self, out, lse):
        ...
        # 更新lse  new_lse = log(exp(old_lse) + exp(new_lse))
        for batch_idx in range(self.batch):
            q_start_idx = sum(self.q_seqlens[:batch_idx])
            q_len = self.q_seqlens[batch_idx]
            lse_i = lse[q_start_idx:(q_start_idx+q_len)].permute(1, 0).unsqueeze(0)  # (1, heads, bs)
        ...
        self.new_les = result_lse_new.transpose(1, 0)
        print("result_lse_new is: ", result_lse_new)
        print("result_lse_new is: ", result_lse_new.shape)
    ...
    def gen_out_tensor(self, online=False):
        ...
        if self.is_int8_flag:
            ...
            if not self.isring:
                ...
                print("self.new lse is: ", self.new_les)
            else:
                out = out.view(q_ntokens, heads * embedv)
    ...
    def golden_calc(self, in_tensors):
        ...
        golden_out_o = torch.tensor(self.golden_out_o)
        return [golden_out_o, self.new_les]
    ...
    def golden_compare(self, out_tensors, golden_tensors):
        ...
        # intensor_list, outtensor_list = get_dump_tensor("/home/w30062832/ringattention/ascend-op-common-lib/tests/pythontest/optest/ringattention/RingAttn_dump/isring0")
    ...
    def test_flash_attention_case_fa_encoder_nocache_nomask_template(self):
        ...
        OP_NAME = "RingAttentionOperation"
        OP_PARAM = {"type": 6, "qSeqLen": q_seqLen, "kvSeqLen": kv_seqLen, "headSize": heads, "kvHead":kv_head, "headDimV": embeddimV, "tor": tor, "isClamp" : is_clamp,
                    "clampMin" : clamp_min, "clampMax" : clamp_max, "maskType": ISMASK, "isRing":isring, "isTriuMask": 1}
        ...
        self.set_data_params(dynamic_batch = dynamic_batch,
                             ...
                             op_type = OP_PARAM["type"], mask_type = MASK_TYPE_NO_HEAD, no_cache = True, tor = tor,q_seqlens = q_seqLen,
                             lse=old_lse, last_o=old_out, isring=isring, is_mask=ISMASK)
        self.gen_out_tensor()
        ...
        print("input_o :", old_out.shape)
        print("input_o :", old_out)
        print("input_lse :", old_lse)
        # print("input_o :", old_out.reshape(q_seqLen[0], 2048))#[0][:512])
        old_lse = old_lse.transpose(1, 0)
        print("input_lse :", old_lse.shape)

        if ISMASK:
            return self.execute([self.q, self.k, self.v, torch.tensor([], dtype=torch.int), mask2, torch.tensor([], dtype=torch.float),
                                 ...
                                 old_out,old_lse],
                                [torch.tensor(attention_out).half(), output_lse])
        else:
            return self.execute([self.q, self.k, self.v, torch.tensor([], dtype=torch.int), torch.tensor([], dtype=torch.float), torch.tensor([], dtype=torch.float),
                                 torch.tensor([], dtype=torch.float), torch.tensor([], dtype=torch.int32),
                                 torch.tensor([], dtype=torch.float), torch.tensor([], dtype=torch.int32),
                                 torch.tensor([], dtype=torch.float), torch.tensor([], dtype=torch.float),
                                 old_out,old_lse],
