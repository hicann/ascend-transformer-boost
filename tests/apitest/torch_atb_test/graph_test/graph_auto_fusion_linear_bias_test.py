...
def run_test():
    ...
    graph = torch_atb.GraphBuilder("matmul_add_fuse_test") \
        ...
        .add_operation(linear, ["a", "b", "d"], ["c"]) \
        .add_operation(linear_1, ["f", "g", "i"], ["h"]) \
        .add_operation(elewise_add_2, ["c", "h"], ["k"]) \
        .build({"matmul_add"})
    a = torch.ones(m, n, dtype=torch.float16)
    b = torch.ones(k, n, dtype=torch.float16)
    d = torch.ones(1, n, dtype=torch.float16)

    f = torch.ones(m, n, dtype=torch.float16)
    g = torch.ones(k, n, dtype=torch.float16)
    i = torch.ones(1, n, dtype=torch.float16)
