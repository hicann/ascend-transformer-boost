...
def run_test():
    ...
    linear_param.has_bias = False
    linear_param.transpose_b = True
    linear_gelu = torch_atb.Operation(linear_param)
    ...
    graph = torch_atb.GraphBuilder("matmul_add_fuse_test") \
        .set_input_output(["a", "b", "d", "f", "g", "i", "l", "m"], ["k", "o"]) \
        ...
        .add_operation(linear_gelu, ["l", "m"], ["n"]) \
        .add_operation(activate_gelu, ["n"], ["o"]) \
        .build({"matmul_add", "matmul_gelu"})
    ...
    j = torch.randn(m, n, dtype=torch.float16)
    k = torch.randn(k, n, dtype=torch.float16)

    tensors_npu = [tensor.npu() for tensor in [a, b, d, f, g, i, j, k]]
    ...
    def golden():
        ...
        result_gelu = torch.matmul(j, k.transpose(0, 1))
        result_gelu_1 = F.gelu(result_gelu)
        return [result, result_gelu_1]
    ...
    print("cpu_goldens: ", cpu_goldens[1])
    print("npu_outputs: ", npu_outputs[1].cpu())
    ...
    difference = (cpu_goldens[1] - npu_outputs[1].cpu()).numpy()
    print("difference = ", difference)
    mean = np.mean(difference)
    print("差异均值", mean)
    std = np.std(difference)
