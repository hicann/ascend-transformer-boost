...
def run_test():
    ...
    graph = torch_atb.GraphBuilder("matmul_add_fuse_test") \
        .set_input_output(["a", "b", "d", "f", "g", "i", "l", "m"], [ "o"]) \
        ...
        .add_operation(linear_gelu, ["k", "m"], ["n"]) \
    ...
    j = torch.randn(m, k, dtype=torch.float16)
    k = torch.randn(n, k, dtype=torch.float16)
    ...
    def golden():
        ...
        result_gelu = torch.matmul(result, k.transpose(0, 1))
        result_gelu_1 = F.gelu(result_gelu)
        return [result_gelu_1]
