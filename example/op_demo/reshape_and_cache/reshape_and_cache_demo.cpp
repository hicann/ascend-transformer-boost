...
atb::VariantPack PrepareVariantPack(atb::Context *contextPtr, aclrtStream stream)
{
    ...
    atb::Tensor tensorKey = CreateTensorFromVector(contextPtr, stream, keyData, ACL_FLOAT16, aclFormat::ACL_FORMAT_ND,
                                                   {NUM_TOKENS, NUM_HEAD, K_HEAD_SIZE});
    std::vector<float> valueData = KvGeneration(true);
    atb::Tensor tensorValue = CreateTensorFromVector(contextPtr, stream, valueData, ACL_FLOAT16,
                                                     aclFormat::ACL_FORMAT_ND, {NUM_TOKENS, NUM_HEAD, V_HEAD_SIZE});
    // 创建kvCache tensor
    std::vector<float> kCacheData(NUM_BLOCKS * BLOCK_SIZE * NUM_HEAD * K_HEAD_SIZE, 0);
    atb::Tensor tensorKCache =
        CreateTensorFromVector(contextPtr, stream, kCacheData, ACL_FLOAT16, aclFormat::ACL_FORMAT_ND,
    ...
    atb::Tensor tensorVCache =
        CreateTensorFromVector(contextPtr, stream, vCacheData, ACL_FLOAT16, aclFormat::ACL_FORMAT_ND,
...
{
    if (!Is310P()) {
        std::cout << "This ReshapeAndCache demo only supports Atlas A2/A3 products" << std::endl;
        return 0;
    }
