...
Model::Model(const std::string &modelName, const std::string &param) : modelName_(modelName), param_(param)
{
    ...
    envStr = std::getenv("ATB_PLAN_EXECUTE_ASYNC");
...
atb::Status Model::Execute(aclrtStream stream, std::vector<atb::Tensor> &inTensors,
    ...
    for (size_t i = 0; i < node.outTensors.size(); ++i) {
        ...
        if (node.outTensorTypes.at(i) == Model::INTERMEDIATE_TENSOR) {
            if ((uint64_t)node.torchTensors.at(i).numel() != TensorUtil::GetTensorNumel(outTensorDescs.at(i))) {
                ATB_LOG(INFO) << modelName_ << "  nodes[" << nodeId << "] new outtensors[" << i << "]";
                Timer timer;
                node.torchTensors.at(i) = Utils::CreateAtTensorFromTensorDesc(outTensorDescs.at(i));
...
atb::Status Model::ExecutePlanSync(int nodeId)
{
    ...
    Timer timer;
    atb::Status st = node.operation->Execute(variantPack, (uint8_t*)(node.workspace), node.workspaceSize, stream_);
...
// atb::Status DestroyModel(Model *model)
// {
//     if (model) {
//         delete model;
//     }
