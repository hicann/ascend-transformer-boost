...
OperationTorch::OperationTorch(std::string opName) : opName_(opName), name_(opName)
{
    ...
    ATB_LOG(INFO) << "OperationTorch::OperationTorch, TASK_QUEUE_ENABLE:" <<
        c10_npu::option::OptionsManager().CheckQueueEnable() << ", opName:" << opName << ", opId:" << opId_;
}
    ...
    const std::string &varaintPackParam)
{
    ...
    const aclrtStream stream = Utils::GetCurrentStream();
    st = operation_->Execute(variantPack, (uint8_t*)workspace, workspaceSize, stream);
