...
class ChatGLMModel(ChatGLMPreTrainedModel):
    ...
    def __init__(self, config: ChatGLMConfig):
        ...
        self.token_num = [0]
        self.param_seq_len = [0]
        self.seq_len = None

        self.attention_mask_max_full = torch.full(
            (self.max_sequence_length, self.max_sequence_length), -10000.0, dtype=torch.half).npu()
        self.attention_mask_max_increase = torch.full(
        ...
        acl_param = json.dumps({"transKey": True, "dk": 128, "headNum": 32, "layerNum": self.num_layers,
                                ...
                                "tokenOffset": self.token_num, "seqLen": self.param_seq_len, "operationNum": self.num_layers + 3})
    ...
    def forward(
        ...
        if not self.rotary_flag:
            ...
            self.acl_operation_inputs[2] = self.cos
            self.acl_operation_inputs[3] = self.sin
        ...
        if full_flag:
            seqlen = len(seq)
            self.token_num = [seqlen] * batch_size
            self.param_seq_len = self.token_num
            self.token_offset = torch.tensor(self.token_num, device=input_ids.device)
            self.seq_len = torch.tensor(self.param_seq_len, device=input_ids.device)
            self.attention_mask_max_full[:seqlen, :seqlen] = 0
            self.attention_mask_max_full[:seqlen, :seqlen].masked_fill_(attention_mask.squeeze(), -10000)
            self.acl_operation_inputs[0] = input_ids
            self.acl_operation_inputs[1] = position_ids
            self.acl_operation_inputs[2] = self.cos
            self.acl_operation_inputs[3] = self.sin
            self.acl_operation_inputs[4] = self.attention_mask_max_full
            self.acl_operation_inputs[5] = self.k_cache_input
            self.acl_operation_inputs[6] = self.v_cache_input
            ...
            acl_param = json.dumps({"transKey": True, "dk": 128, "headNum": 32, "layerNum": self.num_layers,
                                    "layerNormEps": self.layernorm_epsilon, "residualAddScale": math.sqrt(2 * self.num_layers),
                                    "tokenOffset": self.token_num, "seqLen": self.param_seq_len})
            acl_model_out = self.acl_decoder_operation.execute(self.acl_operation_inputs, acl_param)
            
            self.attention_mask_max_full[:seqlen, :seqlen] = -10000.0
            self.param_seq_len = [1] * batch_size
            self.seq_len = torch.tensor(self.param_seq_len, device=self.kv_cache.device)
        else:
            self.token_num = [i + 1 for i in self.token_num]
            acl_param = json.dumps({"transKey": True, "dk": 128, "headNum": 32, "layerNum": self.num_layers,
                                    "layerNormEps": self.layernorm_epsilon, "residualAddScale": math.sqrt(2 * self.num_layers),
                                    "tokenOffset": self.token_num, "seqLen": self.param_seq_len})
            self.token_offset = self.token_offset + 1
            self.acl_operation_inputs[0] = input_ids
            self.acl_operation_inputs[1] = position_ids
            self.acl_operation_inputs[2] = self.cos
            self.acl_operation_inputs[3] = self.sin
            self.acl_operation_inputs[4] = self.attention_mask_max_increase
            self.acl_operation_inputs[5] = self.k_cache_input
            self.acl_operation_inputs[6] = self.v_cache_input
            self.acl_operation_inputs[7] = self.token_offset
            self.acl_operation_inputs[8] = self.seq_len
        ...
        return BaseModelOutputWithPast(
            last_hidden_state=acl_model_out[0],
...
class ChatGLMForConditionalGeneration(ChatGLMPreTrainedModel):
    ...
    def forward(
        ...
        acl_lm_logits = self.acl_lm_head.execute(
