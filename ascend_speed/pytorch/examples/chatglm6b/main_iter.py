...
def main():
    ...
    while True:
        ...
        if query.strip() == "clear":
            ...
            continue
        count = 0
        for response, history in model.stream_chat_iter(tokenizer, query, history=history):
            if stop_stream:
                stop_stream = False
                break
            else:
...
def test_multi_batch():
    ...
    while True:
        ...
        for response_list, history_list in model.stream_chat_iter(tokenizer, query_list, history=[history]):
            if stop_stream:
                stop_stream = False
                break
            else:
