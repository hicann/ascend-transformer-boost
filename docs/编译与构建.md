...
ATB的基本编译命令是`bash scripts/build.sh`，默认构建模式下生成版本信息，并创建安装包(默认情况下不会编译python接口)。后可跟参数，实现不同功能：
...
- `testframework`：构建测试框架和 C++ 单元测试二进制（`atb_unittest`、`atb_cinterface`、`kernels_unittest`），生成版本信息，并打包测试框架。其中测试二进制只编译不运行，运行方式详见[测试框架指南](测试框架指南.md)
...
若想查看相关变量含义说明，可访问[环境变量参考-昇腾社区](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/910beta1/API/ascendtb/ascendtb_0032.html)。
