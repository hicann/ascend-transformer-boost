...
- 将新增的头文件`src/kernels/include/asdops/params/addcustom.h`加入到`src/kernels/include/asdops/params/params.h`的头文件中:
    ...
    ```
- `src/kernels/configs/mixkernels/op_list.yaml`文件直接删除或者增加以下内容（这一操作非常重要，将新增算子信息加入列表，后续构建才会将新增算子的实现和接口真正编译进去）：
    ```
    AddcustomOperation:
        AddcustomKernel:
            ascend910b: true
     ```
- 在`src/include/atb/core/runner_type.h`中为RunnerType增加枚举值`RUNNER_TYPE_ADDCUSTOM`；
...
您可参考[环境准备](环境准备.md)进行编译编译和测试环境搭建，环境准备好之后，就可以开始ATB的算子开发之旅。
## ATB算子实现
...
Status AddcustomOperation::InTensorDescsCheck(const SVector<TensorDesc> &inTensorDescs) const
{
    ...
    // dtype
    // if (inTensorDescs[0].dtype != ACL_INT32 || inTensorDescs[1].dtype != ACL_INT32) {
    //     ATB_LOG(ERROR) << GetLogPrefix() << "Addcustom only supports int32";
    //     return ERROR_INVALID_PARAM;
    // }
    // shape
